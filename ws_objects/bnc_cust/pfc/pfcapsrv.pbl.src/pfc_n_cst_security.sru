...
If il_numberofrows > 0 Then 
	For li_idx = 1 To il_numberofrows
		...
		//Check if this column is scaned already and property set before
		If ls_column = ls_previous_column Then Continue;
		...
		//Get the previous scaned column so need to do it again
		ls_previous_column = ls_column
	Next
End If
