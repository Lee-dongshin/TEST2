$PBExportHeader$w_cu130_e.srw
$PBExportComments$원자재 출고요청 등록
forward
global type w_cu130_e from w_com010_e
end type
type dw_list from datawindow within w_cu130_e
end type
end forward
...
public function boolean uf_jego (string as_mat_cd);string ls_mat_cd
	...
	and   (INIT_QTY + NMAL_IN_QTY + TRAN_IN_QTY + GGONG_IN_QTY) - (NMAL_OUT_QTY + TRAN_OUT_QTY + GGONG_OUT_QTY + SALE_QTY + SMPLE_QTY + ETC_QTY) - RESV_QTY > 0;
...
il_rows = dw_list.retrieve(is_brand, is_req_ymd, gs_shop_cd, RightA(gs_shop_cd,4), gs_shop_cd)
IF il_rows > 0 THEN
   dw_body.SetFocus()
END IF
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified! THEN				/* New Record */
		if dw_body.getitemstring(1,"mat_cd") = "" then 
			messagebox("확인","자재코드를 입력하세요..")
			return -1
		elseif dw_body.getitemstring(1,"color") = "" then 
			messagebox("확인","자재칼라를 입력하세요..")
			return -1
		elseif dw_body.getitemnumber(1,"qty") = 0 then 
			messagebox("확인","자재수량을 입력하세요..")	
			return -1
		end if
...
il_rows = dw_body.retrieve(is_brand, is_req_ymd, 0, gs_shop_cd, RightA(gs_shop_cd,4), gs_shop_cd, 'N')
	...
	CASE "mat_cd"				
		   ...
		   gst_cd.ai_div          = ai_div
			...
			IF Isvalid(Message.PowerObjectParm) THEN
				...
				if uf_jego(lds_Source.GetItemString(1,"mat_cd")) then 
...
type cb_close from w_com010_e`cb_close within w_cu130_e
end type

type cb_delete from w_com010_e`cb_delete within w_cu130_e
boolean enabled = true
end type

type cb_insert from w_com010_e`cb_insert within w_cu130_e
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_cu130_e
end type

type cb_update from w_com010_e`cb_update within w_cu130_e
end type

type cb_print from w_com010_e`cb_print within w_cu130_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_cu130_e
end type

type gb_button from w_com010_e`gb_button within w_cu130_e
integer width = 3598
end type

type dw_head from w_com010_e`dw_head within w_cu130_e
integer y = 160
integer width = 3598
integer height = 108
string dataobject = "d_cu130_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_cu130_e
integer beginy = 292
integer endx = 3598
integer endy = 292
end type

type ln_2 from w_com010_e`ln_2 within w_cu130_e
integer beginy = 296
integer endx = 3598
integer endy = 296
end type

type dw_body from w_com010_e`dw_body within w_cu130_e
integer y = 304
integer width = 3598
integer height = 692
string dataobject = "d_cu130_d01"
end type
...
type dw_print from w_com010_e`dw_print within w_cu130_e
integer x = 247
integer y = 300
string dataobject = "d_cu130_r01"
end type
...
string title = "none"
string dataobject = "d_cu130_l01"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type

event clicked;string ls_brand, ls_req_ymd, ls_mat_cd
integer li_req_no
...
choose case dwo.name
	...
	case else
		...
		if il_rows > 0 then
			...
			if dw_list.getitemstring(row,"fin_yn") = "Y" then
				cb_delete.enabled = false
				cb_update.enabled = false		
			else
				cb_delete.enabled = true
