$PBExportHeader$w_cu102_d.srw
$PBExportComments$Style별 내역서
forward
global type w_cu102_d from w_com010_d
end type
type dw_mast from datawindow within w_cu102_d
end type
type dw_pic from datawindow within w_cu102_d
end type
end forward
...
global w_cu102_d w_cu102_d

type variables
string  is_style_no, is_flag, is_style, is_chno
end variables
...
this.dw_pic=create dw_pic
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.dw_mast
this.Control[iCurrent+2]=this.dw_pic
end on

on w_cu102_d.destroy
call super::destroy
...
CHOOSE CASE as_column
	CASE "style_no"				
			IF ai_div = 1 THEN 	
				IF gf_style_chk(LeftA(as_data, 8), MidA(as_data, 9, 1), gs_shop_cd) = True THEN
					RETURN 0
				END IF 
			END IF
			   gst_cd.ai_div          = ai_div
				...
				gst_cd.default_where   = " 	WHERE CUST_CD like  '__" + RightA(gs_shop_cd,4) + "' " 
...
type cb_close from w_com010_d`cb_close within w_cu102_d
end type

type cb_delete from w_com010_d`cb_delete within w_cu102_d
integer x = 731
end type

type cb_insert from w_com010_d`cb_insert within w_cu102_d
boolean visible = true
integer x = 1079
string text = "Sketch"
end type

event cb_insert::clicked;call super::clicked;//   dw_pic.reset()
//   is_style_no =  dw_head.GetitemString(1,"style_no")		
//	IF is_style_no = "" OR isnull(is_style_no) THEN		
//		return
//	END IF
//	is_style =  left(is_style_no,8) 
//	is_chno  =  right(is_style_no,1)
//
...
type cb_retrieve from w_com010_d`cb_retrieve within w_cu102_d
end type

type cb_update from w_com010_d`cb_update within w_cu102_d
end type

type cb_print from w_com010_d`cb_print within w_cu102_d
end type

type cb_preview from w_com010_d`cb_preview within w_cu102_d
end type

type gb_button from w_com010_d`gb_button within w_cu102_d
integer width = 3602
end type

type dw_head from w_com010_d`dw_head within w_cu102_d
integer width = 3584
integer height = 148
string dataobject = "d_cu102_h01"
end type
...
type ln_1 from w_com010_d`ln_1 within w_cu102_d
integer beginy = 324
integer endx = 3611
integer endy = 324
end type

type ln_2 from w_com010_d`ln_2 within w_cu102_d
integer beginy = 328
integer endx = 3611
integer endy = 328
end type

type dw_body from w_com010_d`dw_body within w_cu102_d
integer y = 712
integer width = 3602
integer height = 1328
string dataobject = "d_cu102_d02"
boolean hscrollbar = true
boolean hsplitscroll = true
end type

type dw_print from w_com010_d`dw_print within w_cu102_d
integer x = 187
integer y = 568
integer height = 356
string dataobject = "d_cu102_r01"
end type
...
CHOOSE CASE dwo.name
	CASE "cb_prt"	    
		  IF dw_pic.RowCount() = 0 Then
   		  MessageBox("인쇄오류","인쇄할 자료가 없습니다!")
   	     il_rows = 0
        ELSE
           il_rows = dw_pic.Print()
        END IF				
//	CASE "cb_view"		
//		dw_pic.ShareData(dw_print)
