$PBExportHeader$w_sh101_080610.srw
$PBExportComments$판매일보등록
forward
global type w_sh101_080610 from w_com010_e
end type
type dw_1 from datawindow within w_sh101_080610
end type
type cb_1 from commandbutton within w_sh101_080610
end type
type st_1 from statictext within w_sh101_080610
end type
type dw_2 from datawindow within w_sh101_080610
end type
type st_2 from statictext within w_sh101_080610
end type
type tab_1 from tab within w_sh101_080610
end type
...
type st_3 from statictext within w_sh101_080610
end type
type dw_3 from datawindow within w_sh101_080610
end type
type dw_5 from datawindow within w_sh101_080610
end type
type dw_4 from datawindow within w_sh101_080610
end type
type st_4 from statictext within w_sh101_080610
end type
type st_5 from statictext within w_sh101_080610
end type
type dw_6 from datawindow within w_sh101_080610
end type
type dw_7 from datawindow within w_sh101_080610
end type
type dw_8 from datawindow within w_sh101_080610
end type
type dw_9 from datawindow within w_sh101_080610
end type
type dw_10 from datawindow within w_sh101_080610
end type
type dw_11 from datawindow within w_sh101_080610
end type
type dw_12 from datawindow within w_sh101_080610
end type
type dw_list from datawindow within w_sh101_080610
end type
type dw_13 from datawindow within w_sh101_080610
end type
type cb_2 from commandbutton within w_sh101_080610
end type
type dw_back_sale from datawindow within w_sh101_080610
end type
type dw_member_sale from datawindow within w_sh101_080610
end type
type st_6 from statictext within w_sh101_080610
end type
type dw_14 from datawindow within w_sh101_080610
end type
type dw_15 from datawindow within w_sh101_080610
end type
end forward
...
global w_sh101_080610 w_sh101_080610
...
elseif gs_brand = "N" then
	 ...
	 where brand = :gs_brand 
		...
		and color = :ls_color 
		and size  = :ls_size
		and  ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20081' )				
//		and ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20074' 
//           or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) >= '20074'  and plan_yn = 'Y' ))
		and sojae  <> 'C' ;
...
elseif gs_brand = "O" then
	 ...
	 where brand = :gs_brand 
		...
		and  year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20081'           
//		and ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20081' 
//           or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) >= '20081'  and plan_yn = 'Y' ))			  
		and sojae  <> 'C' ;	
...
public function boolean wf_give_rate_chk (integer al_give_rate, string as_coupon_date);long i,j, ll_t_row, ll_give_rate, ll_dc_rate
string ls_coupon_no, ls_t_coupon, ls_brand, ls_style_no, ls_year, ls_season, ls_item, ls_sale_type, ls_style
...
//		if ll_sale_qty > 0 and (isnull(ls_t_coupon) or ls_t_coupon = "") and ll_dc_rate <= 20 and ls_year = '2007' and (ls_season = 'A' or ls_season = 'W')  then		
//			if ll_sale_qty > 0 and (isnull(ls_t_coupon) or ls_t_coupon = "") and ls_year = '2007' and (ls_season = 'A' or ls_season = 'W')  and ( ll_dc_rate  = 0 or (is_use_shop = "WG0001" and  left(ls_style_no,8) = "WW7AH205"))  and (ls_item <> 'Z' or ( is_use_shop = "WG0001" and (left(ls_style_no,8) = "WF7WZ911" OR left(ls_style_no,8) = "WK7AZ118" )) )then		//2007년 FW 정상판매만 10%할인권 적용 기획제외 (2007년 8월말 행상)
//	if ll_sale_qty > 0 and (isnull(ls_t_coupon) or ls_t_coupon = "") and ls_sale_type < '40' and ls_year = '2007' and (ls_season = 'A' or ls_season = 'W')  then						
			
			if ll_sale_qty > 0 &
				and (isnull(ls_t_coupon) or ls_t_coupon = "") &
				and ls_year + ls_season >= '2007W'  &
...
CHOOSE CASE as_column
	...
	CASE "style_no"		
		   ...
		   gst_cd.ai_div          = ai_div
			...
			if MidA(gs_shop_cd,3,3) = '1900' or MidA(gs_shop_cd,3,4) = '1906' or MidA(gs_shop_cd,3,4) = '1907' then
				gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and  year + convert(char(01),dbo.sf_inter_sort_seq('003',season))  >  '20054'  " 				 			
			else
				if gs_brand = "N" then
//	  				   gst_cd.default_where   =  "WHERE brand = '" + gs_brand + "' and ( year + convert(char(01),dbo.sf_inter_sort_seq('003',season))  >  '20073' OR STYLE IN  ('NK7AP227') ) and sojae <> 'C' " 					  
//						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20074'  or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) >= '20074'  and plan_yn = 'Y' ) ) and sojae <> 'C'  " 
						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20081')  and sojae <> 'C'  " 
				elseif gs_brand = "O" then
		 				gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20081' and sojae <> 'C'  " 
//						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20081'  or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) >= '20081'  and plan_yn = 'Y' ) ) and sojae <> 'C'  " 						 
		  		else 
		 				gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20081' and sojae <> 'C'  " 					
//						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and ( ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20074'  OR STYLE IN  ('WW7WE605','WW7WE608','WW7WE609') ) or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) = '20072' and plan_yn = 'Y') ) and sojae <> 'C'  " 
				end if					
			end if	

// OR STYLE IN  ('OK6AD704','OK6AD720','OK6AP704','OL6AJ397','OL6AJ399','OW6AB316','OW6AB339','OW6AB349','OW6AB356','OW6AH389','OK6AZ053')  
//			if  gs_brand = "W" then
// 			gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and ( (year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON))) > '20054'   OR STYLE IN  ('WW5WE061','WW5WE064','WW5WE070','WW5WE082','WW5WE090','WW5WE092','WW5WE093','WW5WE680','WW5WB891')  ) and sojae <> 'C'  " 
//		   else 
//			gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20054'    " 				 
//		   end if
...
type cb_close from w_com010_e`cb_close within w_sh101_080610
boolean visible = false
integer x = 389
end type

type cb_delete from w_com010_e`cb_delete within w_sh101_080610
integer x = 1577
integer taborder = 70
end type

type cb_insert from w_com010_e`cb_insert within w_sh101_080610
boolean visible = false
integer taborder = 60
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh101_080610
integer x = 2469
integer width = 384
...
type cb_update from w_com010_e`cb_update within w_sh101_080610
integer taborder = 50
end type

type cb_print from w_com010_e`cb_print within w_sh101_080610
boolean visible = false
integer taborder = 80
end type

type cb_preview from w_com010_e`cb_preview within w_sh101_080610
boolean visible = false
integer x = 1193
integer y = 48
integer taborder = 90
...
type gb_button from w_com010_e`gb_button within w_sh101_080610
end type

type dw_head from w_com010_e`dw_head within w_sh101_080610
integer y = 152
integer height = 96
...
type ln_1 from w_com010_e`ln_1 within w_sh101_080610
integer beginy = 256
integer endy = 256
end type

type ln_2 from w_com010_e`ln_2 within w_sh101_080610
integer beginy = 260
integer endy = 260
end type

type dw_body from w_com010_e`dw_body within w_sh101_080610
...
type dw_print from w_com010_e`dw_print within w_sh101_080610
end type
