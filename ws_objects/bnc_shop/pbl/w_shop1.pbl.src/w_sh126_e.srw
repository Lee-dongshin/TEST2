...
public function boolean wf_stock_chk (long al_row, string as_style_no, integer ai_qty);String ls_style, ls_chno, ls_color, ls_size , ls_find, ls_style_no, LS_SHOP_TYPE
...
Decimal ldc_marjin
Long   ll_tag_price ,ll_curr_price, ll_out_price, ll_dc_rate,LL_SALE_PRICE
...
Select brand,     year,     season ,item, sojae     , tag_price   
  into :ls_brand,  :ls_year, :ls_season, :ls_item, :ls_sojae, :ll_tag_price
...
//END IF
 ...
 select dbo.SF_GetShop_type(:is_box_ymd,:gs_shop_cd,:ls_style)
 into :ls_shop_type
 from dual;	
 

 select dbo.SF_SalePrice_color(:is_box_ymd,:gs_shop_cd,:ls_shop_type, :ls_style, :ls_color)
 into :LL_SALE_PRICE
 from dual;	
...
if il_rows = 1 then
   dw_body.ResetUpdate()
   commit  USING SQLCA;
	messagebox("알림!", "박스별 작업 완료 후 (13.반품처리 물류의뢰)에서  처리 선택 후 저장 하시면 물류에 반품 처리 의뢰가 완료 됩니다!")
	cb_1.SetFocus()
else
...
string text = "※ 반품의뢰하신 내역을 조회해 보실 수 있습니다."
