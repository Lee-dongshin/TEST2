$PBExportHeader$w_sh191_e.srw
$PBExportComments$예약판매등록
forward
global type w_sh191_e from w_com010_e
end type
type dw_1 from datawindow within w_sh191_e
end type
type cb_1 from commandbutton within w_sh191_e
end type
type st_1 from statictext within w_sh191_e
end type
type st_2 from statictext within w_sh191_e
end type
type dw_10 from datawindow within w_sh191_e
end type
type dw_11 from datawindow within w_sh191_e
end type
type dw_15 from datawindow within w_sh191_e
end type
type st_ok_coupon1 from statictext within w_sh191_e
end type
type st_ok_coupon2 from statictext within w_sh191_e
end type
type shl_member from statichyperlink within w_sh191_e
end type
type dw_17 from datawindow within w_sh191_e
end type
type em_1 from editmask within w_sh191_e
end type
type st_8 from statictext within w_sh191_e
end type
type st_9 from statictext within w_sh191_e
end type
type st_12 from statictext within w_sh191_e
end type
type dw_list from datawindow within w_sh191_e
end type
type shl_manual from statichyperlink within w_sh191_e
end type
end forward
...
long id_remain_point, ll_stock_qty
...
//		and color = :ls_color 
//		and size  = :ls_size
//	   and year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) >= '20151'		
//		and isnull(for_lotte,'N') <>  case when :gs_shop_div = 'L' then 'N' else 'Y' end
//		and sojae  <> 'C'   ;
...
*/
if ls_shop_type > "3"  AND GS_BRAND <> 'J' then 
...
public function boolean wf_stock_chk (long al_row, string as_style_no);String ls_style, ls_chno, ls_color, ls_size , ls_find, ls_shop_type,ls_empty_4
Long   ll_cnt_stop, ll_row_count, i, k, ll, ll_chk_qty, ll_stock_qty1
...
ls_shop_type  = dw_body.getitemstring(al_row, "shop_type")
ls_empty_4 = dw_body.GetItemString(al_row,"empty_4")
...
for i = 1 to ll_row_count 
		...
		ls_find = "style_no = '" + ls_style + ls_chno +  ls_color + ls_size + "' and empty_4 = '" + ls_empty_4 + "'  "
...
this.Control[iCurrent+7]=this.dw_15
this.Control[iCurrent+8]=this.st_ok_coupon1
this.Control[iCurrent+9]=this.st_ok_coupon2
this.Control[iCurrent+10]=this.shl_member
this.Control[iCurrent+11]=this.dw_17
...
this.Control[iCurrent+17]=this.shl_manual
end on

on w_sh191_e.destroy
call super::destroy
...
Long       ll_row_cnt , ll_ord_qty, ll_ord_qty_chn, li_shop_cnt, ll_stock_qty2
Boolean    lb_check 
...
//						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and ( ( ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20104' ) or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) = '20104'  and plan_yn = 'Y' )  )" + &		           								
					...
					elseif gs_brand = "O" then
//						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and (( ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20101' ) )" + &																																				
						gst_cd.default_where   = "WHERE '" + gs_brand_grp + "' like '%' + brand + '%'  and ( ( ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20094' ) or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) = '20094'  and plan_yn = 'Y' )  )" + &
										" or style in (select a.style from tb_56012_d a (nolock)	where a.shop_type = '3'	and a.sale_type = '33' " + &
										" and a.shop_cd = '" + gs_shop_cd + "' and '" + is_yymmdd + "' between a.start_ymd and a.end_ymd " + &
										" and  a.year + a.season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where '" + is_yymmdd + "' between a.frm_ymd and a.to_ymd " + &
										" and a.shop_cd = '" + gs_shop_cd + "' and a.shop_cd = b.shop_cd and a.frm_ymd = b.frm_ymd and b.cancel <> 'Y') )) and sojae <> 'C' and isnull(for_lotte,'N') = 'Y' " 					
...
if ( gs_brand =  "N" or gs_brand =  "J" ) then
...
long ll_rowcnt
string ls_deposit_chk, ls_date, ls_datetime
...
string	ls_sale_type_tmp, ls_style, ls_season, ls_coupon_no_chk , ls_sale_no, ls_cust_ymd
...
string ls_date_time
long ll_sale_cnt
String ps_sale_no, ps_yymmdd, ps_shop_cd, ps_shop_type, ts_sale_no, ps_no, ps_empty_4
...
//예약판매등록 데이터 저장 전 고객인도일 체크///////
//고객 인도일 한번에 등록 또는 추후에 등록 구분하여 저장체크 ///
/// 2018.09.04. 김근호 ///
...
for i=1 to ll_row_count
	...
	if not isnull(Trim(dw_body.GetItemString(i,"empty_4"))) then
		If LenA(Trim(dw_body.GetItemString(i,"empty_4"))) <> 8 then
			messagebox('확인','고객인도일을 확인해주세요.')
			RETURN -1
		else
			if gf_datechk(Trim(dw_body.GetItemString(i,"empty_4"))) = False THEN
				messagebox('확인','고객인도일을 확인해주세요.')
				dw_body.Setitem(i, "empty_4", '')
				RETURN -1
			end if 
		...
		if dw_body.GetItemString(i,"yymmdd") <= dw_body.GetItemString(i,"empty_4") then
			
			
			
		   ps_yymmdd = dw_body.GetItemString(i,"yymmdd")  
			ls_cust_ymd = dw_body.GetItemString(i,"empty_4") //고객인도일을 판매일자로 처리
			ps_shop_cd = dw_body.GetItemString(i,"shop_cd")
			ps_shop_type = dw_body.GetItemString(i,"shop_type")
			ps_sale_no = dw_body.GetItemString(i,"sale_no")
			ps_no = dw_body.GetItemString(i,"no")
			ps_empty_4 = dw_body.GetItemString(i,"empty_4")
			
			//고객인도일 입력체크
			If ls_date_2 < ps_empty_4 then
				messagebox('확인','고객인도일을 미리 입력 하실 수 없습니다.')
				dw_body.Setitem(i, "empty_4", '')
				RETURN -1
			end if
			...
			//sale_no 맥스값 가져오기//
			if ts_sale_no = '0' then
				select right(isnull(max(SALE_NO), 0) + 10001, 4)
				  into :ts_sale_no
				 from tb_53010_h
				where yymmdd = :ls_cust_ymd //:ps_yymmdd
				and shop_cd = :ps_shop_cd
				and shop_type = :ps_shop_type;
				
//			messagebox('확인','고객인도일 ' + left(ps_yymmdd,4) + '/' + mid(ps_yymmdd,5,2) + '/' + mid(ps_yymmdd,7,2) + ' 로 판매등록 되었습니다.')
			messagebox('확인','고객인도일 ' + LeftA(ls_cust_ymd,4) + '/' + MidA(ls_cust_ymd,5,2) + '/' + MidA(ls_cust_ymd,7,2) + ' 로 판매등록 되었습니다.')
			...
			insert into tb_53010_h( yymmdd,
								...
								select 
											...
											empty_10
											from tb_53016_h
											where isnull(empty_4,' ') = ' '  
											and yymmdd = :ps_yymmdd
											and shop_cd = :ps_shop_cd
											and shop_type =:ps_shop_type
											and sale_no = :ps_sale_no
											and no = :ps_no;


		else
			messagebox('확인','판매등록일보다 고객인도일이 빠를 수 없습니다. 확인해주세요.')
			dw_body.Setitem(i, "empty_4", '')
			RETURN -1
		end if
...
type cb_close from w_com010_e`cb_close within w_sh191_e
boolean visible = false
integer x = 389
end type

type cb_delete from w_com010_e`cb_delete within w_sh191_e
integer x = 1769
integer width = 315
integer taborder = 70
end type

type cb_insert from w_com010_e`cb_insert within w_sh191_e
boolean visible = false
integer taborder = 60
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh191_e
integer x = 2469
integer width = 384
...
type cb_update from w_com010_e`cb_update within w_sh191_e
integer taborder = 50
end type

type cb_print from w_com010_e`cb_print within w_sh191_e
integer x = 1376
integer width = 393
integer taborder = 80
...
type cb_preview from w_com010_e`cb_preview within w_sh191_e
boolean visible = false
integer x = 1193
integer y = 48
integer taborder = 90
...
type gb_button from w_com010_e`gb_button within w_sh191_e
end type

type dw_head from w_com010_e`dw_head within w_sh191_e
integer y = 152
integer width = 2601
integer height = 96
string dataobject = "d_sh191_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_sh191_e
integer beginy = 336
integer endy = 336
end type

type ln_2 from w_com010_e`ln_2 within w_sh191_e
integer beginy = 340
integer endy = 340
end type

type dw_body from w_com010_e`dw_body within w_sh191_e
event ue_set_column ( long al_row )
integer x = 5
integer y = 348
integer width = 2885
integer height = 960
string dataobject = "d_sh191_d01_new"
...
CHOOSE CASE dwo.name
	...
	CASE "style_no"	     //  Popup 검색창이 존재하는 항목 
			...
			if gs_brand = 'N' and (gs_shop_cd = 'NG0008' or gs_shop_cd = 'NG0009' or gs_shop_cd = 'NG0037' or gs_shop_cd = 'NG1141' or gs_shop_cd = 'NG0006')  and MidA(ls_set_style,5,4) = 'I099' then
...
type dw_print from w_com010_e`dw_print within w_sh191_e
integer x = 1385
integer y = 224
integer width = 1349
integer height = 948
boolean enabled = false
string dataobject = "d_sh191_p01"
...
long backcolor = 67108864
string text = "※ 고객인도일 등록 후 수정/삭제 불가능합니다. 신중히 등록해주세요!"
boolean focusrectangle = false
end type
...
long textcolor = 255
long backcolor = 67108864
string text = "※ 예약판매 등록 후 일보조회로 고객인도일을 입력하세요."
boolean focusrectangle = false
end type
...
If Trim(this.GetitemString(row, "empty_4")) <> '' then
	cb_delete.enabled = False
else
	cb_delete.enabled = True
end if
...
ls_url = "http://with.ibeaucre.co.kr/instant/manual/dept_shop.asp?v=a02"
//ls_url = "http://new-membership.ibeaucre.co.kr/mem_ins_form.htm?shop_cd="+gs_shop_cd
