...
//messagebox("", is_year_f + is_season_s_f )
...
if ls_shop_type = "4" then
	dw_body.SetItem(al_row, "tag_price", ldc_sale_price_1) 
	...
	if IsNull(ldc_sale_price_1) or ldc_sale_price_1  = 0 then
...
String  	  ls_style_no, ls_size_end, ls_tran_cust, ls_tran_type, ls_work_gubn, ls_year_season, ls_date, ls_shop_type_1, ls_year_season_1, ls_year, ls_season
...
//				   END IF
		   ...
		   gst_cd.ai_div          = ai_div
			...
			if MidA(gs_shop_cd,3,4) = "1900" or MidA(gs_shop_cd,3,4) = "1800" then
		      gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and sojae  <> 'C' and isnull(for_lotte,'N') <> 'Y' " 	
//			elseif mid(gs_shop_cd,2,1) = "B" then
//			      gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and sojae  <> 'C' and isnull(for_lotte,'N') <> 'Y' " 									
//		   elseif mid(gs_shop_cd,2,1) = "X" then
//			      gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and sojae  <> 'C' and isnull(for_lotte,'N') <> 'Y' " 					
...
//															"  and style in ( select style from tb_56012_d (nolock) where brand = '"+ gs_brand + "' and shop_cd = '" + gs_shop_cd + "' and year = '"+is_year_f+"' and season = '"+is_season_f+"' and '"+is_yymmdd+"' between start_ymd and end_ymd )" +&
							gst_cd.default_where	=	" WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'    and ( year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_yymmdd + "' "  + &
															...
															"  or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '"+is_year_f+is_season_s_f+"' ) ) " +& 
															"  and sojae <> 'C' and isnull(for_lotte,'N') <> 'Y' "
...
//															"  and style in ( select style from tb_56012_d (nolock) where brand = '"+ gs_brand + "' and shop_cd = '" + gs_shop_cd + "' and year = '"+is_year_t+"' and season = '"+is_season_t+"' and '"+is_yymmdd+"' between start_ymd and end_ymd )" +&
				...
				ELSEif  gs_brand = "N" or gs_brand = "J"  then
					if ls_date <= is_close_ymd_t then
						//마감일자 전의 판매
						//정상+행사
						if is_s_gubn_f = 'Y' and is_e_gubn_f = 'Y' and is_p_gubn_f = 'Y' then
//															"  and style in ( select style from tb_56012_d (nolock) where brand = '"+ gs_brand + "' and shop_cd = '" + gs_shop_cd + "' and year = '"+is_year_f+"' and season = '"+is_season_f+"' and '"+is_yymmdd+"' between start_ymd and end_ymd )" +&
// 20211125				gst_cd.default_where	=	" WHERE brand LIKE '[NJ]%'  and ( year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_yymmdd + "' "  + &
//							gst_cd.default_where	=	" WHERE brand_GRP LIKE '%" + gs_brand + "%'  and ( year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_yymmdd + "' "  + &							
							gst_cd.default_where	=	" WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'  and ( year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_yymmdd + "' "  + &							
															" between a.frm_ymd and a.to_ymd and a.shop_cd =  '" + gs_shop_cd + "' and a.shop_cd = b.shop_cd and a.frm_ymd = b.frm_ymd  and b.cancel <> 'Y' union select year + season from tb_52011_d (nolock) where brand  LIKE '[NJ]%'  ) " + & 
															"  or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '"+is_year_f+is_season_s_f+"' ) ) " +& 
															"  and sojae <> 'C' and isnull(for_lotte,'N') <> 'Y' "
...
//															"  and style in ( select style from tb_56012_d (nolock) where brand = '"+ gs_brand + "' and shop_cd = '" + gs_shop_cd + "' and year = '"+is_year_t+"' and season = '"+is_season_t+"' and '"+is_yymmdd+"' between start_ymd and end_ymd )" +&
				...
				elseif gs_brand = "B" or gs_brand = 'P' or gs_brand = 'K' or gs_brand = 'U' then
					...
					gst_cd.default_where   = "WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'  and ( year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_yymmdd + "' "  + &
										 " between a.frm_ymd and a.to_ymd and a.shop_cd =  '" + gs_shop_cd + "' and a.shop_cd = b.shop_cd and a.frm_ymd = b.frm_ymd  and b.cancel <> 'Y' union select year + season from tb_52011_d (nolock) where brand = left('" + gs_shop_cd + "',1)  ) " + & 
										 "  or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20103'   )  ) and isnull(for_lotte,'N') <> 'Y' "  
				end if	
...
//			if  ls_shop_type = '4' and ls_year_season <= '20104' and gs_brand = 'N' then 
//				messagebox("경고!", "겨울이전 행사제품 등록은 불가능합니다!")
//				ib_itemchanged = FALSE
//				return 1
//			elseif  ls_shop_type = '4' and ls_year_season <= '20104' and gs_brand = 'O' then 
//				messagebox("경고!", "겨울이전 행사제품 등록은 불가능합니다!")
//				ib_itemchanged = FALSE
//				return 1				
//			end if
	...
	CASE "to_shop_cd"	
		   ...
		   gst_cd.ai_div          = ai_div
			...
			if gs_shop_div = "L" then
				...
				if gs_shop_cd = "NB1890" then
					gst_cd.default_where   = "WHERE Shop_Stat = '00' and shop_div in ('G','K','D','B','X') " + & 
													 "  AND BRAND     = '" + gs_brand + "'" + & 
													 "  AND shop_cd   not in ( '" + gs_shop_cd + "','OB1802','nb1802','bb1802','Pb1802','IK1301', 'IE1913','IE1912','ND1900','OD1900' ) "
				elseif gs_brand = "O" OR gs_brand = "Y"  then
					gst_cd.default_where   = "WHERE Shop_Stat = '00' and shop_div in ('G','K','D','B','X','O') " + & 
													 "  AND BRAND     = '" + gs_brand + "'" + & 
													 "  AND shop_cd   not in ( '" + gs_shop_cd + "','OB1802','nb1802','bb1802','Pb1802','IK1301', 'IE1913','IE1912','ND1900','OD1900' ) "
													 
				else													 
					gst_cd.default_where   = "WHERE Shop_Stat = '00' and ( shop_div in ('G','K','D','B','O')  or shop_cd in ('JX4996','NX4999') ) " + & 
...
//					and sojae  <> 'C'   ;
			...
			elseif gs_brand = "J" then
				...
				Select count(*)
				  ...
				  from vi_12024_1 with (nolock)
				 where style = :ls_style 
					and ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20164' 
						  or ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) >= '20164'  and plan_yn = 'Y' ))
					and sojae  <> 'C' ;
...
//		
		...
		elseif gs_brand = "J"  OR gs_brand = "N" then
...
//				and sojae  <> 'C'
				and sojae <> case when brand <> 'B'  and brand <> 'L' and brand <> 'D' and brand <> 'U'  then 'C' else '' end;	
		end if
	end if	
...
if gs_BRAND = "J" then 
	DW_BODY.object.RT_YN.protect = 0
else
	DW_BODY.object.RT_YN.protect = 1
end if
...
event ue_print();call super::ue_print;dw_print.Object.DataWindow.Print.Orientation = 1  // 0:세로, 1:가로
end event
...
CHOOSE CASE dwo.name
	CASE "style_no"
		IF ( gs_brand = "O" OR gs_brand = "D" ) and gs_brand <> MidA(data,1,1) then
