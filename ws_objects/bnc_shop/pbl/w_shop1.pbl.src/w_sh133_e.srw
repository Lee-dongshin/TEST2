$PBExportHeader$w_sh133_e.srw
$PBExportComments$행사판매등록
forward
global type w_sh133_e from w_com010_e
end type
type cb_1 from commandbutton within w_sh133_e
end type
type dw_list from datawindow within w_sh133_e
end type
type st_1 from statictext within w_sh133_e
end type
type dw_1 from datawindow within w_sh133_e
end type
type gb_1 from groupbox within w_sh133_e
end type
type dw_3 from datawindow within w_sh133_e
end type
type dw_2 from datawindow within w_sh133_e
end type
type st_2 from statictext within w_sh133_e
end type
type dw_4 from datawindow within w_sh133_e
end type
type st_3 from statictext within w_sh133_e
end type
end forward
...
global w_sh133_e w_sh133_e

type variables
String is_yymmdd, is_sale_no, is_fr_year, is_fr_season, is_mj_bit = 'N', is_chk_back = 'N', is_back_ymd
...
public function boolean wf_stock_chk (long al_row, string as_style_no);String ls_style, ls_chno, ls_color, ls_size , ls_find, ls_shop_type, ls_shop_cd, ls_dotcom
...
if MidA(gs_shop_cd,1,2) = 'NH' or ls_dotcom = '1' then
	ls_shop_cd = gs_brand + "G" + MidA(gs_shop_cd,3,4)
end if	

select dbo.sf_get_stockqty(:ls_shop_cd, :ls_shop_type, :ls_style, :ls_chno, :ls_color, :ls_size)
  into :ll_stock_qty
  from dual;
...
//	   end if
	...
	if IsNull(ls_shop_type) or Trim(ls_shop_type) = "" then		
		   ...
		   and end_ymd   >= :as_yymmdd
		   and shop_cd    = :ls_shop_cd
			and shop_type  = case when :ls_plan_yn = 'Y' then '3' else '4' end
			and sale_type > '32'	
			and year   = :ls_year 
...
//	   end if
	...
	if MidA(gs_shop_cd,3,6) = "2000" then 	
		...
		if IsNull(ls_shop_type) or Trim(ls_shop_type) = "" then		
			messagebox("경고!", "해당품번에 마진이 없습니다!" + ls_shop_cd )
			RETURN False 			
		end if	 
	end if	
...
if gs_shop_cd = 'OD0002' then
	...
	if IsNull(ls_dot_com) or Trim(ls_dot_com) = "" then
		ls_dot_com = "0"
   end if 
end if	
...
/* 수정일      : 2002.02.15                                                  */
/*===========================================================================*/
String ls_style, ls_chno, ls_color,  ls_size, ls_shop_type, ls_given_fg, ls_given_ymd,ls_dotcom
String ls_brand, ls_year, ls_season, ls_sojae, ls_item, ls_plan_yn , ls_shop_cd, ls_dep_fg,ls_shop_div
Long   ll_tag_price , ll_ord_qty, ll_ord_qty_chn, ll_stock_qty, ls_chk_back, ll_ret
...
elseIF GS_BRAND = "N" then
		Select brand,     year,     season,     
		  ...
		  into :ls_brand, :ls_year, :ls_season, 
			 ...
			 where style = :ls_style 
				...
				and color = :ls_color 
				and size  = :ls_size
  			   and plan_yn <>  case when substring(:gs_shop_cd,1,2)  = "NI" then 'X' else 'Y' end					 
				and size    <>  case when  substring(:gs_shop_cd,3,6) = "2000" then 'CC' else 'XX' end
				and year + season in (	select year + season
												from tb_51036_d a (nolock), tb_51035_h b (nolock)
												where :is_yymmdd between a.frm_ymd and a.to_ymd
		...
		Select brand,     year,     season,     
		  ...
		  into :ls_brand, :ls_year, :ls_season, 
				 :ls_sojae, :ls_item, :ll_tag_price, :ls_plan_yn, :ls_dep_fg     
			  from vi_12024_1 with (nolock)
			 where :gs_brand_grp like '%' + brand + '%' //brand = :gs_brand 
				and style = :ls_style 
				and chno  = :ls_chno
				and color = :ls_color 
				and size  = :ls_size
...
IF SQLCA.SQLCODE <> 0 THEN 
	if  MidA(gs_shop_cd,1,2) = "NI" then
			ll_ret =  messagebox("경고!", "현재 행사품번이 아니거나 행사기간이 아닙니다! 반품등록 하시겠습니까? ", 	Exclamation!, OKCancel!, 2)
			IF ll_ret = 1 THEN
				is_chk_back = "Y"
			ELSE
				is_chk_back = "N"
				Return False 
			END IF
	elseif  MidA(gs_shop_cd,2,1) <> "I" then
			messagebox("경고!", "현재 행사품번이 아니거나 행사기간이 아닙니다!")
			is_chk_back = "N"
			Return False 		
	else		
			is_chk_back = "N"
...
IF ll_tag_price = 0 OR ISNULL(ll_tag_price) THEN
	Select brand,     year,     season,     
		 sojae,     item,     tag_price,     plan_yn, 	dep_fg
	into :ls_brand, :ls_year, :ls_season, 
		 :ls_sojae, :ls_item, :ll_tag_price, :ls_plan_yn, :ls_dep_fg     
...
if ls_plan_yn = "Y" and MidA(gs_shop_cd, 2,1) <> "I" then
	messagebox("알림!" , "행사전용 매장코드가 아닌 경우 기획 제품은 정상판매일보 등록에서만 입력 가능합니다!")
	Return False 
end if
...
if is_chk_back = 'Y' and  MidA(gs_shop_cd,1,2) = "NI" then 
	
//	select max(yymmdd)
//	into :is_back_ymd
//	from tb_42020_h with (nolock)
//	where shop_cd = :ls_shop_cd
//	and shop_type in ('3','4')
//	AND STYLE  = :LS_STYLE
//	AND margin_rate <> 0 ;
	...
	where yymmdd >= convert(char(08), dateadd(mm, -1,getdate()), 112)
	and shop_cd = :ls_shop_cd
	and shop_type in ('3','4')
	AND STYLE  = :LS_STYLE
	and sale_qty > 0
...
if isnull(is_back_ymd) or trim(is_back_ymd) = "" then 
	if MidA(ls_shop_cd,1,2) = "NI" then
   		messagebox('판매 확인','한달이내 행사 판매내역이 없습니다. 확인 바랍니다.')
			return false			
	end if		
	is_back_ymd = is_yymmdd
end if	
...
if IsNull(ls_dotcom) or Trim(ls_dotcom) = "" then
	ls_dotcom = '0'
  // MessageBox("style_chk",ls_dotcom)	
end if

if MidA(gs_shop_cd,1,2) = "NG" and ls_dotcom = '1' then 
...
////////////////////////////////////////////////////////////////////	
	...
	Select shop_type
		...
		Where style      = :ls_style 
		  and color     like :ls_color
		  and start_ymd <= :is_back_ymd
		  and end_ymd   >= :is_back_ymd
		  and shop_cd    = :ls_shop_cd;
...
IF wf_style_set(al_row, ls_style, is_back_ymd, ldc_sale_qty) THEN 
   ...
   dw_body.SetItem(al_row, "style",    ls_style)
	...
	wf_style_set(al_row, ls_style, is_back_ymd, ldc_sale_qty)
ELSE
	Return False
END IF
...
String     ls_empno, ls_qry, ls_year, ls_season,ls_dotcom, ls_shoP_cd, ls_shop_div
...
//		messagebox("is_chk_back", is_chk_back)
//		messagebox("is_back_ymd", is_back_ymd)		
//		if is_chk_back = "Y" then 
//			if gs_brand = "B" or gs_brand = "P" or gs_brand = "K" or gs_brand = 'U' then 					
//				gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and plan_yn <> 'Y' and  year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_back_ymd + "' "  + &
//											 " between a.frm_ymd and a.to_ymd and a.shop_cd =  '" + gs_shop_cd + "' and a.shop_cd = b.shop_cd and a.frm_ymd = b.frm_ymd and b.cancel <> 'Y') "  
//			else
//				gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and plan_yn <> case when substring('"  + gs_shop_cd + "', 1,2) = 'NI' then 'X' else 'Y' end  and  year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_back_ymd + "' "  + &
...
//		else	
			...
			elseIF  gs_brand = "N" THEN
				gst_cd.default_where   = "WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'  and plan_yn <> case when substring('"  + gs_shop_cd + "', 1,2) = 'NI' then 'X' else 'Y' end  and  year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_yymmdd + "' "  + &
											 " between a.frm_ymd and a.to_ymd and a.shop_cd =  '" + gs_shop_cd + "' and a.shop_cd = b.shop_cd and a.frm_ymd = b.frm_ymd and b.cancel <> 'Y') and sojae <> 'C' "  
...
//		end if	
	  ...
	  if MidA(gs_shop_CD,3,6) = "2000" then							
		...
		ELSe 
			...
			if is_chk_back = "Y" and is_back_ymd <> is_yymmdd and MidA(gs_shop_cd, 1,2) = "NI" then 
						...
						Where style      = :ls_style 
						  ...
						  and start_ymd <= :is_back_ymd
						  and end_ymd   >= :is_back_ymd
						  and shop_type >= case when substring(shop_cd, 1,2) = 'NI' then '3' else '4' end
						  and sale_type >= case when substring(shop_cd, 1,2) = 'NI' then '33' else '40' end
...
//				end if	 		
				...
				if IsNull(ls_shop_type) or Trim(ls_shop_type) = "" then		
						messagebox("경고! POPUP", "해당품번의 마진이 없습니다!")
					   dw_body.SetItem(al_row, "style_no", "")
						ib_itemchanged = FALSE
						return 1 
				end if	 		
...
//	and a.shop_cd = :gs_shop_cd ;
   ...
   into :ll_cnt, :is_fr_year, :is_fr_season
	...
	and a.shop_cd = :gs_shop_cd ;	
		...
		if ll_cnt < 1 then
				if MidA(gs_shop_cd ,2,1) = 'G' or MidA(gs_shop_cd ,2,1) = 'K' or MidA(gs_shop_cd ,2,1) = 'I' then
						st_2.text = "※ 등록은 진행 행사가 있는 기간에만 가능합니다. "
						
						ll_ret =  messagebox("경고!", "행사기간이 아닙니다! 반품등록 하시겠습니까? ", 	Exclamation!, OKCancel!, 2)
						IF ll_ret = 1 THEN
							is_chk_back = "Y"
							st_2.text = "※ 행사기간 종료로 1개월내 판매분 반품만 가능!" // (반품시 품번13자리 전체 입력!) "
						//	st_3.text = "※ 행사기간이 지난 품번은 검색 불가하니 반품시 품번13자리 입력 해주세요!" // (반품시 품번13자리 전체 입력!) "							
			...
			if MidA(gs_shop_cd ,2,1) <> 'I' then
				st_2.text = "※ 기획제품은 메뉴 (01.판매일보관리)에서만 등록 가능합니다. "
			else
				st_2.text = ""	
			end if	
			//	st_3.text = ""					
...
int     li_point_seq	
String   ls_shop_type, ls_sale_type, ls_shop_cd, ls_shop_cd2, ls_card_gubn, ls_pay_way,ls_dot_com, ls_shop_div, ls_brand
...
FOR i=1 TO ll_row_count
  ...
  if	MidA(gs_shop_cd,1,2) = "NH" then 
	  ls_shop_cd2 = "NG" + RightA(gs_shop_cd,4)
  else 	  
	  ls_shop_cd2 = gs_shop_cd
  end if	  
...
//NW6SO652 0차로 찍히면 1차로 변경해서 저장함.(by 윤상혁)20160325
		...
		if ( ll_stock_qty <= 0  or  IsNull(ll_stock_qty)) and ll_sale_qty > 0 and ls_style <> 'NW6SO652' then 
	   ...
	   if ll_stock_qty >= 0 and ll_stock_qty < ll_sale_qty and ls_style <> 'NW6SO652' then 
		 ...
		 if MidA(gs_shop_cd,1,2) = "NI" and  wf_stock_chk(i,  ls_style + ls_chno + ls_color + ls_size) = false then 
			MESSAGEBOX("알림!" , ls_style + ls_chno + ls_color + ls_size + " 는 재고가 없는 품번으로 판매가 제한됩니다!")
		   return 0
	   end	if
...
if gs_brand = "I" then
	for m = 1 to ll_row_count
		if dw_body.Getitemnumber(m, "sale_qty") > 0 then
			messagebox("경고!","코인코즈 판매등록은 반품만 가능합니다! 정상 등록시 영업관리팀에 확인하세요!")
			Return 0 
		end if
	next
end if
...
type cb_close from w_com010_e`cb_close within w_sh133_e
boolean visible = false
integer x = 389
end type

type cb_delete from w_com010_e`cb_delete within w_sh133_e
integer taborder = 70
end type

type cb_insert from w_com010_e`cb_insert within w_sh133_e
integer taborder = 60
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh133_e
integer x = 2469
integer width = 384
...
type cb_update from w_com010_e`cb_update within w_sh133_e
integer taborder = 50
end type

type cb_print from w_com010_e`cb_print within w_sh133_e
boolean visible = false
integer taborder = 80
end type

type cb_preview from w_com010_e`cb_preview within w_sh133_e
boolean visible = false
integer x = 1193
integer y = 48
integer taborder = 90
...
type gb_button from w_com010_e`gb_button within w_sh133_e
end type

type dw_head from w_com010_e`dw_head within w_sh133_e
integer x = 0
integer y = 152
integer height = 96
...
type ln_1 from w_com010_e`ln_1 within w_sh133_e
integer beginy = 332
integer endy = 332
end type

type ln_2 from w_com010_e`ln_2 within w_sh133_e
integer beginy = 336
integer endy = 336
end type

type dw_body from w_com010_e`dw_body within w_sh133_e
event ue_set_column ( long al_row )
integer x = 9
integer y = 348
integer height = 832
string dataobject = "d_sh133_d01"
...
type dw_print from w_com010_e`dw_print within w_sh133_e
end type
...
long textcolor = 8388736
long backcolor = 67108864
string text = "※ 행사기간이 지난 품번은 검색 불가하니 반품시 품번13자리 입력 해주세요!"
boolean focusrectangle = false
end type
