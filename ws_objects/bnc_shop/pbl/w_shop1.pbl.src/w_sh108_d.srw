...
type rb_6 from radiobutton within w_sh108_d
end type
...
select ISDATE(:is_fr_ymd)
into :ll_day_chk
from dual;

if ll_day_chk <> 1 then
	MessageBox(ls_title,"일자가 잘못 되었습니다. 시작일자를 확인해주세요!")
	dw_head.SetFocus()
...
//		IF len(trim(data)) = 4 then RETURN 0		
...
if gs_brand = '%' then
	...
	messagebox("알림!", "복합매장의 경우 각 브랜드별 매장으로 로그인하여 확인가능합니다!")
	...
	dw_body.DataObject = "d_sh108_d06"
	dw_body.SetTransObject(SQLCA)
end if
