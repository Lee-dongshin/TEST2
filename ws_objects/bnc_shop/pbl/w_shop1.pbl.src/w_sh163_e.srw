...
type st_4 from statictext within w_sh163_e
end type
type st_5 from statictext within w_sh163_e
end type
end forward
...
elseIF LeftA(upper(GS_SHOP_CD),1) = 'D' then // like 'NG0008' or upper(GS_SHOP_CD) =  'NG1141' or upper(GS_SHOP_CD) =  'NG1092' or upper(GS_SHOP_CD) =  'NG1080' or upper(GS_SHOP_CD) =  'NG1100' THEN 	
elseIF LeftA(upper(GS_SHOP_CD),1) = 'A' then // like 'NG0008' or upper(GS_SHOP_CD) =  'NG1141' or upper(GS_SHOP_CD) =  'NG1092' or upper(GS_SHOP_CD) =  'NG1080' or upper(GS_SHOP_CD) =  'NG1100' THEN 		
else	
...
string   ls_invoice_com_shop, ls_invoice_no_shop, ls_c_addr_chk, ls_brand
...
CHOOSE CASE dwo.name
	...
	CASE "invoice_com_shop"		
		   IF data = "한진신용" THEN
					dw_body.Setitem(row, "invoice_shop_amt",  0)
	      END IF
	...
	CASE "rt_accept"
		 ...
		 if ls_rt_proc = "N"  then
			...
			if MidA(gs_shop_cd,1,1) = 'O' or MidA(gs_shop_cd,1,1) = 'D' or MidA(gs_shop_cd,1,1) = 'A' then
					...
					if ls_yymmdd < "20201112" then
						...
						select isnull(rt_accept,"N"), isnull(fr_shop_cd,'XXXXXX')
							...
							if ls_accept_chk = 'Y'  and  ls_fr_shop_cd <> gs_shop_cd  then
								...
								parent.Trigger Event ue_retrieve()
								 ...
								 DECLARE SP_SH163_UPDATE1 PROCEDURE FOR SP_SH163_UPDATE  
										@yymmdd   = :ls_yymmdd,   
										@ord_ymd  = :ls_ord_ymd,   
										@shop_cd  = :ls_shop_cd,
										@order_no = :ls_order_no,   			
						...
						if data = "C" then
							Net = MessageBox("경고", "거부시 해당 요청을 다시 발송 할 수 없습니다. 계속하시겠습니까?", Exclamation!, OKCancel!, 2)
						elseif data = "Y" then						
							Net = 1
						else 
							Net = 0
						...
						select isnull(rt_accept,"N"), isnull(fr_shop_cd,'XXXXXX')
							...
							if ( ls_accept_chk = 'Y' or ls_accept_chk = 'C') and  ls_fr_shop_cd <> gs_shop_cd  then
								messagebox("알림!", "지시불이행처리 또는 발송예약 처리를 한 품번 입니다.")
								parent.Trigger Event ue_retrieve()
								...
								if Net = 1 then
									 DECLARE SP_SH163_UPDATE2 PROCEDURE FOR SP_SH163_UPDATE2  
											@yymmdd   = :ls_yymmdd,   
											@ord_ymd  = :ls_ord_ymd,   
											@shop_cd  = :ls_shop_cd,
											@order_no = :ls_order_no,   			
...
string text = "오후 4시 30분 이후 발송 선택이 불가능합니다."
boolean focusrectangle = false
end type
...
long textcolor = 255
long backcolor = 67108864
string text = "※ 택배 송장정보와 금액을 꼭 확인 입력 하세요."
boolean focusrectangle = false
end type
...
long textcolor = 255
long backcolor = 67108864
string text = "※ 거부 클릭과 동시에 해당 요청은 더 이상 처리 불가능합니다."
boolean focusrectangle = false
end type
