...
ls_size  = MidA(as_style_no, 12, 2)
   ...
   ls_house_cd = dw_body.getitemstring(al_row, "house_cd")
	...
	if LeftA(ls_style,1) = "O" or LeftA(ls_style,1) = "D" then
		if ll_stock_qty <= 0 OR ISNULL(ll_stock_qty) then
					messagebox("알림!", as_data + " " +  "창고에 재고가 없는 품번입니다. 출고요청이 불가합니다!")
					RETURN 1 
		end if	
   elseif	LeftA(ls_style,1) = "N" or LeftA(ls_style,1) = "M" or LeftA(ls_style,1) = "E"  then		  
		if ll_stock_qty <= 0 OR ISNULL(ll_stock_qty) then
					messagebox("알림!", as_data + " " +  "창고에 재고가 없는 품번입니다. 출고요청이 불가합니다!")
					RETURN 1 
		end if	
...
ll_row_count = dw_body.RowCount()
  ...
  if gs_brand = "Y" and ll_stock_qty <= 0 then
...
//messagebox("is_out_style",		is_out_style)
		   ...
		   gst_cd.ai_div          = ai_div
			...
			IF GS_BRAND <> "Y" THEN
				if is_out_style = "ALL" then
					gst_cd.default_where   = "WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'" + &						
													 "  and plan_yn = 'N'"			
				elseif is_out_style = "Ost" then												 
					gst_cd.default_where   = "WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'" + &						
													 " and brand = 'O' and plan_yn = 'N'"			
				elseif is_out_style = "Yst" then												 
					gst_cd.default_where   = "WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'" + &						
													 " and brand = 'Y' and plan_yn = 'N'"																 
				else													 
					gst_cd.default_where   = "WHERE  '" + gs_brand_grp + "' like '%' + brand + '%'" + &						
													 "  and plan_yn = 'N'"			
			...
			IF Trim(as_data) <> "" THEN
				gst_cd.Item_where = "style  LIKE '" + ls_style + "%'" + &
				                ...
				                " and size  LIKE '" + ls_size + "%'" + &
									 ...
									 " and style + color  NOT IN (SELECT D.STYLE + d.color  FROM TB_52040_m D (nolock) WHERE  d.shop_BRAND   = left( '" + gs_shop_cd + "' ,1) " + &
