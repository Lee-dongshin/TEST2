$PBExportHeader$w_sh128_e.srw
$PBExportComments$매장RT 요청등록
forward
global type w_sh128_e from w_com010_e
end type
type st_1 from statictext within w_sh128_e
end type
type st_2 from statictext within w_sh128_e
end type
type dw_1 from datawindow within w_sh128_e
end type
type dw_2 from datawindow within w_sh128_e
end type
end forward
...
public function boolean wf_stock_chk (long al_row, string as_style_no);String ls_style, ls_chno, ls_color, ls_size , ls_find, ls_year, ls_temp
Long   ll_stock_qty, ll_cnt_stop, ll_row_count, i, k, ll, ll_chk_qty, ll_out_qty, ll_rtrn_qty, ll_sale_qty
...
ls_temp =  MidA(ls_style,3,1)

	select DBO.SF_INTER_CD1('002', :ls_temp)
	into 	:ls_year
	from dual;



	select sum(ISNULL(A.out_qty,0)), sum(ISNULL(A.rtrn_qty,0)), sum(ISNULL(A.sale_qty,0))
	into :ll_out_qty, :ll_rtrn_qty, :ll_sale_qty
	 from tb_44012_s a (nolock)
	 where a.shop_cd 	= :gs_shop_cd
...
dw_body.Setitem(al_row, "out_qty", ll_out_qty)
dw_body.Setitem(al_row, "rtrn_qty", ll_rtrn_qty)
...
if gs_brand = "O" then
	if li_day_no > 4 or  li_day_no = 1 then	
//		st_2.text = "※ 요청등록은 일요일과 월요일에만 가능합니다!"
		st_2.text = "※ 요청등록 및 수정은 목요일부터 일요일까지 가능합니다!"
	
	else
		st_2.text = "※ 요청등록 및 수정은 목요일부터 일요일까지 가능합니다!"
		messagebox("알림!" , "요청등록 및 수정은 목요일부터 일요일까지 가능합니다!")
		RETURN FALSE
	end if
elseif gs_brand = "J" then
	//if  li_day_no = 1 or li_day_no = 7 or li_day_no = 6  then	
	//	st_2.text = "※요청등록 및 수정은 금(2개),토(3개),일요일(5개)에만 가능합니다!"
	
	//else
		st_2.text = "※요청은 월요일부터 일요일까지 총10개 품번만 가능합니다!"
		messagebox("알림!" , "요청은 월요일부터 일요일까지 총10개 품번만 가능합니다!")
//		RETURN FALSE
	//end if
	//st_2.text = ""
end if
...
string   ls_yymmdd, ls_style,ls_style_no,ls_color,ls_size, ls_fr_ymd
long ii
string ls_find
long ll_found, ll_row, ll_row_dw1,ll_rows
integer li_day_no
...
if gs_brand = 'J' or MidA(gs_shop_cd,1,2) = 'NG' or  MidA(gs_shop_cd,1,2) = 'NB' then 
	
		//if li_day_no = 1 then
			
			select  convert(char(08), dateadd(day, case when (SELECT DATEPART(WEEKDAY, :is_yymmdd)) = 1 then  (SELECT DATEPART(WEEKDAY, :is_yymmdd))  - 7 else  (SELECT DATEPART(WEEKDAY, :is_yymmdd)) * -1 + 2 end, :is_yymmdd ),112)
			into :ls_fr_ymd
			from dual;
			
	
		ll_rows = dw_2.retrieve(ls_fr_ymd, gs_shop_cd)			
		dw_2.VISIBLE = TRUE
		dw_1.VISIBLE = FALSE
...
//			END IF												 
			...
			IF Isvalid(Message.PowerObjectParm) THEN
				...
				if ls_shop_type > "3" and (gs_brand = "J" OR gs_brand = "N") then 
					messagebox("경고!", "행사 품번은 의뢰할수 없습니다!")
...
//				if ls_bujin_chk =  "Y" and mid(gs_shop_cd,1,1) <> "J" then 
				if ls_bujin_chk =  "Y" and MidA(gs_shop_cd,1,1) = "N" then 				
...
/* 수정일      : 2002.04.23                                                  */
/*===========================================================================*/
long i, ll_row_count, ii, ll_found, ll_row, ll_no, ll_rqst_qty, ll_row_dw1, ll_row_dw2
datetime ld_datetime
string ls_yymmdd, ls_style, ls_no, ls_color, ls_size, ls_style_no
integer li_day_no, li_day_limit, li_cnt
...
if gs_brand = 'J' then
	...
	for ii = 1 to ll_row_count //ll_row_dw2
// 	   ls_style = dw_2.getitemstring(ii, "style")
	   ...
	   ls_style = dw_body.getitemstring(ii, "style")
		...
		ll_found = dw_2.Find("style = '" + ls_style_no + "'", 1, dw_2.RowCount())
	
		if ll_found <= 0 then
			ll_row = dw_2.insertrow(0)
			dw_2.setitem(ll_row,"style",  ls_style_no)
		end if			
...
elseif MidA(gs_shop_cd,1,2) = 'NG' or MidA(gs_shop_cd,1,2) = 'NB' then
...
if MidA(gs_shop_cd,2,5) <> "D1900" then 
	...
	if gs_brand = 'J' then 
			...
			if ll_row_dw1 > li_day_limit then
				messagebox("경고!", "이번 주 요청할수 있는 품번은 " + string(li_day_limit, "0000") + "개로 제한되어 있습니다!")
				 RETURN -1
			end if	
...
elseif gs_brand = "J" then
	...
	st_2.text = "※ 요청등록 및 수정은 이번주 수요일부터 다음주 화요일까지 가능합니다!"
end if	
end event

type cb_close from w_com010_e`cb_close within w_sh128_e
end type

type cb_delete from w_com010_e`cb_delete within w_sh128_e
end type

type cb_insert from w_com010_e`cb_insert within w_sh128_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh128_e
end type

type cb_update from w_com010_e`cb_update within w_sh128_e
end type

type cb_print from w_com010_e`cb_print within w_sh128_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_sh128_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_sh128_e
end type

type dw_head from w_com010_e`dw_head within w_sh128_e
integer width = 2802
integer height = 160
...
type ln_1 from w_com010_e`ln_1 within w_sh128_e
integer beginy = 344
integer endy = 344
end type

type ln_2 from w_com010_e`ln_2 within w_sh128_e
integer beginy = 348
integer endy = 348
end type

type dw_body from w_com010_e`dw_body within w_sh128_e
...
type dw_print from w_com010_e`dw_print within w_sh128_e
end type
...
boolean bringtotop = true
boolean titlebar = true
string title = "이번주 요청한 스타일 수"
string dataobject = "d_sh128_d03"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
