$PBExportHeader$w_sh141_e.srw
$PBExportComments$rounge_b_판매일보등록
forward
global type w_sh141_e from w_com010_e
end type
type dw_1 from datawindow within w_sh141_e
end type
type cb_1 from commandbutton within w_sh141_e
end type
type st_1 from statictext within w_sh141_e
end type
type dw_2 from datawindow within w_sh141_e
end type
type st_2 from statictext within w_sh141_e
end type
type st_3 from statictext within w_sh141_e
end type
type dw_3 from datawindow within w_sh141_e
end type
type dw_5 from datawindow within w_sh141_e
end type
type dw_4 from datawindow within w_sh141_e
end type
type st_4 from statictext within w_sh141_e
end type
type st_5 from statictext within w_sh141_e
end type
type dw_8 from datawindow within w_sh141_e
end type
type dw_9 from datawindow within w_sh141_e
end type
type dw_10 from datawindow within w_sh141_e
end type
type dw_11 from datawindow within w_sh141_e
end type
type dw_12 from datawindow within w_sh141_e
end type
type dw_13 from datawindow within w_sh141_e
end type
type cb_2 from commandbutton within w_sh141_e
end type
type dw_back_sale from datawindow within w_sh141_e
end type
type dw_member_sale from datawindow within w_sh141_e
end type
type st_6 from statictext within w_sh141_e
end type
type dw_14 from datawindow within w_sh141_e
end type
type dw_15 from datawindow within w_sh141_e
end type
type st_ok_coupon1 from statictext within w_sh141_e
end type
type st_ok_coupon2 from statictext within w_sh141_e
end type
type dw_16 from datawindow within w_sh141_e
end type
type shl_member from statichyperlink within w_sh141_e
end type
type dw_17 from datawindow within w_sh141_e
end type
type dw_18 from datawindow within w_sh141_e
end type
type dw_19 from datawindow within w_sh141_e
end type
type dw_cosmetic from datawindow within w_sh141_e
end type
type em_1 from editmask within w_sh141_e
end type
type st_7 from statictext within w_sh141_e
end type
type dw_20 from datawindow within w_sh141_e
end type
type dw_list from datawindow within w_sh141_e
end type
type tab_1 from tab within w_sh141_e
end type
...
type dw_6 from datawindow within w_sh141_e
end type
type dw_7 from datawindow within w_sh141_e
end type
type sle_id from singlelineedit within w_sh141_e
end type
type st_online_id from statictext within w_sh141_e
end type
type st_online_gubn from statictext within w_sh141_e
end type
type rb_1 from radiobutton within w_sh141_e
end type
type rb_2 from radiobutton within w_sh141_e
end type
type cb_online_coupon from commandbutton within w_sh141_e
end type
type cb_back_sale from commandbutton within w_sh141_e
end type
type dw_head_2 from datawindow within w_sh141_e
end type
end forward
...
decimal idc_dc_rate_org, id_dc_rate_O, id_sale_price_O
...
string is_set_style_chk, is_coupon='N'
end variables
...
FOR i=1 TO ll_row_count	
	...
	if (MidA(ls_style_no,1,1) <> 'L') and is_coupon_gubn <> '03'  then
		...
		if (gs_shop_cd<>'ND1900' and gs_shop_cd<>'OD1900' and gs_shop_cd<>'ID1900') then
			if (ld_tag_price = ld_sale_price and MidA(ls_sale_type,1,1) = '1') or (MidA(ls_sale_type,1,1) = '1' and  ld_dc_rate <= 10 )  and MidA(ls_style_no,1,1) <> 'N' then
			...
			elseif (ld_tag_price = ld_sale_price and MidA(ls_sale_type,1,1) = '1') or (MidA(ls_sale_type,1,1) = '1' and  ld_dc_rate < 10 ) and MidA(ls_style_no,1,1) = 'N' then
...
if MidA(gs_shop_cd_1,1,2) = 'XX' then
	...
	//	gs_brand = mid(as_style,1,1)
		
			
		if MidA(as_style,1,1)  = "N" or MidA(as_style,1,1)  = "M" or MidA(as_style,1,1)  = "E"   then
			gs_brand = "N"
		else 
...
if is_yymmdd >= '20190408' and (gs_brand = 'B' or gs_brand = 'L' or gs_brand = 'V' or gs_brand = 'G' OR gs_brand = 'F' ) then 
	messagebox('브랜드확인!','이터널그룹/뷰티 브랜드는 이터널전용 매장관리에서 등록 하세요!')
	return false
end if
...
//		gs_brand = mid(as_style_no,1,1)
		...
		if MidA(as_style_no,1,1)  = "N" or MidA(as_style_no,1,1)  = "M" or MidA(as_style_no,1,1)  = "E"   then
			gs_brand = "N"
		else 
...
if is_yymmdd >= '20190408' and (MidA(as_style_no, 1, 1) = 'B' or MidA(as_style_no, 1, 1) = 'L' or MidA(as_style_no, 1, 1) = 'V' or MidA(as_style_no, 1, 1) = 'G' OR MidA(as_style_no, 1, 1) = 'F' ) then 
	messagebox('브랜드확인!','이터널그룹/뷰티 브랜드는 이터널전용 매장관리에서 등록 하세요!')
	return false
end if
...
this.Control[iCurrent+19]=this.dw_back_sale
this.Control[iCurrent+20]=this.dw_member_sale
this.Control[iCurrent+21]=this.st_6
this.Control[iCurrent+22]=this.dw_14
this.Control[iCurrent+23]=this.dw_15
this.Control[iCurrent+24]=this.st_ok_coupon1
this.Control[iCurrent+25]=this.st_ok_coupon2
this.Control[iCurrent+26]=this.dw_16
this.Control[iCurrent+27]=this.shl_member
...
this.Control[iCurrent+35]=this.dw_list
this.Control[iCurrent+36]=this.tab_1
this.Control[iCurrent+37]=this.dw_6
this.Control[iCurrent+38]=this.dw_7
this.Control[iCurrent+39]=this.sle_id
this.Control[iCurrent+40]=this.st_online_id
this.Control[iCurrent+41]=this.st_online_gubn
this.Control[iCurrent+42]=this.rb_1
this.Control[iCurrent+43]=this.rb_2
this.Control[iCurrent+44]=this.cb_online_coupon
this.Control[iCurrent+45]=this.cb_back_sale
this.Control[iCurrent+46]=this.dw_head_2
end on

on w_sh141_e.destroy
call super::destroy
...
Boolean    lb_check 
string ls_shop_snm, ls_ok, ls_date, gs_brand2
DataStore  lds_Source 
...
if MidA(gs_shop_cd_1,1,2) = 'XX' then
	if MidA(as_data,1,1) = '8' then
		...
		//	gs_brand = mid(as_data,1,1)
			
			if MidA(as_data,1,1)  = "N" or MidA(as_data,1,1)  = "M" or MidA(as_data,1,1)  = "E"   then
			gs_brand = "N"
			else 
...
if MidA(gs_shop_cd,1,2) = 'NI'  then
	messagebox("주의!", '행사 매장에서는 사용할 수 없습니다!')
	return false
end if	
...
if gs_shop_cd = "XX4251" then
	gs_BRAND_GRP = "[NEM]"
else 
	gs_BRAND_GRP = "%"
end if	
	
if gs_user_id = "JO0002" or gs_shop_div = 'O' then 
...
string	ls_sale_type_tmp, ls_style, ls_season, ls_online_id, ls_online_gubn, ls_sale_id, ls_shop_cd2
...
FOR i=1 TO ll_row_count
		...
		if LeftA(dw_body.GetItemString(i, "style_no"),1) = '8' then
			...
			//gs_brand = left(dw_body.GetItemString(i, "style_no"),1)
			
					
			if LeftA(dw_body.GetItemString(i, "style_no"),1)  = "N" or LeftA(dw_body.GetItemString(i, "style_no"),1)  = "M" or LeftA(dw_body.GetItemString(i, "style_no"),1)  = "E"   then
				gs_brand = "N"
			else 
...
---온라인------------------------------------------------------- */
		...
		IF idw_status = NewModified! THEN			/* New Record */  
			...
			dw_body.Setitem(i, "empty_3",  ls_sale_id) //판매사원넣기
...
//		ls_card_no_origin = dw_body.getitemnumber(i, "card_no")	
	   ...
	   if ii > 1 then 
		...
		if is_member_return = 'N'  and ls_shop_type <> '3' THEN  // 회원반품일때는 처리하지말고 RETURN    
//		if is_member_return = 'N'  THEN  // 회원반품일때는 처리하지말고 RETURN    
		  wf_line_chk(i)
		end if			
NEXT
...
type cb_close from w_com010_e`cb_close within w_sh141_e
boolean visible = false
integer x = 389
end type

type cb_delete from w_com010_e`cb_delete within w_sh141_e
integer x = 1769
integer width = 315
integer taborder = 70
end type

type cb_insert from w_com010_e`cb_insert within w_sh141_e
boolean visible = false
integer taborder = 60
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh141_e
integer x = 2469
integer width = 384
...
type cb_update from w_com010_e`cb_update within w_sh141_e
integer taborder = 50
end type

type cb_print from w_com010_e`cb_print within w_sh141_e
integer x = 1376
integer width = 393
integer taborder = 80
...
type cb_preview from w_com010_e`cb_preview within w_sh141_e
boolean visible = false
integer x = 1193
integer y = 48
integer taborder = 90
...
type gb_button from w_com010_e`gb_button within w_sh141_e
end type

type dw_head from w_com010_e`dw_head within w_sh141_e
integer y = 152
integer height = 184
...
type ln_1 from w_com010_e`ln_1 within w_sh141_e
integer beginy = 352
integer endy = 352
end type

type ln_2 from w_com010_e`ln_2 within w_sh141_e
integer beginy = 356
integer endy = 356
end type

type dw_body from w_com010_e`dw_body within w_sh141_e
event ue_set_column ( long al_row )
integer x = 5
integer y = 368
integer width = 2880
integer height = 872
string dataobject = "d_sh141_d01"
...
//dw_body.object.b_coupon.visible = false
//dw_body.object.b_coupon.enabled = false
end event
...
Long   ll_curr_price, ll_sale_price, ll_collect_price, ld_goods_amt, ll_dc_rate, ld_sale_price
long   ld_dc_rate
...
long ll_row_count
	CHOOSE CASE dwo.name
		case "event_id"
			dw_body.accepttext()
			ll_row_count = this.getrow()
			
			messagebox('ll_row_count',ll_row_count)
...
type dw_print from w_com010_e`dw_print within w_sh141_e
integer x = 1385
integer y = 224
integer width = 1349
integer height = 948
boolean enabled = false
string dataobject = "d_sh141_p01"
