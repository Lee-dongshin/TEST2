$PBExportHeader$w_sh139_e.srw
$PBExportComments$전용상품판매
forward
global type w_sh139_e from w_com010_e
end type
type cb_1 from commandbutton within w_sh139_e
end type
type dw_list from datawindow within w_sh139_e
end type
type st_1 from statictext within w_sh139_e
end type
type dw_1 from datawindow within w_sh139_e
end type
type gb_1 from groupbox within w_sh139_e
end type
type dw_3 from datawindow within w_sh139_e
end type
type dw_2 from datawindow within w_sh139_e
end type
type st_2 from statictext within w_sh139_e
end type
type dw_4 from datawindow within w_sh139_e
end type
end forward
...
global w_sh139_e w_sh139_e

type variables
String is_yymmdd, is_sale_no, is_fr_year, is_fr_season
...
Long   ll_tag_price , ll_ord_qty, ll_ord_qty_chn, ll_stock_qty
...
//			end if	 
	...
	if MidA(gs_shop_cd,3,4) >= "1900" and MidA(gs_shop_cd,3,4) <= "1913" then 				
		...
		gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and for_lotte = 'Y'  and sojae <> 'C' "  
...
String ls_chno, ls_color, ls_size
...
FOR i=1 TO ll_row_count
   idw_status = dw_body.GetItemStatus(i, 0, Primary!)
	...
	if ll_sale_qty > 0  and ( MidA(gs_shop_cd,3,6) <> "2000"  and  MidA(gs_shop_cd,2,1) <> "E") then
...
//			and size  = :ls_size;
		...
		if ll_stock_qty <= 0 and ll_sale_qty >0 then 
...
type cb_close from w_com010_e`cb_close within w_sh139_e
boolean visible = false
integer x = 389
end type

type cb_delete from w_com010_e`cb_delete within w_sh139_e
integer taborder = 70
end type

type cb_insert from w_com010_e`cb_insert within w_sh139_e
integer taborder = 60
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh139_e
integer x = 2469
integer width = 384
...
type cb_update from w_com010_e`cb_update within w_sh139_e
integer taborder = 50
end type

type cb_print from w_com010_e`cb_print within w_sh139_e
boolean visible = false
integer taborder = 80
end type

type cb_preview from w_com010_e`cb_preview within w_sh139_e
boolean visible = false
integer x = 1193
integer y = 48
integer taborder = 90
...
type gb_button from w_com010_e`gb_button within w_sh139_e
end type

type dw_head from w_com010_e`dw_head within w_sh139_e
integer y = 152
integer height = 96
string dataobject = "d_sh139_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_sh139_e
integer beginy = 256
integer endy = 256
end type

type ln_2 from w_com010_e`ln_2 within w_sh139_e
integer beginy = 260
integer endy = 260
end type

type dw_body from w_com010_e`dw_body within w_sh139_e
event ue_set_column ( long al_row )
integer x = 9
integer y = 256
integer width = 2885
integer height = 908
string dataobject = "d_sh139_d01"
...
type dw_print from w_com010_e`dw_print within w_sh139_e
end type
