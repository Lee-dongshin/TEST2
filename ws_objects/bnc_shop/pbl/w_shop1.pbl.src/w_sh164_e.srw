$PBExportHeader$w_sh164_e.srw
$PBExportComments$직택당일미처리분예약
forward
global type w_sh164_e from w_com010_e
end type
type st_1 from statictext within w_sh164_e
end type
type st_2 from statictext within w_sh164_e
end type
type st_3 from statictext within w_sh164_e
end type
type st_4 from statictext within w_sh164_e
end type
type st_5 from statictext within w_sh164_e
end type
end forward
...
IF LeftA(upper(GS_SHOP_CD),2) = 'NG' or  LeftA(upper(GS_SHOP_CD),2) = 'NB' or LeftA(upper(GS_SHOP_CD),2) = 'JO' then // like 'NG0008' or upper(GS_SHOP_CD) =  'NG1141' or upper(GS_SHOP_CD) =  'NG1092' or upper(GS_SHOP_CD) =  'NG1080' or upper(GS_SHOP_CD) =  'NG1100' THEN 
//IF left(upper(GS_SHOP_CD),2) = 'NG' then // like 'NG0008' or upper(GS_SHOP_CD) =  'NG1141' or upper(GS_SHOP_CD) =  'NG1092' or upper(GS_SHOP_CD) =  'NG1080' or upper(GS_SHOP_CD) =  'NG1100' THEN 	
elseIF LeftA(upper(GS_SHOP_CD),2) = 'OG' then // like 'NG0008' or upper(GS_SHOP_CD) =  'NG1141' or upper(GS_SHOP_CD) =  'NG1092' or upper(GS_SHOP_CD) =  'NG1080' or upper(GS_SHOP_CD) =  'NG1100' THEN 
else	
	messagebox("참고!", "온앤온,  상설 매장에서만 사용이 가능합니다!")
   RETURN FALSE
end if	
...
IF DaysAfter(dw_head.GetItemDate(1, "fr_ymd"), dw_head.GetItemDate(1, "to_ymd")) > 14 THEN
   MessageBox(ls_title,"14일 이상은 조회할수 없습니다!")
   dw_head.SetFocus()
...
//			dw_body.DataObject = "d_sh163_d01"
//		else
//			dw_body.DataObject = "d_sh163_d02"
//		end if
//		
...
//string   ls_invoice_com_shop, ls_invoice_no_shop, ls_c_addr_chk
...
//	ls_c_addr_chk		 = dw_body.GetitemString(i, "c_addr_chk")		
//
...
//
//
//				if ls_c_addr_chk = 'J' and ( isnull(ld_invoice_shop_amt) or  ld_invoice_shop_amt <= 3000 )  then
//				   if ls_invoice_com_shop = "한진신용"  then
//					   dw_body.Setitem(i, "invoice_shop_amt",  0)
//					else	
//						messagebox("경고!" , "제주도 운송비를 확인등록 해주세요!")
//						return -1				
//					end if	
//				end if		
//
...
//   ls_to_shop_cd = dw_body.GetitemString(i, "to_shop_cd")
// //  ls_to_shop_cd = left(gs_shop_cd,1) + "D1970" // dw_body.GetitemString(i, "to_shop_cd")
//	ls_rt_yn      = "N"
//	ls_tran_cust  = "M99"
//	ls_tran_type  = "000"
//	li_box_no     = 0	
...
dw_head.Setitem(1, "fr_ymd", ld_to_ymd)
dw_head.Setitem(1, "to_ymd", ld_fr_ymd)
...
if ls_chk_time >= "16:45:00" OR ls_chk_time <= "11:00:05" then
	this.Trigger Event ue_retrieve()
end if	
end event

type cb_close from w_com010_e`cb_close within w_sh164_e
end type

type cb_delete from w_com010_e`cb_delete within w_sh164_e
boolean visible = false
end type

type cb_insert from w_com010_e`cb_insert within w_sh164_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh164_e
end type

type cb_update from w_com010_e`cb_update within w_sh164_e
boolean visible = false
end type

type cb_print from w_com010_e`cb_print within w_sh164_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_sh164_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_sh164_e
end type

type dw_head from w_com010_e`dw_head within w_sh164_e
integer y = 156
integer height = 176
string dataobject = "d_sh164_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_sh164_e
integer beginy = 344
integer endy = 344
end type

type ln_2 from w_com010_e`ln_2 within w_sh164_e
integer beginy = 348
integer endy = 348
end type

type dw_body from w_com010_e`dw_body within w_sh164_e
integer y = 360
integer width = 2853
integer height = 1424
string dataobject = "d_sh164_d02"
...
CHOOSE CASE dwo.name
	...
	CASE "rt_accept"
		 ...
		 if ls_rt_proc = "N" and ls_yymmdd >= ls_diff_ymd then
						...
						select isnull(rt_accept,"N"), isnull(fr_shop_cd,'XXXXXX')
							...
							if ls_accept_chk = 'Y'  and  ls_fr_shop_cd <> gs_shop_cd  then
								...
								parent.Trigger Event ue_retrieve()
								 ...
								 DECLARE SP_SH164_UPDATE PROCEDURE FOR SP_SH164_UPDATE  
										@yymmdd   = :ls_yymmdd,   
										@ord_ymd  = :ls_ord_ymd,   
										@shop_cd  = :ls_shop_cd,
		...
		messagebox("알림!", "예약은 전일 요청내역까지만 가능합니다!")
		parent.Trigger Event ue_retrieve()
	end if	
...
type dw_print from w_com010_e`dw_print within w_sh164_e
end type
...
string text = "※ 예약희망시 예약확인을 먼저 클릭하시면 "
...
string text = "오후 4시 45분 이후부터 예약가능합니다."
boolean focusrectangle = false
end type
...
long textcolor = 255
long backcolor = 67108864
string text = "※ 익일 예약매장 지정을 위한 작업입니다."
boolean focusrectangle = false
end type
...
long textcolor = 255
long backcolor = 67108864
string text = "※ 예약하신 품번은 익일 직택에 우선 포함되어 지정됩니다."
boolean focusrectangle = false
end type
