...
type cb_proc from commandbutton within w_sh335_e
end type
end forward
...
public function boolean wf_stock_chk2 (long al_row, string as_style_no)
...
public function boolean wf_style_chk2 (long al_row, string as_style_no)
end prototypes
...
public function boolean wf_stock_chk2 (long al_row, string as_style_no);String ls_style, ls_chno, ls_color, ls_size , ls_find, ls_shop_type
Long   ll_stock_qty, ll_cnt_stop, ll_row_count, i, k, ll, ll_chk_qty
...
//else	
	...
	for i = 1 to ll_row_count - 1
		ls_find = "style_no_to = '" + ls_style + ls_chno +  ls_color + ls_size + "'"
...
/*===========================================================================*/
String ls_style, ls_chno, ls_color,  ls_size, ls_shop_type, ls_bujin_chk
String ls_brand, ls_year, ls_season, ls_sojae, ls_item, ls_plan_yn  , ls_style_fr
Long   ll_tag_price 


ls_style_fr = dw_body.getitemstring(al_row, "style")
...
if ls_style <> ls_style_fr then
	MessageBox("확인", "전환 대상과 품번이 다릅니다!")
	RETURN FALSE 
END IF
...
//end if	
//
IF wf_stock_chk2(al_row, as_style_no) THEN 
   dw_body.SetItem(al_row, "style_no_to", as_style_no)
	dw_body.SetItem(al_row, "to_chno",     ls_chno)
	dw_body.SetItem(al_row, "to_color",    ls_color)
	dw_body.SetItem(al_row, "to_size",     ls_size)
ELSE
	Return False
END IF
...
String     ls_style, ls_chno, ls_color, ls_size, ls_shop_type, ls_bujin_chk, ls_style_o
Long       ll_row_cnt 
Boolean    lb_check 
...
//				   dw_body.SetRow(ll_row_cnt)  
	...
	CASE "style_no_to"		
		...
		ls_style_o = dw_body.GetItemString(al_row,"style")
		
			IF ai_div = 1 THEN 	
				IF wf_style_chk2(al_row, as_data)  THEN
				   ll_row_cnt = dw_body.RowCount()
				   IF al_row = ll_row_cnt THEN 
					   ll_row_cnt = dw_body.insertRow(0)
				   END IF
...
//			end if	
			...
			IF Trim(as_data) <> "" THEN
				gst_cd.Item_where = "style  LIKE '" + ls_style_o + "%'" + &
				                " and chno  LIKE '" + ls_chno + "%'" + &
				                " and color LIKE '" + ls_color + "%'" + &
				                " and size  LIKE '" + ls_size + "%'"
			ELSE
...
//				end if	
 				...
 				IF wf_stock_chk2(al_row, lds_Source.GetItemString(1,"style_no_to")) THEN 
				   dw_body.SetItem(al_row, "style_no_to", lds_Source.GetItemString(1,"style_no"))
//				   dw_body.SetItem(al_row, "style", lds_Source.GetItemString(1,"style"))
				   dw_body.SetItem(al_row, "to_chno", lds_Source.GetItemString(1,"chno"))
				   dw_body.SetItem(al_row, "to_color", lds_Source.GetItemString(1,"color"))
				   dw_body.SetItem(al_row, "to_size", lds_Source.GetItemString(1,"size"))
				   ib_changed = true
               cb_update.enabled = true
				   /* 다음컬럼으로 이동 */
//				   ll_row_cnt = dw_body.RowCount()
...
IF idw_status <> NewModified! and ls_proc_yn = "Y" THEN 
   dw_body.SetFocus()
	RETURN 
END IF 
...
event dw_body::constructor;call super::constructor;DataWindowChild ldw_color,ldw_to_color
...
This.GetChild("to_color", ldw_to_color)
ldw_to_color.SetTransObject(SQLCA)
...
CHOOSE CASE dwo.name
	CASE "style_no", 	   "style_no_to"   //  Popup 검색창이 존재하는 항목 
		IF ib_itemchanged THEN RETURN 1
...
string text = "<-- 대상리스트를 저장 완료 후 작업하세요!"
...
select count(*)
into :li_cnt
from tb_42029_h (nolock)
where yymmdd = :is_yymmdd
and  shop_cd = :gs_shop_cd
and   ISNULL(proc_yn,'n') = 'N';

if li_cnt <= 0 or isnull(li_cnt) then
	 MessageBox("알림", is_yymmdd + "일자에 미처리된 대상이 없습니다. 요청일자를 확인하세요!")
	Return -1 		
end if	
	 

Net = MessageBox("경고", "대상 품번의 재고가 전환품번으로 변경됩니다. 계속하시겠습니까?", Exclamation!, OKCancel!, 2)
...
IF Net = 1 THEN 
	...
	st_1.text = "<-- 처리 중입니다. 잠시 기다려 주세요!  "
		...
		DECLARE SP_sh335_P01 PROCEDURE FOR SP_sh335_P01  
			@yymmdd		= :is_yymmdd,
			@shop_cd		= :gs_shop_cd;

	
		 EXECUTE SP_sh335_P01 ;
		 
		IF SQLCA.SQLCODE = -1 THEN 
			rollback  USING SQLCA;				
			MessageBox("SQL오류", SQLCA.SqlErrText) 
			st_1.text = "<-- 오류가 발생했습니다. 확인 후 다시 작업하세요!"
//			st_1.visible = false			
			Return -1 			
		ELSE
 		 	commit  USING SQLCA;
...
//			st_1.visible = false
			st_1.text = "<-- 대상리스트를 저장 완료 후 작업하세요!"
