$PBExportHeader$w_sh365_e.srw
$PBExportComments$사내판매등록
forward
global type w_sh365_e from w_com010_e
end type
type dw_view from datawindow within w_sh365_e
end type
type cb_input from commandbutton within w_sh365_e
end type
end forward
...
integer width = 2976
integer height = 2044
string title = "d_sh365_h01"
event ue_input ( )
dw_view dw_view
cb_input cb_input
end type
global w_sh365_e w_sh365_e

type variables
DataWindowChild	idw_brand, idw_color, idw_size
String is_brand, is_yymmdd, is_rqst_no, is_pay_way, is_empno, is_empnm, is_bill_ymd
end variables
...
event ue_input();dw_view.Visible   = False
...
Long   ll_tag_price,  ll_cnt , ll_dc_cnt
Decimal ldc_dc_rate, ldc_dc_rate1, ldc_dc_rate2
SetNull(ls_null)
...
if ldc_dc_rate1 > ldc_dc_rate2 then
 ldc_dc_rate = ldc_dc_rate1
else 
 ldc_dc_rate = ldc_dc_rate2	
end if
...
if ls_bujin_chk = "Y"  and is_brand <> "B" then
		dw_body.SetItem(al_row, "dc_rate",    70)	
		Return True
elseif ll_dc_cnt <> 0  and is_brand <> "B" then	
...
//if gs_shop_cd <> "NG1077" then 
//	MessageBox(ls_title,"현재 테스트매장만 사용가능합니다!")
//	return false
//end if
...
is_empnm = dw_head.GetItemString(1, "kname")
...
from dual;
	...
	if li_date_part = 3 then // or li_date_part = 5
		if ls_time > '14:01' then
		  MessageBox(ls_title,"오후 2시이전까지 요청 가능합니다!")
		  return false		
		end if		
...
String 	  ls_plan_yn, ls_SHOP_TYPE, ls_dept_nm
...
//				   dw_head.SetItem(al_row, "dept_name", ls_shop_nm)
//					RETURN 0
//				END IF 
//			END IF
//		   gst_cd.ai_div          = ai_div
...
//------------------------------------------------		
			IF ai_div = 1 THEN 
				if MidA(as_data,2,1) <> 'G' and MidA(as_data,2,1) <> 'K' and  gf_emp_nm(as_data, ls_empnm) = 0 THEN
					dw_head.Setitem(al_row, "kname", ls_empnm)
					...
					dw_head.Setitem(al_row, "dept_name", ls_dept_nm)
					RETURN 0
				END IF 
			END IF
		   gst_cd.ai_div          = ai_div
			...
			/* 관련부서 산출 */ 
			gst_cd.default_where   = "WHERE dept_code =  '" + gs_shop_cd + "'  and status_yn = 'Y' " 
			...
			IF Isvalid(Message.PowerObjectParm) THEN
				...
				dw_head.SetItem(al_row, "dept_name", lds_Source.GetItemString(1,"dept_nm"))				
				/* 다음컬럼으로 이동 */
...
il_rows = dw_view.retrieve(is_yymmdd, is_rqst_no, is_empno, is_empnm, is_brand, is_pay_way, gs_shop_cd)
IF il_rows > 0 THEN
...
type cb_close from w_com010_e`cb_close within w_sh365_e
end type

type cb_delete from w_com010_e`cb_delete within w_sh365_e
end type

type cb_insert from w_com010_e`cb_insert within w_sh365_e
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh365_e
end type

type cb_update from w_com010_e`cb_update within w_sh365_e
end type

type cb_print from w_com010_e`cb_print within w_sh365_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_sh365_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_sh365_e
end type

type dw_head from w_com010_e`dw_head within w_sh365_e
integer x = 5
integer height = 196
string dataobject = "d_sh365_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_sh365_e
integer beginy = 380
integer endy = 380
end type

type ln_2 from w_com010_e`ln_2 within w_sh365_e
integer beginy = 384
integer endy = 384
end type

type dw_body from w_com010_e`dw_body within w_sh365_e
event ue_set_col ( string as_column )
integer y = 396
integer height = 1392
string dataobject = "d_sh365_d02"
...
type dw_print from w_com010_e`dw_print within w_sh365_e
end type

event dw_print::doubleclicked;call super::doubleclicked;String ls_rqst_no, ls_empno, ls_empnm, ls_pay_way , ls_dept_nm
...
IF dw_body.Retrieve(is_yymmdd, ls_rqst_no,  ls_empno, gs_shop_cd) > 0 THEN 
   dw_body.visible = True						  
   dw_view.visible = False 
...
IF dw_body.Retrieve(is_yymmdd, ls_rqst_no,  ls_empno, gs_shop_cd) > 0 THEN 
   ...
   dw_view.visible = False 
	...
	dw_head.setitem(1, "rqst_no", ls_rqst_no)
	dw_head.setitem(1, "empno", ls_empno)	
	dw_head.setitem(1, "kname", ls_empnm)		
	dw_head.setitem(1, "dept_name", ls_dept_nm)			
	dw_head.setitem(1, "pay_way", ls_pay_way)	
