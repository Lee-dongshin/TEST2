...
//당월목표 / (당월판매금액 / 당월판매수량) / 1.7 이  50% 이상이면 100장 
//
//+
//
//전월행사제외판매금액 / 전월목표 가 100% 이상이면  100장
...
if MidA(gs_shop_cd,1,1) <> "Y" or gs_shop_cd <> 'OG0294' then
	if MidA(gs_shop_cd,1,1) <> "D" then
		if ll_rqst_qty > 0 and idc_month_limit < idc_month_out + ll_rqst_qty and gs_shop_div <> "K" and gs_brand <> "B" then 
			messagebox("주의!", '당월 쇼핑백 출고제한 수량을 초과 요청 할 수 없습니다!')
			return -1
		end if	
