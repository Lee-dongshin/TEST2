$PBExportHeader$w_sh210_e.srw
$PBExportComments$누락 마일리지 등록
forward
global type w_sh210_e from w_com010_e
end type
end forward
...
string   ls_date,  ls_buy_day, ls_shop_cd, ls_good_no, ls_color, ls_size, l_c_yn, ls_title
...
ls_buy_day = dw_body.GetItemString(1, "buy_day")
if IsNull(ls_buy_day) or Trim(ls_buy_day) = "" then
   MessageBox(ls_title,"구입일자를 입력하십시요!")
   dw_body.SetFocus()
   dw_body.SetColumn("buy_day")
   return 0
end if

ls_shop_cd = dw_body.GetItemString(1, "shop_cd")
if IsNull(ls_shop_cd) or Trim(ls_shop_cd) = "" then
   MessageBox(ls_title,"구입매장 입력하십시요!")
   dw_body.SetFocus()
   dw_body.SetColumn("shop_cd")
   return 0
end if

ls_good_no = dw_body.GetItemString(1, "good_no")
if IsNull(ls_good_no) or Trim(ls_good_no) = "" then
   MessageBox(ls_title,"스타일을 입력하십시요!")
   dw_body.SetFocus()
   dw_body.SetColumn("good_no")
   return 0
end if

ls_color = dw_body.GetItemString(1, "color")
if IsNull(ls_color) or Trim(ls_color) = "" then
   MessageBox(ls_title,"칼라를 입력하십시요!")
   dw_body.SetFocus()
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified! THEN				/* New Record */
      dw_body.Setitem(i, "reg_id", gs_shop_cd)
		dw_body.Setitem(i, "reg_dt", ls_date)
...
event ue_retrieve();call super::ue_retrieve;string  ls_jumin,   ls_tel_no3
long    i, ll_row_count

 dw_head.AcceptText() 

 ls_jumin = dw_head.Getitemstring(1, "jumin")
 ls_tel_no3 = dw_head.Getitemstring(1, "tel_no3")

if  LenA(ls_jumin) = 13 then		
...
type cb_close from w_com010_e`cb_close within w_sh210_e
end type

type cb_delete from w_com010_e`cb_delete within w_sh210_e
boolean visible = false
end type

type cb_insert from w_com010_e`cb_insert within w_sh210_e
boolean visible = false
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh210_e
end type

type cb_update from w_com010_e`cb_update within w_sh210_e
end type

type cb_print from w_com010_e`cb_print within w_sh210_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_sh210_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_sh210_e
end type

type dw_head from w_com010_e`dw_head within w_sh210_e
event type long ue_item_changed ( long row,  dwobject dwo,  string data )
string dataobject = "d_sh210_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_sh210_e
end type

type ln_2 from w_com010_e`ln_2 within w_sh210_e
end type

type dw_body from w_com010_e`dw_body within w_sh210_e
string dataobject = "d_sh210_d01"
boolean vscrollbar = false
boolean livescroll = false
end type

type dw_print from w_com010_e`dw_print within w_sh210_e
end type
