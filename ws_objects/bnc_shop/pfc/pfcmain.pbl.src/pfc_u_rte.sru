...
if GetFileOpenName ("Open", as_filename, ls_filename, "", &
	...
	if modified then
		...
		if IsNull (ls_title) or LenA (ls_title) = 0 then
			ls_title = "Save"
		end if
		ls_docname = is_filename
		if LenA (ls_docname) = 0 then
			ls_docname = "Document"
		end if
...
if LenA (is_pageinputfield) <= 0 then
	return
end if
...
if LenA (ls_selectedtext) > 0 then
