$PBExportHeader$w_21005_e.srw
$PBExportComments$원자재 원가견적서
forward
global type w_21005_e from w_com020_e
end type
end forward

global type w_21005_e from w_com020_e
end type
global w_21005_e w_21005_e
...
il_rows = dw_list.retrieve(is_brand, is_year, is_season, is_mat_cd, MidA(is_cust_cd,3,4))
dw_body.Reset()
IF il_rows > 0 THEN
   dw_list.SetFocus()
END IF
	...
	CASE "mat_cd"				
			IF ai_div = 1 THEN 	
				...
				ELSEIF gf_mat_nm(as_data, ls_mat_nm) = 0 THEN
						...
						select top 1 brand , mat_year, mat_season 
								into :is_brand, :is_year, :is_season
								from tb_21010_m (nolock)
								where mat_cd like :as_data + '%';
		   ...
		   gst_cd.ai_div          = ai_div
			...
			gst_cd.default_where   = " where brand = '" + is_brand + "' " + &
											 " and   cust_cd = '" + gs_user_id + "' " + &
											 " and   mat_year like '" + is_year + "%' " + &
											 " and   mat_season like '" + is_season + "%' " 
...
event open;call super::open;dw_head.setitem(1,"cust_cd",gs_user_id)
dw_head.setitem(1,"cust_nm",gs_user_nm)

end event

type cb_close from w_com020_e`cb_close within w_21005_e
end type

type cb_delete from w_com020_e`cb_delete within w_21005_e
end type

type cb_insert from w_com020_e`cb_insert within w_21005_e
boolean visible = false
end type

type cb_retrieve from w_com020_e`cb_retrieve within w_21005_e
end type

type cb_update from w_com020_e`cb_update within w_21005_e
boolean visible = false
end type

type cb_print from w_com020_e`cb_print within w_21005_e
end type

type cb_preview from w_com020_e`cb_preview within w_21005_e
end type

type gb_button from w_com020_e`gb_button within w_21005_e
end type

type cb_excel from w_com020_e`cb_excel within w_21005_e
end type

type dw_head from w_com020_e`dw_head within w_21005_e
integer height = 164
string dataobject = "d_21005_h01"
end type
...
type ln_1 from w_com020_e`ln_1 within w_21005_e
integer beginy = 352
integer endy = 352
end type

type ln_2 from w_com020_e`ln_2 within w_21005_e
integer beginy = 356
integer endy = 356
end type

type dw_list from w_com020_e`dw_list within w_21005_e
integer y = 380
integer height = 1660
string dataobject = "d_21005_l01"
end type
...
type dw_body from w_com020_e`dw_body within w_21005_e
integer x = 814
integer y = 380
integer width = 2779
integer height = 1660
string dataobject = "d_21005_d01"
end type
...
type st_1 from w_com020_e`st_1 within w_21005_e
integer y = 380
integer height = 1660
end type

type dw_print from w_com020_e`dw_print within w_21005_e
integer x = 183
integer y = 1224
string dataobject = "d_21005_r01"
end type
