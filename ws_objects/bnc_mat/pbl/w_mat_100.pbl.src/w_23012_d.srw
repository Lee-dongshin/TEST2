...
if is_brand = 'J' or is_brand = 'E' or is_brand = 'M'   then
	ls_shop_cd = 'N' + MidA(GS_USER_ID, 2,5)
elseif is_brand = 'O' or is_brand = 'A'   then
		ls_shop_cd = GS_USER_ID
//	ls_shop_cd = 'O' + MID(GS_USER_ID, 2,5)	
else	
	IF GS_DB = 'E' THEN
		ls_shop_cd = 'N' + MidA(GS_USER_ID, 2,5)	
	ELSE 	
		ls_shop_cd = IS_BRAND + MidA(GS_USER_ID, 2,5)	
	END IF	
end if	



il_rows = dw_body.retrieve(is_brand, is_bill_date, is_bill_type, ls_shop_cd, is_dept_gubn, is_m_type_0, is_m_type_1, is_m_type_2, is_m_type_3, is_m_type_4, is_m_type_5, is_m_type_6, is_m_type_7, is_m_type_8, is_m_type_9)
IF il_rows > 0 THEN
   dw_body.SetFocus()
...
IF gf_cdate(ld_datetime,0)  THEN  
	dw_head.setitem(1,"bill_date",MidA(string(ld_datetime,"yyyymmdd"),1,6))
