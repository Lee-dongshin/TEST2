$PBExportHeader$w_mat_d04.srw
$PBExportComments$입고반품조회
forward
global type w_mat_d04 from w_com010_d
end type
end forward

global type w_mat_d04 from w_com010_d
end type
global w_mat_d04 w_mat_d04

type variables
string is_brand, is_year, is_season, is_item, is_st_ymd, is_ed_ymd, is_cust_cd, is_mat_cd, is_in_gubn
string is_st_ord_ymd, is_ed_ord_ymd
datawindowchild idw_brand, idw_year, idw_season, idw_item
end variables

forward prototypes
public subroutine wf_confirm_mat (string as_mat_cd)
end prototypes

public subroutine wf_confirm_mat (string as_mat_cd);gsv_cd.gs_cd7 = as_mat_cd
open(w_21013_e)
end subroutine
...
is_st_ord_ymd = dw_head.GetItemString(1, "st_ord_ymd")
is_ed_ord_ymd = dw_head.GetItemString(1, "ed_ord_ymd")
...
il_rows = dw_body.retrieve('%', '%','%','%', is_st_ymd, is_ed_ymd,is_st_ord_ymd, is_ed_ord_ymd, MidA(gs_user_id,2), '%','%')
IF il_rows > 0 THEN
   dw_body.SetFocus()
...
IF gf_cdate(ld_datetime,-1)  THEN  
	...
	dw_head.setitem(1,"st_ord_ymd",string(ld_datetime,"yyyymmdd"))	
end if


IF gf_cdate(ld_datetime,0)  THEN  
	dw_head.setitem(1,"ed_ymd",string(ld_datetime,"yyyymmdd"))
	dw_head.setitem(1,"ed_ord_ymd",string(ld_datetime,"yyyymmdd"))		
end IF
end event
...
//
//il_rows = dw_print.retrieve(is_brand, is_year, is_season, is_item, is_st_ymd, is_ed_ymd, is_cust_cd, is_mat_cd, is_in_gubn)
...
type cb_close from w_com010_d`cb_close within w_mat_d04
boolean visible = false
end type

type cb_delete from w_com010_d`cb_delete within w_mat_d04
end type

type cb_insert from w_com010_d`cb_insert within w_mat_d04
end type

type cb_retrieve from w_com010_d`cb_retrieve within w_mat_d04
end type

type cb_update from w_com010_d`cb_update within w_mat_d04
end type

type cb_print from w_com010_d`cb_print within w_mat_d04
end type

type cb_preview from w_com010_d`cb_preview within w_mat_d04
end type

type gb_button from w_com010_d`gb_button within w_mat_d04
end type

type cb_excel from w_com010_d`cb_excel within w_mat_d04
end type

type dw_head from w_com010_d`dw_head within w_mat_d04
integer width = 3959
integer height = 168
string dataobject = "d_mat_h31"
end type
...
type ln_1 from w_com010_d`ln_1 within w_mat_d04
integer beginy = 352
integer endy = 352
end type

type ln_2 from w_com010_d`ln_2 within w_mat_d04
integer beginy = 356
integer endy = 356
end type

type dw_body from w_com010_d`dw_body within w_mat_d04
integer y = 376
integer height = 1644
string dataobject = "d_mat_d31"
...
if row > 0 then 
			ls_search 	= this.GetItemString(row,'mat_cd')
			if LenA(ls_search) = 10 then wf_confirm_mat(ls_search)
//	choose case dwo.name
//		case 'mat_cd','mat_nm'
//			ls_search 	= this.GetItemString(row,'mat_cd')
//			if len(ls_search) = 10 then wf_confirm_mat(ls_search)
//	end choose	
end if
end event

type dw_print from w_com010_d`dw_print within w_mat_d04
string dataobject = "d_mat_r31"
end type
