...
li_currpos = PosA (ls_syntax, "arguments=((", 1)

If li_currpos > 0 Then
	// Loop until all the arguments have been found.
	do while (MidA(ls_syntax, li_currpos, 1) <> ")") And (li_currpos < LenA(ls_syntax))
		// Reset values.
		li_namestartpos = 0
		li_nameendpos = 0
		li_typestartpos = 0
		...
		// Get the column NAME.
		li_namestartpos = PosA (ls_syntax, "~"", li_currpos) + 1
		li_nameendpos = PosA (ls_syntax, "~"", li_namestartpos)
		If li_namestartpos <=0 or li_nameendpos <=0 Then Return -1		
		ls_args[li_numargs] = Trim (MidA (ls_syntax, li_namestartpos, li_nameendpos - li_namestartpos))

		// Get the column TYPE.
		li_typestartpos = li_nameendpos + 3
		li_typeendpos = PosA (ls_syntax, ")", li_typestartpos)
		If li_typestartpos <=0 or li_typeendpos <=0 Then Return -1		
		ls_types[li_numargs] = Trim (MidA (ls_syntax, li_typestartpos, li_typeendpos - li_typestartpos))
...
Else
	...
	li_currpos = PosA (ls_syntax, "arg(", 1)
	...
	do while li_currpos > 0 
		...
		// Get the column NAME.
		li_namestartpos = PosA (ls_syntax, "name", li_currpos)
		li_nameendpos = PosA (ls_syntax, "type", li_namestartpos)
		If li_namestartpos <=0 or li_nameendpos <=0 Then Return -1				
		ls_workstring = MidA (ls_syntax, li_namestartpos, li_nameendpos - li_namestartpos) 
		...
		li_typeendpos = PosA (ls_syntax, ")", li_nameendpos) 
		If li_typestartpos <=0 or li_typeendpos <=0 Then Return -1		
		ls_workstring = MidA (ls_syntax, li_typestartpos, li_typeendpos - li_typestartpos) 
		ls_types[li_numargs] = Trim (lnv_string.of_GetKeyValue (ls_workstring, "type", "=" ))
		
		// Look for the next ARG( keyword.
		li_currpos = PosA (ls_syntax, "arg(", li_typeendpos)		
	loop
End If
