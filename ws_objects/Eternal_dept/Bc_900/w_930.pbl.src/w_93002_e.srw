$PBExportHeader$w_93002_e.srw
$PBExportComments$사용자 프로그램 관리
forward
global type w_93002_e from w_com020_e
end type
type pb_next from picturebutton within w_93002_e
end type
type pb_previous from picturebutton within w_93002_e
end type
end forward
...
global w_93002_e w_93002_e

type variables
String is_person_id, is_check1 = 'N', is_check2 = 'N'
...
string     ls_person_id, ls_person_nm
DataStore  lds_Source

CHOOSE CASE as_column
	CASE "person_id"					// 사용자 번호
			IF ai_div = 1 THEN 	// ItemChanged!  -> Call
				IF wf_user_info(as_data) <> 0 THEN
					MessageBox("입력오류","등록되지 않은 사용자번호 입니다!")
					RETURN 1
				END IF
				
			ELSE								// F1 key Or PopUp Button Click -> Call
				gst_cd.window_title    = "사원번호 검색" 
				...
				IF Isvalid(Message.PowerObjectParm) THEN
					...
					ls_person_id = lds_Source.GetItemString(1,"person_id")
					dw_head.SetItem(al_row, "person_id", ls_person_id)
					IF wf_user_info(ls_person_id) <> 0 THEN
						MessageBox("입력오류","등록되지 않은 사용자번호 입니다!")
						RETURN 1
					END IF				
...
il_rows = dw_list.retrieve(is_person_id)
//dw_body.Reset()
ll_rows = dw_body.retrieve(is_person_id)
IF il_rows > 0 THEN
   dw_list.SetFocus()
...
if IsNull(is_person_id) or Trim(is_person_id) = "" then
   MessageBox(ls_title,"사원 코드를 입력하십시요!")
   dw_head.SetFocus()
...
type cb_close from w_com020_e`cb_close within w_93002_e
end type

type cb_delete from w_com020_e`cb_delete within w_93002_e
boolean visible = false
end type

type cb_insert from w_com020_e`cb_insert within w_93002_e
boolean visible = false
end type

type cb_retrieve from w_com020_e`cb_retrieve within w_93002_e
end type

type cb_update from w_com020_e`cb_update within w_93002_e
end type

type cb_print from w_com020_e`cb_print within w_93002_e
boolean visible = false
end type

type cb_preview from w_com020_e`cb_preview within w_93002_e
boolean visible = false
end type

type gb_button from w_com020_e`gb_button within w_93002_e
end type

type cb_excel from w_com020_e`cb_excel within w_93002_e
boolean visible = false
end type

type dw_head from w_com020_e`dw_head within w_93002_e
integer width = 3557
integer height = 216
string dataobject = "d_93002_h01"
end type
...
This.GetChild("brand_cd", ldw_child)
...
type ln_1 from w_com020_e`ln_1 within w_93002_e
integer beginy = 412
integer endy = 412
end type

type ln_2 from w_com020_e`ln_2 within w_93002_e
integer beginy = 416
integer endy = 416
end type

type dw_list from w_com020_e`dw_list within w_93002_e
integer x = 14
integer y = 428
integer width = 1719
integer height = 1604
string dataobject = "d_93002_d01"
end type
...
type dw_body from w_com020_e`dw_body within w_93002_e
integer x = 1888
integer y = 428
integer width = 1719
integer height = 1604
string dataobject = "d_93002_d02"
end type
...
type st_1 from w_com020_e`st_1 within w_93002_e
boolean visible = false
integer y = 380
integer height = 1652
...
type dw_print from w_com020_e`dw_print within w_93002_e
end type
