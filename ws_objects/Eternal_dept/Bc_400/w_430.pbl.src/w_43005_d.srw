...
type rb_5 from radiobutton within w_43005_d
end type
...
type variables
DataWindowChild idw_brand,  idw_year,idw_season, idw_item, idw_shop_type, idw_shop_div , idw_sojae
string is_brand, is_yymmdd,  is_shop_type, is_shop_div, is_year, is_season,  is_item, is_sojae
...
il_rows = dw_list.retrieve(is_yymmdd, is_brand, is_shop_type, is_year, is_season, is_item, is_shop_div, is_sojae)
IF il_rows > 0 THEN
   dw_list.SetFocus()
...
il_rows = dw_body.retrieve(is_yymmdd, is_brand, ls_shop_cd, is_shop_type, is_year, is_season, is_item, is_shop_div, ls_option, is_sojae)
IF il_rows > 0 THEN
   dw_body.SetFocus()
ELSEIF il_rows = 0 THEN
   MessageBox("조회", "조회할 자료가 없습니다.")
