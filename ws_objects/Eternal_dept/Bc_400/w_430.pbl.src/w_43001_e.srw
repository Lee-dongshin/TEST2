$PBExportHeader$w_43001_e.srw
$PBExportComments$창고간 이동 관리
forward
global type w_43001_e from w_com010_e
end type
type dw_1 from datawindow within w_43001_e
end type
type dw_rtrn from datawindow within w_43001_e
end type
end forward
...
IF isnull(is_out_no) THEN  
	IF gf_style_outno_re(is_yymmdd, is_brand, is_out_no) = FALSE THEN RETURN -1
END IF

//messagebox("", is_out_no)
...
//		select substring(convert(varchar(5), convert(decimal(5), :ls_no) +  10001 )	, 2, 4) 
//		into :ls_no
//		from dual;		 
		
	   dw_body.Setitem(i, "no",  ls_no )//String(i, "0000"))
		
//		messagebox("ls_no", ls_no)
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_43001_e","0")
end event

type cb_close from w_com010_e`cb_close within w_43001_e
end type

type cb_delete from w_com010_e`cb_delete within w_43001_e
end type

type cb_insert from w_com010_e`cb_insert within w_43001_e
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_43001_e
end type

type cb_update from w_com010_e`cb_update within w_43001_e
end type

type cb_print from w_com010_e`cb_print within w_43001_e
end type

type cb_preview from w_com010_e`cb_preview within w_43001_e
end type

type gb_button from w_com010_e`gb_button within w_43001_e
end type

type cb_excel from w_com010_e`cb_excel within w_43001_e
boolean visible = false
end type

type dw_head from w_com010_e`dw_head within w_43001_e
integer width = 3406
...
type ln_1 from w_com010_e`ln_1 within w_43001_e
integer beginy = 416
integer endy = 416
end type

type ln_2 from w_com010_e`ln_2 within w_43001_e
integer beginy = 420
integer endy = 420
end type

type dw_body from w_com010_e`dw_body within w_43001_e
integer x = 1280
integer y = 436
integer width = 2313
integer height = 1608
...
type dw_print from w_com010_e`dw_print within w_43001_e
end type
