...
string is_jup_grp, is_house_cd, is_pda_no, is_proc_type, is_in_gubn
DataWindowChild idw_brand, idw_shop_type, idw_house_cd,idw_jup_gubn
end variables
...
is_in_gubn = dw_head1.GetItemString(1, "in_gubn")
if IsNull(is_in_gubn) or Trim(is_in_gubn) = "" then
   MessageBox(ls_title,"입고전표 구분을 입력하십시요!")
   dw_head1.SetFocus()
   dw_head1.SetColumn("in_gubn")
   return false
end if
...
if is_view_opt = "B" then 
	...
	il_rows = dw_1.retrieve(is_brand, is_fr_ymd, is_to_ymd, is_house_cd, is_proc_gubn, is_pda_no)
	IF il_rows > 0 THEN
		dw_1.SetFocus()
		dw_1.visible = true			
	else	
	...
	dw_1.visible = false		
	il_rows = dw_list.retrieve(is_brand, is_fr_ymd, is_to_ymd, is_house_cd, is_proc_gubn, is_pda_no)
	dw_body.Reset()
	IF il_rows > 0 THEN
		dw_list.SetFocus()
	END IF
...
CHOOSE CASE dwo.name
	...
	CASE "proc_type"	     //  Popup 검색창이 존재하는 항목 
		IF ib_itemchanged THEN RETURN 1
		if data =  "O"  or data =  "R"  or  data =  "S"  then 
			dw_head1.object.in_gubn.visible = false
			dw_head1.object.t_in_gubn.visible = false			
		else 	
			dw_head1.object.in_gubn.visible = true
			dw_head1.object.t_in_gubn.visible = true			
		end if		
...
This.GetChild("in_gubn", idw_jup_gubn)
