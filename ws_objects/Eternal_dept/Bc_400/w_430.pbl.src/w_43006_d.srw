...
For i = 1 to ll_row_cnt 
   ...
   k = dw_assort.find(ls_find, 1, ll_assort_cnt)	
	...
	IF k > 0 THEN 
      ...
      ls_modify = ' t_size'    + String(k) + '.text= "' + dw_result2.object.size_nm[i] + '"' 
		ls_Error = dw_body.Modify(ls_modify)
...
//   	end if
		if i> 0 then 
      ls_modify = ' t_size'    + String(i) + '.text= "' + dw_result.object.size_nm[i] + '"' 
   	end if
		ls_Error = dw_list.Modify(ls_modify)
		ls_Error = dw_body.Modify(ls_modify)
...
this.Control[iCurrent+1]=this.dw_shop_div
this.Control[iCurrent+2]=this.dw_detail
this.Control[iCurrent+3]=this.dw_assort
this.Control[iCurrent+4]=this.dw_color
this.Control[iCurrent+5]=this.dw_result
this.Control[iCurrent+6]=this.dw_1
this.Control[iCurrent+7]=this.dw_2
this.Control[iCurrent+8]=this.dw_result2
end on
