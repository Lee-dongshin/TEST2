$PBExportHeader$w_42025_e.srw
$PBExportComments$오픈예정매장출고처리
forward
global type w_42025_e from w_com010_e
end type
type dw_color from datawindow within w_42025_e
end type
type dw_size from datawindow within w_42025_e
end type
end forward
...
global w_42025_e w_42025_e

type variables
DataWindowChild idw_brand,idw_color,idw_size, idw_shop_type
string is_brand, is_yymmdd, is_shop_cd, is_shop_type, is_out_ymd, is_file_nm, is_open_no
           ...
           if ll_found <= 0 then 		
           	...
           	il_rows ++ 								
				...
				if ls_bujin_chk = "Y" then 
					dw_body.Setitem(il_rows, "bujin", "부진")
            end if 			
				dw_body.Setitem(il_rows, "price", ldc_tag_price)
				dw_body.setitem(il_rows, "out_qty", dec(Trim(MidA(ls_data, 49,1))) )												
//           else				
//							
//				dw_body.setitem(ll_found, "out_qty", dw_body.getitemdecimal(ll_found , "out_qty") + dec(Trim(Mid(ls_data, 49,1))) )												
//
  			 end if			
		...
		MEssagebox("오류확인", "총" + "" + string(il_rows) + "" + "건중 " + "" +  string(li_cnt_err) + "" + "개의 잘못된 데이터가 있습니다!" )
			...
			il_rows = dw_body.retrieve(is_shop_cd, is_shop_type, is_yymmdd, IS_OPEN_NO)
...
IF dw_head.AcceptText() <> 1 THEN RETURN FALSE
	...
	if IsNull(is_file_nm) or Trim(is_file_nm) = "" then
			is_OPEN_NO = dw_head.GetItemString(1, "OPEN_NO") 
			if IsNull(is_OPEN_NO) or Trim(is_OPEN_NO) = "" then 
				MessageBox(ls_title,"전표번호를 입력하십시요!") 
				dw_head.SetFocus() 
				dw_head.SetColumn("OPEN_NO") 
				return false
			end if
	END IF		
...
//	end if
	...
	if IsNull(is_shop_type) or Trim(is_shop_type) = "" then 
		MessageBox(ls_title,"출고매장형태를 입력하십시요!") 
		dw_head.SetFocus() 
...
/* 수정일      : 2001..                                                      */
/*===========================================================================*/
long i, ll_row_count, li_cnt_err, jj
datetime ld_datetime
string	ls_style_chk, ls_style , ls_year, ls_season , ls_item, ls_sojae, LS_OPEN_NO
...
select substring(convert(varchar(5), convert(decimal(5),isnull(max(OPEN_no), '0')) + 10001), 2, 4)
INTO :LS_OPEN_NO
from	tb_42025_work;

dw_head.SETITEM(1, "open_no", ls_open_no)
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified! THEN				/* New Record */
      ...
      ls_style = dw_body.getitemstring(i, "style")
	   ...
	   DW_BODY.SETITEM(I, "OPEN_NO"   , LS_OPEN_NO)						
	   DW_BODY.SETITEM(I, "no" , dw_body.getitemNumber(I, "compute_4"))						
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_42025_e","0")
end event

type cb_close from w_com010_e`cb_close within w_42025_e
end type

type cb_delete from w_com010_e`cb_delete within w_42025_e
end type

type cb_insert from w_com010_e`cb_insert within w_42025_e
boolean visible = false
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_42025_e
end type

type cb_update from w_com010_e`cb_update within w_42025_e
end type

type cb_print from w_com010_e`cb_print within w_42025_e
end type

type cb_preview from w_com010_e`cb_preview within w_42025_e
end type

type gb_button from w_com010_e`gb_button within w_42025_e
end type

type cb_excel from w_com010_e`cb_excel within w_42025_e
end type

type dw_head from w_com010_e`dw_head within w_42025_e
integer y = 160
integer height = 292
string dataobject = "d_42025_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_42025_e
integer beginy = 468
integer endy = 468
end type

type ln_2 from w_com010_e`ln_2 within w_42025_e
integer beginy = 472
integer endy = 472
end type

type dw_body from w_com010_e`dw_body within w_42025_e
integer x = 9
integer y = 484
integer width = 3584
integer height = 1552
string dataobject = "d_42025_D01"
...
CHOOSE CASE dwo.name
	CASE "style" 
		IF ib_itemchanged THEN RETURN 1
   	return Parent.Trigger Event ue_Popup(dwo.name, row, data, 1)
     	dw_body.Setitem(row, "amt", dw_body.getitemdecimal(row, "price") * dw_body.getitemdecimal(row, "out_qty"))
...
//			dw_body.Setitem(il_rows, "style_chk", ls_style_chk)
   	...
   	dw_body.Setitem(row, "amt", ldc_tag_price * dw_body.getitemdecimal(row, "out_qty"))
...
type dw_print from w_com010_e`dw_print within w_42025_e
integer x = 2510
integer y = 1464
end type
