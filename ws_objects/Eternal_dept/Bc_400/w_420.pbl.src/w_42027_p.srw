$PBExportHeader$w_42027_p.srw
$PBExportComments$출고합계출력(DPS)
forward
global type w_42027_p from w_com020_d
end type
type cb_print1 from commandbutton within w_42027_p
end type
type cbx_a4 from checkbox within w_42027_p
end type
type dw_print1 from datawindow within w_42027_p
end type
end forward
...
String is_yymmdd, is_check_print, is_shop_cd, is_shop_type, is_out_no, is_fr_dps_num, is_to_dps_num, is_rqst_date
...
if cbx_a4.checked then 		
	ls_jup_name = "(매 장 용)"								 
			...
			if k > 1 then
							dw_print.dataobject = "d_com422_DPS_a4_new"
							dw_print.SetTransObject(SQLCA)
						
				ll_rows = dw_print.retrieve(is_brand, ls_yymmdd, is_fr_dps_num, is_to_dps_num , ll_work_no[1], ll_work_no[2], ll_work_no[3], ll_work_no[4], ll_work_no[5], ll_work_no[6], ll_work_no[7], ll_work_no[8], ll_work_no[9], ll_work_no[10], &
													 ll_work_no[11], ll_work_no[12], ll_work_no[13], ll_work_no[14], ll_work_no[15], ll_work_no[16], ll_work_no[17], ll_work_no[18], ll_work_no[19], ll_work_no[20], &
													 ll_work_no[21], ll_work_no[22], ll_work_no[23], ll_work_no[24], ll_work_no[25], ll_work_no[26], ll_work_no[27], ll_work_no[28], ll_work_no[29], ll_work_no[30], &
													 	is_rqst_date)							
...
il_rows = dw_list.retrieve(is_brand, is_yymmdd_st, is_yymmdd_ed, is_fr_dps_num, is_to_dps_num, is_house_cd, is_jup_gubn, is_out_type, is_gubun, is_rqst_date)
dw_body.Reset()
...
is_fr_dps_num = dw_head.GetItemString(1, "fr_dps_num")
IF IsNull(is_fr_dps_num) OR Trim(is_fr_dps_num) = "" THEN is_fr_dps_num = '000'

is_to_dps_num = dw_head.GetItemString(1, "to_dps_num")
IF IsNull(is_to_dps_num) OR Trim(is_to_dps_num) = "" THEN is_to_dps_num = '999'
...
is_rqst_date = dw_head.GetItemstring(1, "rqst_date")
IF IsNull(is_rqst_date) OR Trim(is_rqst_date) = "" THEN
   MessageBox(ls_title,"요청일자를 입력하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("rqst_date")
   RETURN FALSE
END IF
...
CHOOSE CASE as_column
	CASE "fr_dps_num"
		...
		IF ai_div = 1 THEN 	
			...
			IF gf_shop_dps_nm(as_data, is_brand, "S", ls_shop_nm) = 0 THEN
		...
		IF Trim(as_data) <> "" THEN
			gst_cd.Item_where = "dps_num LIKE '" + as_data + "%'"
		ELSE
			gst_cd.Item_where = ""
		END IF
		...
		IF Isvalid(Message.PowerObjectParm) THEN
			...
			dw_head.SetItem(al_row, "fr_dps_num", lds_Source.GetItemString(1,"dps_num"))			
			dw_head.SetItem(al_row, "shop_nm_st", lds_Source.GetItemString(1,"shop_snm"))
			/* 다음컬럼으로 이동 */
			dw_head.SetColumn("fr_dps_num")
	...
	CASE "to_dps_num"
		...
		IF Isvalid(Message.PowerObjectParm) THEN
			...
			dw_head.SetRow(al_row)
			dw_head.SetColumn(as_column)
			dw_head.SetItem(al_row, "to_dps_num", lds_Source.GetItemString(1,"dps_num"))			
...
if k > 1 then
          	dw_print.dataobject = "d_com422_dps"
				dw_print.SetTransObject(SQLCA)
...
//				ll_rows = dw_print.retrieve(is_brand, ls_yymmdd, is_fr_dps_num, is_to_dps_num ,ll_work_no[1], ll_work_no[2], ll_work_no[3], ll_work_no[4], ll_work_no[5], ll_work_no[6], ll_work_no[7], ll_work_no[8], ll_work_no[9], ll_work_no[10], is_rqst_date)
				ll_rows = dw_print.retrieve(is_brand, ls_yymmdd, is_fr_dps_num, is_to_dps_num ,ll_work_no[1], ll_work_no[2], ll_work_no[3], ll_work_no[4], ll_work_no[5], ll_work_no[6], ll_work_no[7], ll_work_no[8], ll_work_no[9], ll_work_no[10], &
													 ll_work_no[11], ll_work_no[12], ll_work_no[13], ll_work_no[14], ll_work_no[15], ll_work_no[16], ll_work_no[17], ll_work_no[18], ll_work_no[19], ll_work_no[20], &
													 ll_work_no[21], ll_work_no[22], ll_work_no[23], ll_work_no[24], ll_work_no[25], ll_work_no[26], ll_work_no[27], ll_work_no[28], ll_work_no[29], ll_work_no[30], &
													 	is_rqst_date)
...
dw_head.setitem(1, "rqst_date", ls_date)
end event

type cb_close from w_com020_d`cb_close within w_42027_p
end type

type cb_delete from w_com020_d`cb_delete within w_42027_p
end type

type cb_insert from w_com020_d`cb_insert within w_42027_p
end type

type cb_retrieve from w_com020_d`cb_retrieve within w_42027_p
end type

type cb_update from w_com020_d`cb_update within w_42027_p
end type

type cb_print from w_com020_d`cb_print within w_42027_p
end type

type cb_preview from w_com020_d`cb_preview within w_42027_p
integer width = 384
end type

type gb_button from w_com020_d`gb_button within w_42027_p
end type

type cb_excel from w_com020_d`cb_excel within w_42027_p
end type

type dw_head from w_com020_d`dw_head within w_42027_p
integer y = 144
integer height = 372
string dataobject = "d_42027_h01_dps"
end type
...
type ln_1 from w_com020_d`ln_1 within w_42027_p
integer beginy = 520
integer endy = 520
end type

type ln_2 from w_com020_d`ln_2 within w_42027_p
integer beginy = 524
integer endy = 524
end type

type dw_list from w_com020_d`dw_list within w_42027_p
integer x = 14
integer y = 540
integer width = 1184
integer height = 1500
string dataobject = "d_42027_d01_dps"
end type
...
//messagebox("is_fr_dps_num", is_fr_dps_num)
//messagebox("is_to_dps_num", is_to_dps_num)
...
il_rows = dw_body.retrieve(is_brand, is_yymmdd, is_fr_dps_num, is_to_dps_num, is_house_cd, &
									is_jup_gubn, is_out_type,  il_work_no, is_gubun,is_rqst_date)
Parent.Trigger Event ue_button(7, il_rows)
Parent.Trigger Event ue_msg(1, il_rows)

end event

type dw_body from w_com020_d`dw_body within w_42027_p
integer x = 1211
integer y = 536
integer width = 2391
integer height = 1504
string dataobject = "d_42027_d02_dps"
end type
...
type st_1 from w_com020_d`st_1 within w_42027_p
integer x = 1193
integer y = 540
integer height = 1500
end type

type dw_print from w_com020_d`dw_print within w_42027_p
integer x = 1143
integer y = 896
integer width = 2281
integer height = 440
string dataobject = "d_com422_dps"
end type
