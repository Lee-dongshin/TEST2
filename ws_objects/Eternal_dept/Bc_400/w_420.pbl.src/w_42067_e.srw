...
String ls_shop_cd, ls_shop_type, ls_box_size, ls_out_no_b, li_no, ls_rot_shop_no
long i, ll_row_count, ll_sqlcode, ll_cnt
datetime ld_datetime

IF dw_body.AcceptText() <> 1 THEN RETURN -1

IF MessageBox("확인", "선택한 제품을 창고로 반품처리 하시겠습니까 ?", Question!, YesNo! ) = 2 THEN 
	RETURN 0 
END IF
...
//	ls_box_no_house 	= dw_body.GetItemString(i, "rqst_no_house")	
  ...
  IF idw_status = DataModified! and ls_proc_yn = "Y" THEN				/* New Record */
	...
	/* 반품데이터 적용 */
		...
		:ls_BOX_SIZE,	BOX_NO,			'스캔:' + :ls_yymmdd + '/' + :ls_out_no + '/' + House_CD
		 from tb_42021_shop								  
		 where yymmdd    = :ls_yymmdd
		   and out_no    = :ls_out_no
			and shop_cd   = :ls_shop_cd
...
This.GetChild("house", ldw_house_cd)
