$PBExportHeader$w_53010_e.srw
$PBExportComments$판매등록_복합매장
forward
global type w_53010_e from w_com010_e
end type
type cb_input from commandbutton within w_53010_e
end type
type st_2 from statictext within w_53010_e
end type
type dw_list from datawindow within w_53010_e
end type
type dw_1 from datawindow within w_53010_e
end type
type dw_2 from datawindow within w_53010_e
end type
type st_1 from statictext within w_53010_e
end type
end forward
...
IF ls_style  <> 'NC5AJ973' AND ls_style <> 'NC5AJ911' AND ls_style <> 'NC5AL911' AND ls_style <> 'NC5AJ910' AND ls_style <> 'NC5MB628' AND ls_style <> 'NC5AS910' AND ls_style <> 'NC5MB962' AND ls_style <> 'NC5MB964' AND ls_style <> 'NC5MJ965' AND ls_style <> 'NC5AJ913' AND ls_style <> 'NC5AL913' AND ls_style <> 'NC5AS913' AND ls_style <> 'NC5AJ915' AND ls_style <> 'NC5AL915' AND ls_style <> 'NC5AS915' THEN 				
	if is_shop_type <> '9' and ls_sojae = "C" and LeftA(ls_style,1) <> 'B' and LeftA(ls_style,1) <> 'P' and LeftA(ls_style,1) <> 'K'  and LeftA(ls_style,1) <> 'U'  then 
...
from eternal.dbo.tb_12020_m with (nolock)
where style like :ls_style + '%';
...
//				if is_shop_type <> '9' and ls_sojae = "C" then 
				if is_shop_type <> '9' and ls_sojae = "C" and LeftA(ls_style,1) <> 'B' and LeftA(ls_style,1) <> 'P' and LeftA(ls_style,1) <> 'K' and LeftA(ls_style,1) <> 'U' and LeftA(ls_style,1) <> 'L' and LeftA(ls_style,1) <> 'V' then 
					messagebox("경고!", "중국수출 모델은 기타에서만 등록 가능합니다!")
					ib_itemchanged = FALSE
					return 1
				end if	
...
//				END IF 				
               ...
               cb_update.enabled = true					
			      ...
			      lb_check = TRUE 
					...
					dw_1.retrieve(ls_style, is_shop_cd)
					dw_2.retrieve(ls_style, is_shop_cd)
               This.Post Event ue_tot_set()
				END IF
				ib_itemchanged = FALSE
			END IF
		...
		is_brand = dw_head.GetItemString(1, "brand")
		   ...
		   gst_cd.ai_div          = ai_div
			...
			IF Isvalid(Message.PowerObjectParm) THEN
				...
				dw_2.retrieve(is_style, is_shop_cd)
			END IF
			Destroy  lds_Source
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_53010_e","0")
end event
...
type cb_close from w_com010_e`cb_close within w_53010_e
integer taborder = 140
end type

type cb_delete from w_com010_e`cb_delete within w_53010_e
integer taborder = 130
end type

type cb_insert from w_com010_e`cb_insert within w_53010_e
integer taborder = 70
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_53010_e
integer x = 2839
integer width = 384
integer taborder = 40
string text = "일보조회(&Q)"
...
type cb_update from w_com010_e`cb_update within w_53010_e
end type

type cb_print from w_com010_e`cb_print within w_53010_e
boolean visible = false
integer taborder = 100
end type

type cb_preview from w_com010_e`cb_preview within w_53010_e
boolean visible = false
integer taborder = 110
end type

type gb_button from w_com010_e`gb_button within w_53010_e
end type

type cb_excel from w_com010_e`cb_excel within w_53010_e
integer taborder = 120
boolean enabled = true
end type

type dw_head from w_com010_e`dw_head within w_53010_e
integer y = 156
integer width = 3447
integer height = 220
string dataobject = "d_53010_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_53010_e
integer beginy = 856
integer endy = 856
end type

type ln_2 from w_com010_e`ln_2 within w_53010_e
integer beginy = 860
integer endy = 860
end type

type dw_body from w_com010_e`dw_body within w_53010_e
...
/* 수정일      : 2002.03.04                                                  */
/*===========================================================================*/
Long    ll_ret, ll_sale_qty, ll_sale_price, ll_collect_price  , ll_dc_rate, ll_row_count, ll_curr_price
Decimal ldc_sale_marjin, ld_goods_amt, ld_dc_rate, ld_sale_price
String  ll_null
string ls_card_no//, ls_shop_type
SetNull(ll_null)
...
CASE "on_dc_rate"
			...
			ldc_sale_marjin = This.GetitemDecimal(row, "sale_rate")
			ll_sale_qty = This.GetitemNumber(row, "sale_qty")
//			ls_shop_type  = This.GetitemNumber(row, "shop_type")
			IF ll_dc_rate > 0 THEN 
				...
				if ld_dc_rate > 0 then
					...
					gf_marjin_price(is_shop_cd, ll_sale_price, ld_dc_rate, ll_collect_price) 
					
					idc_dc_rate_org = ld_dc_rate
					This.Setitem(row, "dc_rate_org",   gsv_cd.gl_cd1) 
					This.Setitem(row, "dc_rate",       ll_dc_rate + idc_dc_rate_org) 
					This.Setitem(row, "sale_rate",     ldc_sale_marjin )// gsv_cd.gdc_cd1) 
					This.Setitem(row, "sale_price",    ll_sale_price)
					This.Setitem(row, "collect_price", ll_collect_price)
					if is_shop_type >= '3' and is_shop_type < '9' then
...
type dw_print from w_com010_e`dw_print within w_53010_e
end type
