$PBExportHeader$w_53030_e.srw
$PBExportComments$판매등록
forward
global type w_53030_e from w_com010_e
end type
type dw_member from datawindow within w_53030_e
end type
type st_1 from statictext within w_53030_e
end type
type cb_input from commandbutton within w_53030_e
end type
type dw_list from datawindow within w_53030_e
end type
end forward
...
IF ll_remain_point < 5000 		then 
	MessageBox("Point 오류", "남은 포인트가 5000점 이상 일때만 사용가능 합니다.")
   wf_goods_amt_clear()
	return false               
END IF

ll_goods_amt = dw_member.GetitemNumber(1, "goods_amt")  // point금액 
ld_mok = MOD(ld_goods_amt , 50000) 
...
FOR i=1 TO ll_row_count
   ...
   ls_item       = RightA(LeftA(ls_style_no,2),1)
	...
	IF ll_goods_amt > 0 and ll_sale_price > ll_goods_amt and  & 
	   ll_sale_qty  > 0 and LeftA(dw_body.Object.sale_type[i], 2) = '11' then // and &
//		ls_item  <> 'X' THEN  
      ls_sale_fg = '2' 
		if ld_goods_amt >=  50000 then 
			dw_body.Setitem(i,"goods_amt", 50000)
		   ld_goods_amt = ld_goods_amt - 50000
		else 	
		   dw_body.Setitem(i,"goods_amt", 0)
		end if
	ELSE
   ...
   /* 교환권 판매 처리 및 가능여부 체크 (정상판매단가가  교환권금액 이상 매출만 가능) */
	IF is_shop_type = '1' or is_shop_type = '3' THEN  /* 정상 매장만 처리 */
      ...
      ls_item      = RightA(LeftA(ls_style_no,2),1)
		...
		ELSEIF LenA(ls_card_no) = 16 and LeftA(dw_body.Object.sale_type[i], 1) < '3' THEN  // 정상+세일 적용 
			ls_sale_fg = '1' 
			dw_body.Setitem(i, "goods_amt", 0)
		ELSE
			ls_sale_fg = '0' 
...
type cb_close from w_com010_e`cb_close within w_53030_e
integer taborder = 140
end type

type cb_delete from w_com010_e`cb_delete within w_53030_e
integer taborder = 130
end type

type cb_insert from w_com010_e`cb_insert within w_53030_e
integer taborder = 70
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_53030_e
integer x = 2839
integer width = 384
integer taborder = 40
string text = "일보조회(&Q)"
...
type cb_update from w_com010_e`cb_update within w_53030_e
end type

type cb_print from w_com010_e`cb_print within w_53030_e
boolean visible = false
integer taborder = 100
end type

type cb_preview from w_com010_e`cb_preview within w_53030_e
boolean visible = false
integer taborder = 110
end type

type gb_button from w_com010_e`gb_button within w_53030_e
end type

type cb_excel from w_com010_e`cb_excel within w_53030_e
boolean visible = false
integer taborder = 120
end type

type dw_head from w_com010_e`dw_head within w_53030_e
integer width = 3447
integer height = 156
...
type ln_1 from w_com010_e`ln_1 within w_53030_e
integer beginy = 340
integer endy = 340
end type

type ln_2 from w_com010_e`ln_2 within w_53030_e
integer beginy = 344
integer endy = 344
end type

type dw_body from w_com010_e`dw_body within w_53030_e
...
type dw_print from w_com010_e`dw_print within w_53030_e
end type
