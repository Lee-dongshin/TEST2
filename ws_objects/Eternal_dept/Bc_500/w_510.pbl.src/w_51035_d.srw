$PBExportHeader$w_51035_d.srw
$PBExportComments$광고선전비 집행 내역
forward
global type w_51035_d from w_com010_d
end type
end forward
...
global w_51035_d w_51035_d

type variables
DataWindowChild	idw_brand
String is_brand, is_empno, is_fr_yymm, is_to_yymm, is_inter_cd
end variables
...
if IsNull(is_fr_yymm) or Trim(is_fr_yymm) = "" then
   MessageBox(ls_title,"집행 시작월을 입력하십시요!")
   dw_head.SetFocus()
...
if IsNull(is_to_yymm) or Trim(is_to_yymm) = "" then
   MessageBox(ls_title,"집행 종료월을 입력하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("to_yymm")
   return false
end if

is_inter_cd = dw_head.GetItemString(1, "inter_cd")
if IsNull(is_inter_cd) or Trim(is_inter_cd) = "" then
   is_inter_cd = '%'
end if
...
dw_head.setitem(1, "inter_cd", '%')



end event

event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_51035_d","0")
end event
...
il_rows = dw_body.retrieve(is_brand, is_empno, is_fr_yymm, is_to_yymm, is_inter_cd)
...
where		a.slip_appl_code like :is_brand + '900'
			...
			and c.acc_code   = '43223'
			and c.cust_cha   = '1';

ld_s2 = dw_body.getitemnumber(1, 'compute_19')
...
ls_modify =	"t_pg_id.Text = '" + is_pgm_id + "'" + &
            ...
            "t_s_1.Text = '" + string(dw_head.getitemnumber(1,'s_1'),'#,###') + "'" + &
				...
				"t_fr_yymm.Text = '" + (dw_head.getitemstring(1,'fr_yymm')) + "'" + &
				"t_to_yymm.Text = '" + (dw_head.getitemstring(1,'to_yymm')) + "'" + &
...
type cb_close from w_com010_d`cb_close within w_51035_d
end type

type cb_delete from w_com010_d`cb_delete within w_51035_d
end type

type cb_insert from w_com010_d`cb_insert within w_51035_d
boolean enabled = false
end type

type cb_retrieve from w_com010_d`cb_retrieve within w_51035_d
end type

type cb_update from w_com010_d`cb_update within w_51035_d
end type

type cb_print from w_com010_d`cb_print within w_51035_d
end type

type cb_preview from w_com010_d`cb_preview within w_51035_d
end type

type gb_button from w_com010_d`gb_button within w_51035_d
end type

type cb_excel from w_com010_d`cb_excel within w_51035_d
end type

type dw_head from w_com010_d`dw_head within w_51035_d
integer x = 9
integer y = 160
integer width = 3598
integer height = 452
string dataobject = "d_51035_h01"
end type
...
idw_grp.insertrow(1)
idw_grp.SetItem(1, "inter_cd", '%')
idw_grp.SetItem(1, "inter_nm", '전체')
end event
...
type ln_1 from w_com010_d`ln_1 within w_51035_d
integer beginy = 612
integer endy = 612
end type

type ln_2 from w_com010_d`ln_2 within w_51035_d
integer beginy = 616
integer endy = 616
end type

type dw_body from w_com010_d`dw_body within w_51035_d
integer y = 632
integer width = 3598
integer height = 1380
string dataobject = "d_51035_d01"
end type
...
type dw_print from w_com010_d`dw_print within w_51035_d
integer x = 2450
integer y = 364
string dataobject = "d_51035_r01"
end type
