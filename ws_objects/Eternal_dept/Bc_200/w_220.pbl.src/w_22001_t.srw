$PBExportHeader$w_22001_t.srw
$PBExportComments$원자재 입고 등록
forward
global type w_22001_t from w_com020_e
end type
type dw_mast from u_dw within w_22001_t
end type
end forward
...
String   ls_color, ls_spec, ls_fin_yn, ls_in_max, ls_in_ymd, ls_in_no
...
//			ls_color     = dw_body.GetItemString (i, "color")
			...
			ll_in_max++
			ls_in_max = String(ll_in_max, '0000')
			dw_body.Setitem(i, "in_no",      ls_in_max)
			...
			ls_out_seq    = dw_body.GetItemString(i, "out_seq")
			  ...
			  VALUES (:is_brand, :is_yymmdd, '01', :ls_in_max, :is_mmat_cd, :ls_color, :ldc_tmp_qty1, :ldc_tmp_qty1,
		...
		Else
			...
			;
			If SQLCA.SQLCODE <> 0 Then
				MessageBox("저장오류", "TB_22011_D(원자재 검단 내역) UPDATE에 실패하였습니다!")
				rollback  USING SQLCA;
				dw_body.SetReDraw(True)
				Return -1
			End If
...
type cb_close from w_com020_e`cb_close within w_22001_t
integer taborder = 120
end type

type cb_delete from w_com020_e`cb_delete within w_22001_t
boolean visible = false
integer taborder = 70
end type

type cb_insert from w_com020_e`cb_insert within w_22001_t
integer taborder = 60
boolean enabled = true
end type

type cb_retrieve from w_com020_e`cb_retrieve within w_22001_t
end type

type cb_update from w_com020_e`cb_update within w_22001_t
integer taborder = 110
end type

type cb_print from w_com020_e`cb_print within w_22001_t
integer taborder = 80
end type

type cb_preview from w_com020_e`cb_preview within w_22001_t
integer taborder = 90
end type

type gb_button from w_com020_e`gb_button within w_22001_t
end type

type cb_excel from w_com020_e`cb_excel within w_22001_t
integer taborder = 100
end type

type dw_head from w_com020_e`dw_head within w_22001_t
integer height = 228
...
type ln_1 from w_com020_e`ln_1 within w_22001_t
integer beginy = 424
integer endy = 424
end type

type ln_2 from w_com020_e`ln_2 within w_22001_t
integer beginy = 428
integer endy = 428
end type

type dw_list from w_com020_e`dw_list within w_22001_t
integer y = 612
integer width = 718
integer height = 1428
integer taborder = 40
...
type dw_body from w_com020_e`dw_body within w_22001_t
integer x = 768
integer y = 612
integer width = 2825
integer height = 1428
...
type st_1 from w_com020_e`st_1 within w_22001_t
integer x = 750
integer y = 612
integer height = 1428
end type

type dw_print from w_com020_e`dw_print within w_22001_t
