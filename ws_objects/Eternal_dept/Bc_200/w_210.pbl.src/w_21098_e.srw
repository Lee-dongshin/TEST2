$PBExportHeader$w_21098_e.srw
$PBExportComments$케어라벨입력
forward
global type w_21098_e from w_com030_e
end type
type dw_2 from datawindow within w_21098_e
end type
type dw_copy_ord from datawindow within w_21098_e
end type
type dw_1 from datawindow within w_21098_e
end type
type dw_3 from datawindow within w_21098_e
end type
type dw_4 from datawindow within w_21098_e
end type
type dw_5 from datawindow within w_21098_e
end type
type dw_9 from datawindow within w_21098_e
end type
type dw_6 from datawindow within w_21098_e
end type
type dw_7 from datawindow within w_21098_e
end type
type dw_8 from datawindow within w_21098_e
end type
end forward
...
public subroutine wf_mat_rate_copy (string as_style, string as_chno, string as_color);string  as_mat_nm1, as_mat_rate1, as_mat_nm2,as_mat_rate2, as_mat_nm3,as_mat_rate3, as_mat_nm4, as_mat_rate4, as_mat_nm5, as_mat_rate5, as_mat_nm6, as_mat_rate6, as_mat_nm7, as_mat_rate7, as_mat_nm8, as_mat_rate8        
string  ls_mat_nm1, ls_mat_rate1, ls_mat_nm2,ls_mat_rate2, ls_mat_nm3,ls_mat_rate3, ls_mat_nm4, ls_mat_rate4, ls_mat_nm5, ls_mat_rate5, ls_mat_nm6, ls_mat_rate6, ls_mat_nm7, ls_mat_rate7, ls_mat_nm8, ls_mat_rate8
...
ls_mat_nm8 =  "' '"  
	...
	SELECT 	A.MAT_NM1,
	 ...
	 if ls_mat_nm1 = "' '"   then 
			SELECT 	dbo.sf_inter_nm('213',A.MAT_NM1)	as mat_nm1 ,
						...
						A.MAT_RATE5, 
						dbo.sf_inter_nm('213',A.MAT_NM6)	as mat_nm6 ,
						A.MAT_RATE6, 
						dbo.sf_inter_nm('213',A.MAT_NM7)	as mat_nm7 ,
						A.MAT_RATE7, 
						dbo.sf_inter_nm('213',A.MAT_NM8)	as mat_nm8 ,
						A.MAT_RATE8 
	 ...
	 as_mat_nm6 = dw_body.GetitemString(1,"mat_nm6")
	 as_mat_nm7 = dw_body.GetitemString(1,"mat_nm7")
	 as_mat_nm8 = dw_body.GetitemString(1,"mat_nm8")	 
	...
	if ls_mat_nm5 <>  "' '" and (dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4) < 100  or  dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4)  <= 100  and  ls_mat_rate5 = "' '" )  	then 
	...
	if ls_mat_nm6 <>  "' '" and (dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4) < 100  or  dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4) + dec(ls_mat_rate5) <= 100  and  ls_mat_rate6 = "' '" )  	then 
	...
	if ls_mat_nm7 <>  "' '" and (dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4) < 100  or  dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4) + dec(ls_mat_rate5) + dec(ls_mat_rate6) <= 100  and  ls_mat_rate7 = "' '" )  	then 
	...
	if ls_mat_nm8 <>  "' '" and (dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4) < 100  or  dec(ls_mat_rate1) + dec(ls_mat_rate2) + dec(ls_mat_rate3) + dec(ls_mat_rate4) + dec(ls_mat_rate5) + dec(ls_mat_rate6) + dec(ls_mat_rate7) <= 100  and  ls_mat_rate8 = "' '" )  	then 
...
//if dw_body.dataobject = "d_21098_d01_test3"  then //겉감
...
//	messagebox("확인","혼용율이 100%가 아닙니다..다시한번 확인해주세요..")
	li_quest = MessageBox("확인","혼용율이 100%가 아닙니다..저장하시겠습니까?..",Exclamation!, OKCancel!, 2)
	IF li_quest <> 1 THEN
			messagebox("확인","혼용율이 100%가 아닙니다..다시한번 확인해주세요")
			return 0
	END IF	
...
//			convert(int,replace(MAT_RATE8,:ls_qute,'')) + 
//			convert(int,replace(MAT_RATE9,:ls_qute,'')) + 
//			convert(int,replace(MAT_RATE10,:ls_qute,'')) + 
//			convert(int,replace(MAT_RATE11,:ls_qute,'')) + 
//			convert(int,replace(MAT_RATE12,:ls_qute,''))  ) % 100) 
//			

		select ((
			convert(int,case when isnull(mat_rate1,'0') = :ls_qute then '0' else isnull(mat_rate1,'0') end) +
			convert(int,case when isnull(mat_rate2,'0') = :ls_qute then '0' else isnull(mat_rate2,'0') end) +
			convert(int,case when isnull(mat_rate3,'0') = :ls_qute then '0' else isnull(mat_rate3,'0') end) +
			convert(int,case when isnull(mat_rate4,'0') = :ls_qute then '0' else isnull(mat_rate4,'0') end) +
			convert(int,case when isnull(mat_rate5,'0') = :ls_qute then '0' else isnull(mat_rate5,'0') end) +
			convert(int,case when isnull(mat_rate6,'0') = :ls_qute then '0' else isnull(mat_rate6,'0') end) +
			convert(int,case when isnull(mat_rate7,'0') = :ls_qute then '0' else isnull(mat_rate7,'0') end) +
			convert(int,case when isnull(mat_rate8,'0') = :ls_qute then '0' else isnull(mat_rate8,'0') end) +
			convert(int,case when isnull(mat_rate9,'0') = :ls_qute then '0' else isnull(mat_rate9,'0') end) +
			convert(int,case when isnull(mat_rate10,'0') = :ls_qute then '0' else isnull(mat_rate10,'0') end) +
			convert(int,case when isnull(mat_rate11,'0') = :ls_qute then '0' else isnull(mat_rate11,'0') end) +
			convert(int,case when isnull(mat_rate12,'0') = :ls_qute then '0' else isnull(mat_rate12,'0') end)  ) % 100) 
				into :li_chk
	...
	select ((
			...
			convert(int,case when isnull(mat_rate6,'0') = :ls_qute then '0' else isnull(mat_rate6,'0') end) +
			convert(int,case when isnull(mat_rate7,'0') = :ls_qute then '0' else isnull(mat_rate7,'0') end) +
			convert(int,case when isnull(mat_rate8,'0') = :ls_qute then '0' else isnull(mat_rate8,'0') end)  ) % 100) 
			into :li_chk
...
event open;call super::open;dw_body.ShareData(dw_8)

end event

type cb_close from w_com030_e`cb_close within w_21098_e
end type

type cb_delete from w_com030_e`cb_delete within w_21098_e
boolean visible = false
end type

type cb_insert from w_com030_e`cb_insert within w_21098_e
string text = "복사"
end type
...
type cb_retrieve from w_com030_e`cb_retrieve within w_21098_e
end type

type cb_update from w_com030_e`cb_update within w_21098_e
end type

type cb_print from w_com030_e`cb_print within w_21098_e
boolean visible = false
end type

type cb_preview from w_com030_e`cb_preview within w_21098_e
boolean visible = false
end type

type gb_button from w_com030_e`gb_button within w_21098_e
integer x = 5
end type

type cb_excel from w_com030_e`cb_excel within w_21098_e
integer width = 439
string text = "케어라벨 복사!"
end type
...
type dw_head from w_com030_e`dw_head within w_21098_e
integer y = 184
integer height = 228
...
type ln_1 from w_com030_e`ln_1 within w_21098_e
end type

type ln_2 from w_com030_e`ln_2 within w_21098_e
end type

type dw_list from w_com030_e`dw_list within w_21098_e
integer x = 0
integer width = 1381
...
string ls_mat_nm1, ls_remark1, ls_country_nm, ls_sect_nm, ls_flag, ls_cust_nm, ls_sojae,ls_mat_nm2, ls_maker_fg
...
ls_maker_fg  = dw_body.GetItemString(1, 'maker_fg') /* DataWindow에 Key 항목을 가져온다 */


if upper(ls_country_nm) <> 'KOREA' and ls_maker_fg <> "1" then 
...
ls_mat_nm2 = dw_body.getitemstring(1,"mat_nm2")
if ls_mat_nm1 = "' '" and ls_mat_nm2 = "' '" then	wf_mat_rate_copy(is_style,is_chno,is_color)	

end event

type dw_body from w_com030_e`dw_body within w_21098_e
integer x = 1403
integer width = 2194
...
ls_filter_str = ''	
ls_filter_str = "  care_code < '주의01' or  care_code > '주의27'  "
...
//		dw_body.dataobject = "d_21098_d02_test3"  //속감등록		
		dw_body.SetTransObject(SQLCA)
		
		dw_8.dataobject = "d_21098_d12"  //속감등록
//		dw_8.dataobject = "d_21098_d12_test3"  //속감등록
		dw_8.SetTransObject(SQLCA)

dw_8.getchild("mat_gubn1_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn2_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn3_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn4_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn5_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn6_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn7_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn8_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

dw_8.getchild("mat_gubn9_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')
dw_8.getchild("mat_gubn10_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')
dw_8.getchild("mat_gubn11_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')
dw_8.getchild("mat_gubn12_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')



dw_8.getchild("mat_nm1_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm2_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm3_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm4_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm5_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm6_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm7_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm8_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

dw_8.getchild("mat_nm9_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')
dw_8.getchild("mat_nm10_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')
dw_8.getchild("mat_nm11_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')
dw_8.getchild("mat_nm12_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')


dw_8.getchild("care_code1",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve()		
		
		dw_body.ShareData(dw_8)
...
//		dw_8.dataobject = "d_21098_d11"  //겉감등록		 
		dw_8.dataobject = "d_21098_d11"  //겉감등록		 
		dw_8.SetTransObject(SQLCA)
...
/* 수정일      : 1999.11.08                                                  */
/*===========================================================================*/
string  ls_care_code, ls_remark1, ls_remark2, ls_remark3, ls_remark4, ls_remark5, ls_remark6, ls_remark7, ls_washing_type
...
string  ls_pic1_chn, ls_pic2_chn, ls_pic3_chn, ls_pic4_chn, ls_pic5_chn
string  ls_washing_chn1, ls_washing_chn2, ls_washing_chn3, ls_washing_chn4, ls_washing_chn5, ls_washing_chn6
string  ls_pic_chn1, ls_pic_chn2, ls_pic_chn3, ls_pic_chn4, ls_pic_chn5, ls_pic_chn6
datetime ld_datetime
string ls_confirm_chn, ls_mat_gubn1_chn
string ls_mat_gubn1,	ls_mat_gubn2,	ls_mat_gubn3,	ls_mat_gubn4,	ls_mat_gubn5,	ls_mat_gubn6,	ls_mat_gubn7,	ls_mat_gubn8
string ls_mat_gubn9,ls_mat_gubn10,ls_mat_gubn11,ls_mat_gubn12
string ls_mat_nm1,	ls_mat_nm2,		ls_mat_nm3,		ls_mat_nm4,		ls_mat_nm5,		ls_mat_nm6,		ls_mat_nm7,		ls_mat_nm8
string ls_mat_nm9,ls_mat_nm10,ls_mat_nm11,ls_mat_nm12
string ls_mat_rate1,	ls_mat_rate2,	ls_mat_rate3,	ls_mat_rate4,	ls_mat_rate5,	ls_mat_rate6,	ls_mat_rate7,	ls_mat_rate8
string ls_mat_rate9,	ls_mat_rate10,	ls_mat_rate11,	ls_mat_rate12	
...
CHOOSE CASE dwo.name
	...
	CASE "care_code" 
   	  ...
   	  select  remark1, remark2, remark3, remark4, remark5, remark6, remark7
		  into    :ls_remark1, :ls_remark2, :ls_remark3, :ls_remark4, :ls_remark5, :ls_remark6, :ls_remark7
		  from tb_care_label
		  ...
		  dw_body.Setitem(1, "remark5", ls_remark5) 
  		  dw_body.Setitem(1, "remark6", ls_remark6)
		  dw_body.Setitem(1, "remark7", ls_remark7)

	  CASE "washing_type"
		  ls_washing_type = string(data)
		  select  washing1, washing2, washing3, washing4, washing5, washing6,
		 		    washing1_chn, washing2_chn, washing3_chn, washing4_chn, washing5_chn
		  into    :is_washing1,  :is_washing2,  :is_washing3,  :is_washing4,  :is_washing5,  :is_washing6,
		  		    :ls_washing_chn1,  :ls_washing_chn2,  :ls_washing_chn3,  :ls_washing_chn4,  :ls_washing_chn5
		  ...
		  dw_body.Setitem(1, "washing1_chn", ls_washing_chn1) 
		  dw_body.Setitem(1, "washing2_chn", ls_washing_chn2) 
		  dw_body.Setitem(1, "washing3_chn", ls_washing_chn3) 
		  dw_body.Setitem(1, "washing4_chn", ls_washing_chn4)
		  dw_body.Setitem(1, "washing5_chn", ls_washing_chn5) 
...
//		  dw_body.Setitem(1, "pic6", ls_pic6)
		  ...
		  ls_pic1_chn =  'c:\bnc_dept\bmp\china\'+ ls_washing_chn1 + '.bmp'
		  dw_body.Setitem(1, "pic_chn1", ls_pic1_chn)

		  ls_pic2_chn =  'c:\bnc_dept\bmp\china\'+ ls_washing_chn2 + '.bmp'
		  dw_body.Setitem(1, "pic_chn2", ls_pic2_chn)

		  ls_pic3_chn =  'c:\bnc_dept\bmp\china\'+ ls_washing_chn3 + '.bmp'
		  dw_body.Setitem(1, "pic_chn3", ls_pic3_chn)

		  ls_pic4_chn =  'c:\bnc_dept\bmp\china\'+ ls_washing_chn4 + '.bmp'
		  dw_body.Setitem(1, "pic_chn4", ls_pic4_chn)

		  ls_pic5_chn =  'c:\bnc_dept\bmp\china\'+ ls_washing_chn5 + '.bmp'
		  dw_body.Setitem(1, "pic_chn5", ls_pic5_chn)

 

	CASE "confirm_kor" 
		  if data = "Y" then
 				dw_body.SetItem(1, "confirm_kor_ymd" ,string(ld_datetime,"yyyymmdd"))
 		  else		 
				dw_body.SetItem(1, "confirm_kor_ymd" ,"")		  		  
			end if		
			
	CASE "conform_yn" 
				
			ls_confirm_chn	= this.getitemstring(1, "confirm_chn")			
			ls_remark1 = this.getitemstring(1, "remark1")
			ls_mat_gubn1 = this.getitemstring(1, "mat_gubn1")
			ls_mat_gubn1_chn = this.getitemstring(1, "mat_gubn1_chn")
			ls_mat_nm1 = this.getitemstring(1, "mat_nm1")
			
			if ls_confirm_chn <> "Y" AND (IsNull(ls_mat_gubn1_chn) or ls_mat_gubn1 <> ls_mat_gubn1_chn ) then
				
			   messagebox("알림!","중문 라벨을 한글라벨과 동일하게 등록합니다!")					
				
				ls_remark2 = this.getitemstring(1, "remark2")
				ls_remark3 = this.getitemstring(1, "remark3")
				ls_remark4 = this.getitemstring(1, "remark4")
				ls_remark5 = this.getitemstring(1, "remark5")
				ls_remark6 = this.getitemstring(1, "remark6")
				ls_remark7 = this.getitemstring(1, "remark7")
				
				dw_8.Setitem(1, "remark1_chn", ls_remark1) 
				dw_8.Setitem(1, "remark2_chn", ls_remark2) 
				dw_8.Setitem(1, "remark3_chn", ls_remark3) 
				dw_8.Setitem(1, "remark4_chn", ls_remark4) 
				dw_8.Setitem(1, "remark5_chn", ls_remark5) 
				dw_8.Setitem(1, "remark6_chn", ls_remark6) 
				dw_8.Setitem(1, "remark7_chn", ls_remark7) 
				
				ls_mat_gubn1 = this.getitemstring(1, "mat_gubn1")
				ls_mat_gubn2 = this.getitemstring(1, "mat_gubn2")
				ls_mat_gubn3 = this.getitemstring(1, "mat_gubn3")
				ls_mat_gubn4 = this.getitemstring(1, "mat_gubn4")
				ls_mat_gubn5 = this.getitemstring(1, "mat_gubn5")
				ls_mat_gubn6 = this.getitemstring(1, "mat_gubn6")
				ls_mat_gubn7 = this.getitemstring(1, "mat_gubn7")
				ls_mat_gubn8 = this.getitemstring(1, "mat_gubn8")
				ls_mat_gubn9 = this.getitemstring(1, "mat_gubn9")
				ls_mat_gubn10 = this.getitemstring(1, "mat_gubn10")
				ls_mat_gubn11 = this.getitemstring(1, "mat_gubn11")
				ls_mat_gubn12 = this.getitemstring(1, "mat_gubn12")				
				
				dw_8.Setitem(1, "mat_gubn1_chn", ls_mat_gubn1) 
				dw_8.Setitem(1, "mat_gubn2_chn", ls_mat_gubn2) 
				dw_8.Setitem(1, "mat_gubn3_chn", ls_mat_gubn3) 
				dw_8.Setitem(1, "mat_gubn4_chn", ls_mat_gubn4) 
				dw_8.Setitem(1, "mat_gubn5_chn", ls_mat_gubn5) 	 
				dw_8.Setitem(1, "mat_gubn6_chn", ls_mat_gubn6) 
				dw_8.Setitem(1, "mat_gubn7_chn", ls_mat_gubn7) 
				dw_8.Setitem(1, "mat_gubn8_chn", ls_mat_gubn8) 			
				dw_8.Setitem(1, "mat_gubn9_chn", ls_mat_gubn9) 		
				dw_8.Setitem(1, "mat_gubn10_chn", ls_mat_gubn10) 		
				dw_8.Setitem(1, "mat_gubn11_chn", ls_mat_gubn11) 		
				dw_8.Setitem(1, "mat_gubn12_chn", ls_mat_gubn12) 						
				
				
				ls_mat_nm1 = this.getitemstring(1, "mat_nm1")
				ls_mat_nm2 = this.getitemstring(1, "mat_nm2")
				ls_mat_nm3 = this.getitemstring(1, "mat_nm3")
				ls_mat_nm4 = this.getitemstring(1, "mat_nm4")
				ls_mat_nm5 = this.getitemstring(1, "mat_nm5")
				ls_mat_nm6 = this.getitemstring(1, "mat_nm6")
				ls_mat_nm7 = this.getitemstring(1, "mat_nm7")
				ls_mat_nm8 = this.getitemstring(1, "mat_nm8")
				ls_mat_nm9 = this.getitemstring(1, "mat_nm9")
				ls_mat_nm10 = this.getitemstring(1, "mat_nm10")
				ls_mat_nm11 = this.getitemstring(1, "mat_nm11")
				ls_mat_nm12 = this.getitemstring(1, "mat_nm12")				
				
				dw_8.Setitem(1, "mat_nm1_chn", ls_mat_nm1) 
				dw_8.Setitem(1, "mat_nm2_chn", ls_mat_nm2) 
				dw_8.Setitem(1, "mat_nm3_chn", ls_mat_nm3) 
				dw_8.Setitem(1, "mat_nm4_chn", ls_mat_nm4) 
				dw_8.Setitem(1, "mat_nm5_chn", ls_mat_nm5) 	 
				dw_8.Setitem(1, "mat_nm6_chn", ls_mat_nm6) 
				dw_8.Setitem(1, "mat_nm7_chn", ls_mat_nm7) 
				dw_8.Setitem(1, "mat_nm8_chn", ls_mat_nm8) 
				dw_8.Setitem(1, "mat_nm9_chn", ls_mat_nm9) 
				dw_8.Setitem(1, "mat_nm10_chn", ls_mat_nm10) 
				dw_8.Setitem(1, "mat_nm11_chn", ls_mat_nm11) 
				dw_8.Setitem(1, "mat_nm12_chn", ls_mat_nm12) 				
				
				ls_mat_rate1  = this.getitemstring(1, "mat_rate1")
				ls_mat_rate2  = this.getitemstring(1, "mat_rate2")
				ls_mat_rate3  = this.getitemstring(1, "mat_rate3")
				ls_mat_rate4  = this.getitemstring(1, "mat_rate4")
				ls_mat_rate5  = this.getitemstring(1, "mat_rate5")
				ls_mat_rate6  = this.getitemstring(1, "mat_rate6")
				ls_mat_rate7  = this.getitemstring(1, "mat_rate7")
				ls_mat_rate8  = this.getitemstring(1, "mat_rate8")
				ls_mat_rate9  = this.getitemstring(1, "mat_rate9")
				ls_mat_rate10 = this.getitemstring(1, "mat_rate10")
				ls_mat_rate11 = this.getitemstring(1, "mat_rate11")
				ls_mat_rate12 = this.getitemstring(1, "mat_rate12")				
				
				dw_8.Setitem(1, "mat_rate1_chn",  ls_mat_rate1) 
				dw_8.Setitem(1, "mat_rate2_chn",  ls_mat_rate2) 
				dw_8.Setitem(1, "mat_rate3_chn",  ls_mat_rate3) 
				dw_8.Setitem(1, "mat_rate4_chn",  ls_mat_rate4) 
				dw_8.Setitem(1, "mat_rate5_chn",  ls_mat_rate5) 	 
				dw_8.Setitem(1, "mat_rate6_chn",  ls_mat_rate6) 
				dw_8.Setitem(1, "mat_rate7_chn",  ls_mat_rate7) 
				dw_8.Setitem(1, "mat_rate8_chn",  ls_mat_rate8) 		
				dw_8.Setitem(1, "mat_rate9_chn",  ls_mat_rate9) 		
				dw_8.Setitem(1, "mat_rate10_chn", ls_mat_rate10) 		
				dw_8.Setitem(1, "mat_rate11_chn", ls_mat_rate11) 		
				dw_8.Setitem(1, "mat_rate12_chn", ls_mat_rate12) 						
 		 end if		
		
		  if data = "Y" then
				dw_body.SetItem(1, "confirm_kor" ,"Y")
 				dw_body.SetItem(1, "confirm_kor_ymd" ,string(ld_datetime,"yyyymmdd"))
			end if		
...
type st_1 from w_com030_e`st_1 within w_21098_e
integer x = 1381
end type

type dw_print from w_com030_e`dw_print within w_21098_e
integer x = 1317
integer y = 616
end type
...
event buttonclicked;string ls_fr_style, ls_fr_chno, ls_to_style, ls_to_chno, ls_color, ls_brand
...
choose case dwo.name
	case "cb_copy_ord"
			...
			If ls_brand = 'L' or ls_brand = 'B' then
				
				DECLARE sp_21093_d07_02 PROCEDURE FOR sp_21093_d07_02  
...
this.getchild("mat_gubn1_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn2_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn3_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn4_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')


this.getchild("mat_gubn5_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn6_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn7_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn8_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn9_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn10_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn11_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_gubn12_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('212')

this.getchild("mat_nm1_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

this.getchild("mat_nm2_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

this.getchild("mat_nm3_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

this.getchild("mat_nm4_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

this.getchild("mat_nm5_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

this.getchild("mat_nm6_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

this.getchild("mat_nm7_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')

this.getchild("mat_nm8_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')


this.getchild("mat_nm9_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')
this.getchild("mat_nm10_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')
this.getchild("mat_nm11_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')
this.getchild("mat_nm12_chn",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve('213')
...
this.getchild("care_code1",ldw_child)
ldw_child.settransobject(sqlca)
ldw_child.retrieve()

end event

event buttonclicked;string ls_confirm_kor
string ls_remark1, ls_remark2, ls_remark3, ls_remark4, ls_remark5, ls_remark6, ls_remark7
string ls_mat_gubn1,	ls_mat_gubn2,	ls_mat_gubn3,	ls_mat_gubn4,	ls_mat_gubn5,	ls_mat_gubn6,	ls_mat_gubn7,	ls_mat_gubn8
STRING ls_mat_gubn9,	ls_mat_gubn10,	ls_mat_gubn11,	ls_mat_gubn12
string ls_mat_nm1,	ls_mat_nm2,		ls_mat_nm3,		ls_mat_nm4,		ls_mat_nm5,		ls_mat_nm6,		ls_mat_nm7,		ls_mat_nm8
string ls_mat_nm9, ls_mat_nm10, ls_mat_nm11, ls_mat_nm12
...
CHOOSE CASE dwo.name
	case "cb_ok"  //발행가능내역
		ls_confirm_kor = dw_body.getitemstring(1, "conform_kor")
		messagebox("ls_confirm_kor",ls_confirm_kor)
		dw_body.setitem(1, "confirm_chn", "Y")
		if ls_confirm_kor = "Y" then
			dw_body.setitem(1, "confirm_yn", "Y")
		end if	
		this.visible = false
