$PBExportHeader$w_12010_e.srw
$PBExportComments$작업 지시서
forward
global type w_12010_e from w_com010_e
end type
type cb_del_all from commandbutton within w_12010_e
end type
type dw_master from datawindow within w_12010_e
end type
type dw_detail from datawindow within w_12010_e
end type
type cb_2 from commandbutton within w_12010_e
end type
type dw_label from datawindow within w_12010_e
end type
type tab_1 from tab within w_12010_e
end type
...
type cb_balju from commandbutton within tabpage_4
end type
type cb_order from commandbutton within tabpage_4
end type
type cb_smat from commandbutton within tabpage_4
end type
...
type cb_sketch from commandbutton within w_12010_e
end type
type cb_copy_style from commandbutton within w_12010_e
end type
type dw_7 from datawindow within w_12010_e
end type
type dw_copy_style from datawindow within w_12010_e
end type
type dw_9 from datawindow within w_12010_e
end type
type dw_10 from datawindow within w_12010_e
end type
type cbx_pop from checkbox within w_12010_e
end type
type dw_spec from datawindow within w_12010_e
end type
end forward
...
global w_12010_e w_12010_e

type variables
String  is_style, is_chno, is_brand, is_year, is_season, is_sojae, is_item , is_country_cd, is_make_type, is_orgmat_cd, is_out_seq, is_out_seq2, is_brand_nm, is_design_gubn, is_exp_ymd
String  is_size[], is_zsojae, is_zitem, is_ctrl_brand, is_out_seq_chn, is_make_ymd,is_concept
Boolean ib_read[]
DataWindowChild idw_color, idw_color_3, idw_color_4 , idw_style_type, idw_fabric_by
...
CHOOSE CASE ai_index
	...
	CASE 2
		...
		tab_1.tabpage_2.dw_2.Reset() 
	CASE 3 
		tab_1.tabpage_3.dw_3.Retrieve(is_style, is_chno, ll_user_level) 
	CASE 4 
		tab_1.tabpage_4.dw_4.Retrieve(is_style, is_chno, ll_user_level) 
	CASE 5 
...
if ls_syear >= "20133"  then
	...
	ls_brand1 = ls_brand
				...
				ldw_child.Retrieve('%',ls_brand1)
				ldw_child.SetRedraw(false)
				if ls_brand1 = "P" or ls_brand1 = "B" or ls_brand1 = "K" or  ls_brand1 = 'L' or  ls_brand1 = 'U'  or  ls_brand1 = 'G' or  ls_brand1 = 'T' then
				else
...
IF ls_syear <= '20134' and ls_brand = 'B'  then
	IF gf_sojae_nm(ls_sojae, ls_nm) <> 0 THEN Return False 
elseIF ls_syear >= '20173' and ls_brand = 'B'  or ls_brand = 'P' then
	IF gf_sojae_nm3(ls_sojae,ls_brand,  LeftA(ls_syear,4), ls_season, ls_nm) <> 0 THEN Return False 	
ELSE
...
Long    i, ll_row , ll_fill
String  ls_color, ls_size , ls_label_size_chk
...
FOR i = 1 TO tab_1.tabpage_1.dw_1.RowCount()
   ...
   IF idw_status = NewModified! THEN				/* New Record */
      ...
      tab_1.tabpage_1.dw_1.Setitem(i, "item",   is_item)
		...
		ll_fill = tab_1.tabpage_1.dw_1.GetitemDecimal(i, "fill_weight")
		dw_body.Setitem(ll_row, "fill_weight",   ll_fill)		
...
FOR i = 1 TO tab_1.tabpage_1.dw_1.RowCount() 
	...
	ls_color = tab_1.tabpage_1.dw_1.GetitemString(i, "color")
	ls_size  = tab_1.tabpage_1.dw_1.GetitemString(i, "size")
	ll_row   = dw_body.find("color = '" + ls_color + "' and size = '" + ls_size + "'", 1, dw_body.RowCount()) 
	IF ll_row > 0 THEN 
		ll_fill = tab_1.tabpage_1.dw_1.GetitemDecimal(i, "fill_weight")
		dw_body.Setitem(ll_row, "fill_weight",   ll_fill)		
...
this.Control[iCurrent+7]=this.cb_sketch
this.Control[iCurrent+8]=this.cb_copy_style
this.Control[iCurrent+9]=this.dw_7
this.Control[iCurrent+10]=this.dw_copy_style
this.Control[iCurrent+11]=this.dw_9
this.Control[iCurrent+12]=this.dw_10
this.Control[iCurrent+13]=this.cbx_pop
this.Control[iCurrent+14]=this.dw_spec
end on

on w_12010_e.destroy
call super::destroy
...
/* 수정일      : 2002.01.08                                                  */
/*===========================================================================*/
String     ls_style, ls_chno, ls_emp_nm, ls_dept, ls_fr_style, ls_fr_chno, ls_mat_cd, ls_cust_nm, ls_brand, ls_syear, ls_season
Boolean    lb_check 
DataStore  lds_Source
DataWindowChild ldw_child
		   ...
		   ls_style  = MidA(as_data, 1, 8)
			...
			IF ai_div = 1 THEN 	
				IF gf_style_chk(ls_style, ls_chno) THEN
					...
					if ls_brand = "B" or ls_brand = "P" or ls_brand = "K" or ls_brand = "L" or ls_brand = "H" or ls_brand = "U" or ls_brand = "G"  or ls_brand = "T"then
					else	
					ldw_child.setFilter( " sojae <> 'A' and sojae <> 'B' and sojae <> 'C' and sojae <> 'D' and sojae <> 'Y' and sojae <> 'O'  ");
					end if
...
//					messagebox("", left(ls_syear,4))				
				...
				ldw_child.Retrieve('%',ls_brand)
				ldw_child.SetRedraw(false)
				if ls_brand = "B" or ls_brand = "P" or ls_brand = "K" or ls_brand = "L" or ls_brand = "H" or ls_brand = "U"  or ls_brand = "G" then
				else	
				ldw_child.setFilter( " sojae <> 'A' and sojae <> 'B' and sojae <> 'C' and sojae <> 'D' and sojae <> 'Y' and sojae <> 'O'  ");
				end if
				ldw_child.Filter()
...
//					tab_1.tabpage4.dw_4.Setitem(al_row, "cust_nm", ls_cust_nm)
//					RETURN 0
//				END IF 
//			END IF
//		   gst_cd.ai_div          = ai_div
...
//				lds_Source = Message.PowerObjectParm
//				dw_4.SetRow(al_row)
//				dw_4.SetColumn(as_column)
//				dw_4.SetItem(al_row, "cust_cd",    lds_Source.GetItemString(1,"custcode"))
//				dw_4.SetItem(al_row, "cust_nm", lds_Source.GetItemString(1,"cust_sname"))
//				/* 다음컬럼으로 이동 */
//				dw_4.TriggerEvent(Editchanged!)
//				dw_4.SetColumn("dlvy_ymd")
//				ib_itemchanged = False 
//				lb_check = TRUE 
//			ELSE
//				lb_check = FALSE 
...
if MidA(is_style,1,1) = "U" or   MidA(is_style,1,1) = "R" then
	dw_master.object.t_11.text = "상품명"
else	
	dw_master.object.t_11.text = "원자재/상품 명"
end if
...
ELSEIF il_rows = 0 THEN 
 ...
 //  dw_master.Setitem(1, "cost_year",    is_year)	
   ...
   CASE 1		/* 조회 */
      if al_rows > 0 then
         ...
         dw_master.Enabled = true
			...
			tab_1.tabpage_4.cb_order.enabled = true
      end if
	CASE 4		/* 삭제 */
...
long i, ll_row_count, ll_size_spec1,ll_size_spec2,ll_size_spec3,ll_size_spec4
datetime ld_datetime
String   ls_mat_cd, ls_mat_nm, ls_mat_nm_chk, ls_ErrMsg, ls_flag, ls_mat_fg, ls_brand,ls_dsgn_emp, ls_cost_year
...
is_exp_ymd = dw_master.GetItemString(1, "exp_ymd")




ls_cost_year = dw_master.GetItemString(1, "cost_year")
	if ls_cost_year = '' or isnull(ls_cost_year) then 
		ls_cost_year = is_year
	end if	
	

if is_sojae = 'C' then //or is_sojae = 'D' or is_sojae = 'E' then 
	if ls_brand = "P" or ls_brand = "B" or ls_brand = "D" or ls_brand = "L" or ls_brand = "H" or ls_brand = "U"  or ls_brand = "V" or ls_brand = "S" or ls_brand = "K" or ls_brand = "F" or ls_brand = "T" then
	else	
...
elseif (ls_brand ='N' or ls_brand = 'O') and (is_item = 'Z' or  is_item = 'W') then 
...
elseif (is_item = 'A' or is_item = 'G' or is_item = 'F' ) AND ls_brand <> 'D' then 
				
		ll_size_spec1 = tab_1.tabpage_2.dw_2.getitemnumber(22,"size_spec_1")
		if isnull(ll_size_spec1) then ll_size_spec1 = 0 
		ll_size_spec2 = tab_1.tabpage_2.dw_2.getitemnumber(23,"size_spec_1")
		if isnull(ll_size_spec2) then ll_size_spec2 = 0 
		ll_size_spec3 = tab_1.tabpage_2.dw_2.getitemnumber(24,"size_spec_1")
		if isnull(ll_size_spec3) then ll_size_spec3 = 0 		
		ll_size_spec4 = tab_1.tabpage_2.dw_2.getitemnumber(25,"size_spec_1")		
		if isnull(ll_size_spec4) then ll_size_spec4 = 0 		
		
		
			if ls_brand = "S" or ls_brand = "B" or ls_brand = "Y" or ls_brand = "V" or ls_brand = "L" or ls_brand = 'G' or ls_brand = 'R' or ls_brand = 'H'  or ls_brand = 'P' or ls_brand = "T"  or ls_brand = "U"  or ls_brand = "W" or ls_brand = "F"  or ls_brand = "Q" then
			else	
...
IF dw_detail.RowCount() = 0 THEN    
	...
	dw_detail.Setitem(1, "cost_year",   ls_cost_year) 				
END IF 
...
dw_detail.Setitem(1, "exp_ymd",   is_exp_ymd) 
dw_detail.Setitem(1, "cost_year",   ls_cost_year) 	
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_12010_e","0")
end event
...
type cb_close from w_com010_e`cb_close within w_12010_e
integer taborder = 140
end type

type cb_delete from w_com010_e`cb_delete within w_12010_e
integer x = 919
integer taborder = 90
end type

type cb_insert from w_com010_e`cb_insert within w_12010_e
integer x = 576
integer taborder = 70
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_12010_e
end type

type cb_update from w_com010_e`cb_update within w_12010_e
integer taborder = 130
end type

type cb_print from w_com010_e`cb_print within w_12010_e
integer x = 1262
...
type cb_preview from w_com010_e`cb_preview within w_12010_e
integer x = 1605
...
type gb_button from w_com010_e`gb_button within w_12010_e
end type

type cb_excel from w_com010_e`cb_excel within w_12010_e
...
type dw_head from w_com010_e`dw_head within w_12010_e
integer width = 3561
integer height = 148
...
type ln_1 from w_com010_e`ln_1 within w_12010_e
integer beginy = 328
integer endy = 328
end type

type ln_2 from w_com010_e`ln_2 within w_12010_e
integer beginy = 332
integer endy = 332
end type

type dw_body from w_com010_e`dw_body within w_12010_e
boolean visible = false
integer x = 5431
integer y = 684
integer width = 489
...
type dw_print from w_com010_e`dw_print within w_12010_e
...
This.GetChild("cost_year", ldw_child)
ldw_child.SetTransObject(SQLCA)
...
event ue_auto_need();String  ls_color  , LS_MAT_CD
...
FOR i = 1 TO tab_1.tabpage_3.dw_3.RowCount()
    ...
    IF tab_1.tabpage_3.dw_3.GetItemStatus(i, 0, Primary!) = NewModified! THEN
	 ...
	 if LeftA(LS_MAT_CD ,1) <> "L" then
		 ldc_need_qty = Round(wf_Get_pcs(ls_color) * ldc_req_qty, 1)	 		 
		 ldc_t_exp_qty = Round(wf_Get_pcs_exp(ls_color) * ldc_req_qty, 1)
	else 
		 ldc_need_qty = Round(wf_Get_pcs(ls_color) * ldc_req_qty, 2)	 		 
		 ldc_t_exp_qty = Round(wf_Get_pcs_exp(ls_color) * ldc_req_qty, 2)
   end if	 
	 
	 /* 남은 잔량이 요척량보다 작을경우 Loss량으로 자동처리 */
	 IF ldc_bf_tneed + ldc_stock_qty - ldc_need_qty < ldc_req_qty and &
		 ldc_bf_tneed + ldc_stock_qty - ldc_need_qty > 0  AND LeftA(LS_MAT_CD,1) <> 'L'        THEN
...
CHOOSE CASE dwo.name
	...
	case "size" 
		...
		select dbo.SF_label_size_CHK(:is_sojae, :is_item, :data, :is_brand)
		into :ls_label_size_CHK
		from dual;
		
		if ls_label_size_CHK = "N" then
			messagebox("확인", "해당 소재,아이템,사이즈에 맞는 라벨용 신체치수(W_91022_E)를 입력하세요!")  
		end if	
...
CHOOSE CASE dwo.name
	CASE "mat_cd"
		...
		if gf_mat_nm(ls_mat_cd, ls_mat_nm) <> 0 then
					ls_mat_cd = data
		end if 	
...
ll_4 = tab_1.tabpage_4.dw_4.rowcount()

for i = 1 to dw_9.rowcount()
	
	ls_mat_cd = dw_9.getitemstring(i,"mat_cd")
	ll_row = tab_1.tabpage_4.dw_4.find("mat_cd = '" + ls_mat_cd + "'", 1, ll_4 )

	if ll_row = 0 then 
		tab_1.tabpage_4.dw_4.insertrow(0)
		ll_4= ll_4 + 1
		tab_1.tabpage_4.dw_4.setitem(ll_4,"style",dw_9.getitemstring(i,"style"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"chno",dw_9.getitemstring(i,"chno"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"mat_cd",dw_9.getitemstring(i,"mat_cd"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"mat_nm",dw_9.getitemstring(i,"mat_nm"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"mat_gubn",dw_9.getitemstring(i,"mat_gubn"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"color",dw_9.getitemstring(i,"color"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"mat_color",dw_9.getitemstring(i,"mat_color"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"mat_fg",dw_9.getitemstring(i,"mat_fg"))
		tab_1.tabpage_4.dw_4.setitem(ll_4,"spec",dw_9.getitemstring(i,"spec"))
		
		tab_1.tabpage_4.dw_4.setitem(ll_4, "req_qty",      1)
		
		ldc_need_qty = Round(wf_Get_pcs('XX'), 1)
		tab_1.tabpage_4.dw_4.setitem(ll_4, "need_qty",   ldc_need_qty)
		tab_1.tabpage_4.dw_4.setitem(ll_4, "t_need_qty", ldc_need_qty)

		setnull(ldc_need_qty)
		ldc_need_qty = Round(wf_Get_pcs_exp('XX'), 1)
		tab_1.tabpage_4.dw_4.setitem(ll_4, "t_exp_qty", ldc_need_qty)		
...
IF Isvalid(Message.PowerObjectParm) THEN
   ...
   ll_row = Parent.dw_4.insertRow(0)
	...
	if cbx_pop.checked = true then
		tab_1.tabpage_4.dw_4.Trigger Event ue_Popup("cust_cd", ll_row, MidA(is_style,1,1), 0)
		tab_1.tabpage_4.dw_4.Trigger Event ue_Popup("dlvy_ymd", ll_row, MidA(is_style,1,1), 0)	
	end if	
...
DO WHILE sqlca.sqlcode = 0 
	...
	IF ll_row = 0 THEN 
		ll_row = parent.dw_4.insertRow(0)
		parent.dw_4.Setitem(ll_row, "mat_fg",     ls_mat_type) 
		parent.dw_4.Setitem(ll_row, "color",      "XX") 
...
//			messagebox("ls_mat_item", ls_mat_item)
		   ...
		   gst_cd.ai_div          = ai_div
			...
			IF Trim(as_data) <> "" THEN
				gst_cd.Item_where = " (custcode LIKE '" + as_data + "%' or cust_name like '%" + as_data + "%') and isnull(smat_gubn,'%') like  '%" + ls_mat_item + "%'   " 				
//				gst_cd.Item_where = "custcode LIKE '%" + as_data + "%'"
			ELSE
				gst_cd.Item_where = ""
			END IF
...
CHOOSE CASE dwo.name
	...
	CASE "req_qty"
		...
		ldc_need_qty = Round(wf_Get_pcs(ls_color) * Dec(Data), 2)
		This.Setitem(row, "need_qty",   ldc_need_qty)
		This.Setitem(row, "t_need_qty", ldc_need_qty + ldc_loss)

		setnull(ldc_need_qty)
		ldc_need_qty = Round(wf_Get_pcs_exp(ls_color) * Dec(Data), 2)
	...
	CASE "loss_qty"
		...
		ldc_need_qty = Round(wf_Get_pcs(ls_color) * Dec(ldc_req_qty),2) 
		...
		ldc_need_qty_chn = Round(wf_Get_pcs_exp(ls_color) * Dec(ldc_req_qty),2)
		if ldc_need_qty <> 0 then 
...
messagebox("알림!", "메인차수(0차)가 있는 경우에만 적용 가능합니다!")
...
tab_1.tabpage_4.dw_4.Retrieve(ls_style, ls_chno, gl_user_level)
