$PBExportHeader$w_11003_e.srw
$PBExportComments$월판매형태별매출계획
forward
global type w_11003_e from w_com010_e
end type
type tab_1 from tab within w_11003_e
end type
...
type dw_detail from datawindow within w_11003_e
end type
type dw_master from datawindow within w_11003_e
end type
type em_1 from editmask within w_11003_e
end type
type em_2 from editmask within w_11003_e
end type
type em_3 from editmask within w_11003_e
end type
type em_4 from editmask within w_11003_e
end type
type cb_1 from commandbutton within w_11003_e
end type
type st_1 from statictext within w_11003_e
end type
end forward
...
ll_row = dw_master.Retrieve(is_brand, is_year, is_season,is_country_cd)
...
Decimal ldc_plan_amt, ldc_sale_collect, ldc_cost_amt
...
IF il_rows > 0 THEN
   ll_row_cnt = dw_body.retrieve(is_brand, is_year, is_season, ai_plan_seq, is_country_cd) 
   FOR i = 1 TO ll_row_cnt 
	   ...
	   ll_find = dw_detail.find("yymm = '" + ls_yymm + "'", 1, dw_detail.RowCount())
		IF ll_find > 0 THEN 
			...
			ldc_sale_collect = dw_body.GetitemDecimal(i, "sale_collect") / 1000
			ldc_cost_amt = dw_body.GetitemDecimal(i, "cost_amt") / 1000
			dw_detail.Setitem(ll_find, "plan_amt_" + ls_sale_div, ldc_plan_amt)
			dw_detail.Setitem(ll_find, "sale_collect_" + ls_sale_div, ldc_sale_collect)
			dw_detail.Setitem(ll_find, "cost_amt_" + ls_sale_div, ldc_cost_amt)
		END IF
   NEXT
   ...
   IF idw_status = DataModified! THEN		      /* Modify Record */
	   ...
	   FOR k = 1 TO 4 		 			      
			   ldc_plan_amt = dw_detail.GetitemDecimal(i, "plan_amt_" + String(k)) * 1000
				ldc_sale_collect = dw_detail.GetitemDecimal(i, "sale_collect_" + String(k)) * 1000
				ldc_cost_amt = dw_detail.GetitemDecimal(i, "cost_amt_" + String(k)) * 1000
	         ...
	         ll_row = dw_body.find(ls_Find, 1, dw_body.RowCount())
               ...
               dw_body.Setitem(ll_row, "plan_amt", ldc_plan_amt)
					dw_body.Setitem(ll_row, "sale_collect", ldc_sale_collect)
					dw_body.Setitem(ll_row, "cost_amt", ldc_cost_amt)
               ...
               dw_body.Setitem(ll_row, "reg_id", gs_user_id)
					dw_body.Setitem(ll_row, "country_cd", is_country_cd)
				END IF
		NEXT 
   END IF
NEXT
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified! THEN				/* New Record */
      ...
      dw_master.Setitem(i, "reg_id", gs_user_id)
		dw_master.Setitem(i, "country_cd", is_country_cd)
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_11003_e","0")
end event

type cb_close from w_com010_e`cb_close within w_11003_e
integer taborder = 170
end type

type cb_delete from w_com010_e`cb_delete within w_11003_e
boolean visible = false
integer taborder = 120
end type

type cb_insert from w_com010_e`cb_insert within w_11003_e
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_11003_e
end type

type cb_update from w_com010_e`cb_update within w_11003_e
integer taborder = 160
end type

type cb_print from w_com010_e`cb_print within w_11003_e
boolean visible = false
integer taborder = 130
end type

type cb_preview from w_com010_e`cb_preview within w_11003_e
boolean visible = false
integer taborder = 140
end type

type gb_button from w_com010_e`gb_button within w_11003_e
end type

type cb_excel from w_com010_e`cb_excel within w_11003_e
boolean visible = false
integer taborder = 150
end type

type dw_head from w_com010_e`dw_head within w_11003_e
integer x = 27
integer width = 2107
integer height = 160
string dataobject = "d_11003_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_11003_e
integer beginy = 352
integer endy = 352
end type

type ln_2 from w_com010_e`ln_2 within w_11003_e
integer beginy = 356
integer endy = 356
end type

type dw_body from w_com010_e`dw_body within w_11003_e
boolean visible = false
integer x = 2171
integer y = 168
integer width = 1344
integer height = 168
string dataobject = "d_11003_d03"
end type

type dw_print from w_com010_e`dw_print within w_11003_e
end type
...
IF ib_changed THEN 
  	CHOOSE CASE gf_update_yn(Parent.title)
		CASE 1
			...
			END IF		
		CASE 2 
			dw_master.Retrieve(is_brand, is_year, is_season,is_country_cd)
