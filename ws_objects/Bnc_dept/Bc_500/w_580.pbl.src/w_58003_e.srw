$PBExportHeader$w_58003_e.srw
$PBExportComments$수출Invoice등록
forward
global type w_58003_e from w_com010_e
end type
type dw_1 from datawindow within w_58003_e
end type
type dw_detail from datawindow within w_58003_e
end type
end forward
...
global w_58003_e w_58003_e

type variables
string is_brand, is_invoice_date, is_invoice_no, is_foreign_currency
decimal id_detail_rows
Datawindowchild  idw_brand, idw_country_cd, idw_foreign_currency
end variables
...
string	ls_Style,ls_chno, ls_color, ls_color_nm, ls_item_nm,ls_Composition, ls_size
...
decimal	ld_sale_qty,ld_Exchange_price,ld_exchange_amt,ld_sale_amt, ld_tot_sale_amt
			...
			FOR	i =  1 to ld_row_count
				   ls_Style  			= dw_1.GetitemString(i,"style")
					...
					dw_detail.Setitem(i,"size",ls_size)
...
/* 수정일      : 2001..                                                  		*/
/*========================================================================-===*/
string 	ls_out_from_date, ls_out_to_date, ls_country_cd, ls_shop_cd, ls_tel_no
...
ELSE
	...
	choose case is_brand 
		case 'N'
			ls_tel_no = '82-2-2225-0327'
		case 'W'
			ls_tel_no = '82-2-2225-0186'			
		case 'A'
			ls_tel_no = '82-2-2225-0164'			
		case else
			ls_tel_no = '82-2-2225-0186'							
	end choose
	dw_body.Setitem(1, "tel_no", ls_tel_no)
...
/* 수정일      : 2001..                                                  */
/*===========================================================================*/
long 		i, ll_row_count, ll_sale_qty
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified! THEN				/* New Record */
      dw_body.Setitem(i, "reg_id", gs_user_id)
		dw_body.SetItemStatus(i, "tel_no", Primary!, NewModified!)
...
FOR i=1 TO ll_row_count
   idw_status = dw_detail.GetItemStatus(i, 0, Primary!)
	ll_sale_qty = dw_detail.getitemnumber(i,"sale_qty")
...
type cb_close from w_com010_e`cb_close within w_58003_e
end type

type cb_delete from w_com010_e`cb_delete within w_58003_e
end type

type cb_insert from w_com010_e`cb_insert within w_58003_e
boolean visible = false
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_58003_e
end type

type cb_update from w_com010_e`cb_update within w_58003_e
end type

type cb_print from w_com010_e`cb_print within w_58003_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_58003_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_58003_e
end type

type cb_excel from w_com010_e`cb_excel within w_58003_e
boolean visible = false
end type

type dw_head from w_com010_e`dw_head within w_58003_e
integer height = 120
string dataobject = "d_58003_h01"
end type
...
This.GetChild("foreign_currency", idw_foreign_currency)
idw_foreign_currency.SetTransObject(SQLCA)
idw_foreign_currency.Retrieve('013')

end event

type ln_1 from w_com010_e`ln_1 within w_58003_e
integer beginy = 292
integer endy = 292
end type

type ln_2 from w_com010_e`ln_2 within w_58003_e
integer beginy = 296
integer endy = 296
end type

type dw_body from w_com010_e`dw_body within w_58003_e
integer y = 308
integer height = 1280
string dataobject = "d_58003_d01"
...
ls_filter_str = "inter_cd in ('01','02','06','03') " 
ldw_foreign_currency.SetFilter(ls_filter_str)
ldw_foreign_currency.Filter( )
...
event dw_body::itemchanged;call super::itemchanged;decimal ld_exchange_rate

CHOOSE CASE dwo.name
	CASE "foreign_currency" 
	     
	    	select case when :data = '02' then usd_c 
			 	         when :data = '06' then euro  
			 	         when :data = '03' then jpy  							 
 			 	         when :data = '01' then 1
						else 	usd_c end	 
				into :ld_exchange_rate
			from tb_91080_c (nolock) where yymmdd = :is_invoice_date;
...
type dw_print from w_com010_e`dw_print within w_58003_e
integer x = 1979
integer y = 388
end type
...
event ue_exchange_refresh(long al_exchange_amt, long al_row);decimal ldc_exchange_amt, ldc_exchange_price, ldc_sale_qty, ldc_exchange_rate
...
IF IS_BRAND = "D" OR IS_BRAND = "O" OR IS_BRAND = "A" OR IS_BRAND = "Y" THEN
...
CHOOSE CASE dwo.name
	CASE "b_delete" 					
		...
		il_rows = dw_detail.DeleteRow(this.getrow())
	...
	CASE "b_add" 					
		...
		if il_rows > 0 then
			...
			select right('0000'+ rtrim(convert(char(4),convert(int,isnull((
				select max(seq_no) from TB_56201_H (nolock) 
				where Brand = :ls_brand
				and   invoice_date = :ls_Invoice_Date
				and   invoice_no   = :ls_Invoice_No),'0000'))+:il_rows)),4)
				into :ls_seq_no
			from dual;
