$PBExportHeader$w_52021_e.srw
$PBExportComments$동일디자인배분
forward
global type w_52021_e from w_com010_e
end type
type dw_assort from datawindow within w_52021_e
end type
type dw_db from datawindow within w_52021_e
end type
type dw_temp from datawindow within w_52021_e
end type
type st_remark from statictext within w_52021_e
end type
type dw_order from datawindow within w_52021_e
end type
type st_trend1 from statictext within w_52021_e
end type
type st_trend2 from statictext within w_52021_e
end type
end forward
...
global w_52021_e w_52021_e

type variables
DataWindowChild idw_fr_color, idw_to_color , idw_shop_lv, idw_area_cd, idw_shop_lv1, idw_house_cd, idw_brand

String  is_fr_style, is_fr_chno, is_fr_color, is_save_opt, is_brand,is_DW_sort
String  is_to_style, is_to_chno, is_to_color, is_yymmdd , is_house_cd, is_DIST_GRP
Long    il_deal_seq
Boolean ib_NewDeal
...
if IsNull(is_brand) or Trim(is_brand) = "" then
   MessageBox(ls_title,"대상매장 브랜드 코드를 입력하십시요!")
   dw_head.SetFocus()
...
/* 기준 품번 판매내역 처리 */
il_rows = dw_temp.retrieve(is_fr_style,"%", is_fr_color, il_deal_seq, is_brand, is_dist_grp)

if is_save_opt = "O" then
		IF il_rows > 0 THEN 
			IF wf_temp_set() THEN 
				
//				MESSAGEBOX( "is_to_style", is_to_style) 
//				MESSAGEBOX( "is_to_chno", is_to_chno) 
//				MESSAGEBOX( "is_to_color", is_to_color) 				
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_52021_e","0")
end event
...
type cb_close from w_com010_e`cb_close within w_52021_e
end type

type cb_delete from w_com010_e`cb_delete within w_52021_e
end type

type cb_insert from w_com010_e`cb_insert within w_52021_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_52021_e
end type

type cb_update from w_com010_e`cb_update within w_52021_e
end type

type cb_print from w_com010_e`cb_print within w_52021_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_52021_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_52021_e
end type

type cb_excel from w_com010_e`cb_excel within w_52021_e
end type

type dw_head from w_com010_e`dw_head within w_52021_e
integer x = 9
integer y = 264
integer width = 3589
integer height = 292
string dataobject = "d_52021_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_52021_e
integer beginy = 568
integer endy = 568
end type

type ln_2 from w_com010_e`ln_2 within w_52021_e
integer beginy = 572
integer endy = 572
end type

type dw_body from w_com010_e`dw_body within w_52021_e
integer y = 588
integer height = 1464
string dataobject = "d_52021_d01"
...
type dw_print from w_com010_e`dw_print within w_52021_e
end type
