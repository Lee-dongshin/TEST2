$PBExportHeader$w_52013_test.srw
$PBExportComments$Order배분출고의뢰
forward
global type w_52013_test from w_com010_e
end type
type dw_assort from datawindow within w_52013_test
end type
type dw_db from datawindow within w_52013_test
end type
type dw_temp from datawindow within w_52013_test
end type
type st_remark from statictext within w_52013_test
end type
end forward
...
global w_52013_test w_52013_test
      ...
      FOR k = 1 TO ll_assort_cnt 
			...
			IF dw_body.GetItemStatus(i, "deal_qty_" + String(k), Primary!) = DataModified! THEN 
               ...
               dw_db.Setitem(ll_find, "reg_id",   gs_user_id)
				...
				END IF
            /* 예약 재고량 처리 */
	         //IF gf_stresv_update (is_style,    is_chno,     ls_color, ls_size, ls_shop_div, ll_resv_qty, ls_ErrMsg) = FALSE THEN 
				IF gf_stresv_update (is_style,    is_chno,     ls_color, ls_size, ls_shop_div, ll_resv_qty, 'X', ls_ErrMsg) = FALSE THEN 		//Mig 에러 : 매개변수 as_house 위치에 'X' 추가
		         il_rows = -1
        			EXIT
	         END IF
			END IF
...
type cb_close from w_com010_e`cb_close within w_52013_test
end type

type cb_delete from w_com010_e`cb_delete within w_52013_test
end type

type cb_insert from w_com010_e`cb_insert within w_52013_test
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_52013_test
end type

type cb_update from w_com010_e`cb_update within w_52013_test
end type

type cb_print from w_com010_e`cb_print within w_52013_test
boolean visible = false
integer x = 1422
integer y = 56
end type

type cb_preview from w_com010_e`cb_preview within w_52013_test
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_52013_test
end type

type cb_excel from w_com010_e`cb_excel within w_52013_test
end type

type dw_head from w_com010_e`dw_head within w_52013_test
integer y = 268
integer height = 276
...
CHOOSE CASE dwo.name
	CASE "color"
		...
		idw_color.Retrieve(left(ls_style_no, 8), right(ls_style_no, 1))
		idw_color.insertRow(1)
		idw_color.Setitem(1, "color", "%")
...
type ln_1 from w_com010_e`ln_1 within w_52013_test
integer beginy = 544
integer endy = 544
end type

type ln_2 from w_com010_e`ln_2 within w_52013_test
integer beginy = 548
integer endy = 548
end type

type dw_body from w_com010_e`dw_body within w_52013_test
integer y = 560
integer height = 1480
...
type dw_print from w_com010_e`dw_print within w_52013_test
end type
