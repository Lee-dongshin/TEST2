$PBExportHeader$w_52030_e.srw
$PBExportComments$동일디자인배분(상설)
forward
global type w_52030_e from w_com010_e
end type
type dw_assort from datawindow within w_52030_e
end type
type dw_db from datawindow within w_52030_e
end type
type dw_temp from datawindow within w_52030_e
end type
type st_remark from statictext within w_52030_e
end type
type dw_order from datawindow within w_52030_e
end type
type st_trend1 from statictext within w_52030_e
end type
type st_trend2 from statictext within w_52030_e
end type
end forward
...
global w_52030_e w_52030_e

type variables
DataWindowChild idw_fr_color, idw_to_color , idw_shop_lv, idw_area_cd, idw_shop_lv1, idw_house_cd, idw_shop_type

String  is_fr_style, is_fr_chno, is_fr_color, is_save_opt,is_DW_sort
String  is_to_style, is_to_chno, is_to_color, is_yymmdd , is_house_cd, is_shop_type
Long    il_deal_seq
Boolean ib_NewDeal
...
FOR i = 1 TO ll_row_cnt 
   ls_shop_cd =  dw_order.object.shop_cd[i] 
	...
	IF ll_row < 1 THEN
      ...
      dw_body.Setitem(ll_row, "shop_lv1_1",   dw_order.object.tb_91100_m_shop_lv1[i])						
	END IF 
...
if IsNull(is_house_cd) or Trim(is_house_cd) = "" then
   MessageBox(ls_title,"출고창고를 선택하십시요!")
   dw_head.SetFocus()
...
/* 수정일      : 2002.01.08                                                  */
/*===========================================================================*/
String     ls_style, ls_chno, ls_color, ls_trend, ls_country, ls_search
Boolean    lb_check 
				   ...
				   dw_head.SetColumn("to_color")
					...
					if isnull(ls_style) or ls_style = '' then 
						...
						if LenA(ls_style) >= 8 then  
							gf_style_color_size_pic(ls_style, '%','%','0','K')
						end if
					end if							
				END IF
...
/* 기준 품번 판매내역 처리 */
il_rows = dw_temp.retrieve(is_fr_style,"%", is_fr_color, il_deal_seq)
				...
				FOR k = 1 TO ll_assort_cnt 
					IF dw_body.GetItemStatus(i, "deal_qty_" + String(k), Primary!) = DataModified! OR  dw_body.GetItemStatus(i, "deal_qty_" + String(k), Primary!) = NewModified! THEN 
						...
						IF ll_find > 0 THEN
							...
							dw_db.Setitem(ll_find, "mod_id",   gs_user_id)
							dw_db.Setitem(ll_find, "mod_dt",   ld_datetime)
							dw_db.Setitem(ll_find, "house_cd", is_house_cd)														
						ELSE
							ll_find = dw_db.insertRow(0)
							dw_db.Setitem(ll_find, "out_ymd",  is_yymmdd)
							dw_db.Setitem(ll_find, "deal_seq", il_deal_seq)
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_52030_e","0")
end event

event open;call super::open;dw_head.SetItem(1, "house_cd"   ,"010000")
end event

type cb_close from w_com010_e`cb_close within w_52030_e
end type

type cb_delete from w_com010_e`cb_delete within w_52030_e
end type

type cb_insert from w_com010_e`cb_insert within w_52030_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_52030_e
end type

type cb_update from w_com010_e`cb_update within w_52030_e
end type

type cb_print from w_com010_e`cb_print within w_52030_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_52030_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_52030_e
end type

type cb_excel from w_com010_e`cb_excel within w_52030_e
end type

type dw_head from w_com010_e`dw_head within w_52030_e
integer x = 9
integer y = 264
integer width = 3589
integer height = 212
string dataobject = "d_52030_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_52030_e
integer beginy = 480
integer endy = 480
end type

type ln_2 from w_com010_e`ln_2 within w_52030_e
integer beginy = 484
integer endy = 484
end type

type dw_body from w_com010_e`dw_body within w_52030_e
integer y = 496
integer height = 1556
string dataobject = "d_52030_d01"
...
type dw_print from w_com010_e`dw_print within w_52030_e
end type
