$PBExportHeader$w_52006_e.srw
$PBExportComments$판매분통제스타일
forward
global type w_52006_e from w_com010_e
end type
end forward

global type w_52006_e from w_com010_e
end type
global w_52006_e w_52006_e
...
CHOOSE CASE as_column
			...
			CASE "style"							// 거래처 코드
				...
				IF Isvalid(Message.PowerObjectParm) THEN
					...
					dw_body.SetItem(al_row, "brand", is_brand)					
...
//MESSAGEBOX("", is_brand + "/" + is_year + "/" + is_season)
...
FOR i=1 TO ll_row_count
   idw_status = dw_body.GetItemStatus(i, 0, Primary!)
	...
	ls_shop_brand = dw_body.GetItemString(i, "shop_brand")
	if IsNull(ls_shop_brand) or Trim(ls_shop_brand) = "" then
		MessageBox("확인!","통제매장 브랜드를 입력하십시요!")
		dw_body.SetFocus()
		dw_body.SetColumn("shop_brand")
		return 0
	end if	
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_52006_e","0")
end event

type cb_close from w_com010_e`cb_close within w_52006_e
end type

type cb_delete from w_com010_e`cb_delete within w_52006_e
end type

type cb_insert from w_com010_e`cb_insert within w_52006_e
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_52006_e
end type

type cb_update from w_com010_e`cb_update within w_52006_e
end type

type cb_print from w_com010_e`cb_print within w_52006_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_52006_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_52006_e
end type

type cb_excel from w_com010_e`cb_excel within w_52006_e
boolean visible = false
end type

type dw_head from w_com010_e`dw_head within w_52006_e
integer y = 224
integer height = 156
string dataobject = "d_52006_h01"
end type
...
CHOOSE CASE dwo.name
	...
	//   messagebox(ls_brand, ls_year)
		this.getchild("season",ldw_season1)
...
type ln_1 from w_com010_e`ln_1 within w_52006_e
end type

type ln_2 from w_com010_e`ln_2 within w_52006_e
end type

type dw_body from w_com010_e`dw_body within w_52006_e
string dataobject = "d_52006_d01"
end type
...
type dw_print from w_com010_e`dw_print within w_52006_e
end type
