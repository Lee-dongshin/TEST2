$PBExportHeader$w_52027_e.srw
$PBExportComments$잔량배분(행사용)
forward
global type w_52027_e from w_com020_e
end type
type st_remark from statictext within w_52027_e
end type
type dw_assort from datawindow within w_52027_e
end type
type dw_db from datawindow within w_52027_e
end type
type dw_temp from datawindow within w_52027_e
end type
type cb_copy from commandbutton within w_52027_e
end type
type cb_reset from commandbutton within w_52027_e
end type
type dw_reset from datawindow within w_52027_e
end type
type st_info from statictext within w_52027_e
end type
type dw_detail from datawindow within w_52027_e
end type
type rb_in from radiobutton within w_52027_e
end type
type rb_rt from radiobutton within w_52027_e
end type
type rb_dir from radiobutton within w_52027_e
end type
end forward
...
global w_52027_e w_52027_e

type variables
String  is_brand,  is_year,  is_season, is_sojae, is_item, is_stock_opt, is_chno_opt
String  is_yymmdd, is_style, is_chno,   is_color, is_color_nm , is_house_cd, is_disc_opt, is_online
Long    il_deal_seq 
Boolean ib_NewDeal
DataStore  ids_copy
...
this.Control[iCurrent+7]=this.dw_reset
this.Control[iCurrent+8]=this.st_info
this.Control[iCurrent+9]=this.dw_detail
this.Control[iCurrent+10]=this.rb_in
this.Control[iCurrent+11]=this.rb_rt
this.Control[iCurrent+12]=this.rb_dir
end on

on w_52027_e.destroy
call super::destroy
...
if IsNull(is_stock_opt) or Trim(is_stock_opt) = "" then
   MessageBox(ls_title,"재고 확인 여부를 입력하십시요!")
   dw_head.SetFocus()
...
is_online = dw_head.GetItemString(1, "online")
if IsNull(is_online) or Trim(is_online) = "" then
   MessageBox(ls_title,"온라인 여부를 입력하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("online")
   return false
end if

if is_online = "Y" then
	messagebox("알림!", "부진온라인 정상 판매 진행이 선택되어 대상인 롯데매장만 조회 됩니다!")
end if	


is_chno_opt = dw_head.GetItemString(1, "chno_opt")
if IsNull(is_chno_opt) or Trim(is_chno_opt) = "" then
   MessageBox(ls_title,"차수무시를 입력하십시요!")
   dw_head.SetFocus()
...
if is_brand = "Y" then 
	if il_deal_seq < 249  then 
		MessageBox(ls_title,"용도에 맞는 배분 번호가 아닙니다. 250번 이후 차수를 사용하세요! ")
		dw_head.SetFocus()
		dw_head.SetColumn("deal_seq")
		return false
	end if
else
		if il_deal_seq < 90 or  il_deal_seq > 100  then 
		MessageBox(ls_title,"용도에 맞는 배분 번호가 아닙니다. 90-100번대 차수를 사용하세요! ")
		dw_head.SetFocus()
		dw_head.SetColumn("deal_seq")
		return false
	end if
...
String   ls_shop_cd, ls_size, ls_find, ls_shop_div, ls_ErrMsg, ls_deal_fg
      ...
      FOR k = 1 TO ll_assort_cnt 
			IF dw_body.GetItemStatus(i, "deal_qty_" + String(k), Primary!) = DataModified! THEN 
               ...
               dw_db.Setitem(ll_find, "deal_seq", il_deal_seq)
					dw_db.Setitem(ll_find, "deal_fg",  ls_deal_fg) 						
...
ids_copy.DataObject = "d_52027_d01"
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_52027_e","0")
end event
...
type cb_close from w_com020_e`cb_close within w_52027_e
integer x = 3374
integer width = 215
end type

type cb_delete from w_com020_e`cb_delete within w_52027_e
integer x = 2606
integer width = 224
boolean enabled = true
end type

type cb_insert from w_com020_e`cb_insert within w_52027_e
integer x = 2066
integer width = 215
boolean enabled = true
string text = "복사(&C)"
...
type cb_retrieve from w_com020_e`cb_retrieve within w_52027_e
integer x = 3168
integer width = 210
end type

type cb_update from w_com020_e`cb_update within w_52027_e
end type

type cb_print from w_com020_e`cb_print within w_52027_e
...
type cb_preview from w_com020_e`cb_preview within w_52027_e
boolean visible = false
integer x = 2830
integer width = 334
boolean enabled = true
...
type gb_button from w_com020_e`gb_button within w_52027_e
end type

type cb_excel from w_com020_e`cb_excel within w_52027_e
integer x = 1778
integer width = 288
end type

type dw_head from w_com020_e`dw_head within w_52027_e
integer height = 204
string dataobject = "d_52027_h01"
end type
...
type ln_1 from w_com020_e`ln_1 within w_52027_e
integer beginy = 372
integer endy = 372
end type

type ln_2 from w_com020_e`ln_2 within w_52027_e
integer beginy = 376
integer endy = 376
end type

type dw_list from w_com020_e`dw_list within w_52027_e
integer x = 0
integer y = 556
integer width = 1061
integer height = 1464
string title = "품번별 가용재고 List"
string dataobject = "d_52027_d02"
end type
...
il_rows = dw_temp.retrieve(is_style, ls_chno, is_color, il_deal_seq, is_online)
IF il_rows > 0 THEN 
	wf_temp_set() 
     ll_row = dw_db.Retrieve(is_yymmdd, il_deal_seq, is_style, is_chno, is_color, '7')
...
event dw_list::doubleclicked;call super::doubleclicked;String 	ls_search, ls_color, ls_chno, ls_style
if row > 0 then 
	choose case dwo.name
		case 'style','style_no'
...
type dw_body from w_com020_e`dw_body within w_52027_e
integer x = 1070
integer y = 388
integer width = 2546
integer height = 1632
string dataobject = "d_52027_d01"
...
event dw_body::constructor;call super::constructor;datawindowchild ldw_shop_lv
This.GetChild("shop_lv", ldw_shop_lv)
ldw_shop_lv.SetTransObject(SQLCA)
ldw_shop_lv.Retrieve('093')
ldw_shop_lv.InsertRow(1)
ldw_shop_lv.SetItem(1, "inter_cd", '%')
ldw_shop_lv.SetItem(1, "inter_nm", '전체')

end event

type st_1 from w_com020_e`st_1 within w_52027_e
integer x = 1061
integer y = 472
integer height = 1532
end type

type dw_print from w_com020_e`dw_print within w_52027_e
integer x = 1477
integer y = 1140
string dataobject = "d_52014_r01"
end type
