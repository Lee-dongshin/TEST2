...
type variables
DataWindowChild idw_color , idw_brand, idw_house_cd
String  is_style, is_chno, is_color, is_yymmdd, is_brand, is_house_cd, is_dist_grp
Long    il_deal_seq 
Boolean ib_NewDeal
end variables
...
public function boolean wf_deal ();Long i, k, ll_row_cnt, ll_assort_cnt ,li_rtrn
...
dw_body.SetRedraw(False) 
 ...
 li_rtrn =  messagebox("확인!", "해당제품의 재고가 배분수량과 다릅니다. 그래도 작업하시겠습니까?" ,Question!,OKCancel!)
	if li_rtrn = 1 then //네
...
if IsNull(is_house_cd) or Trim(is_house_cd) = "" then
   MessageBox(ls_title,"출고 창고코드를 입력하십시요!")
   dw_head.SetFocus()
...
if IsNull(is_dist_grp) or Trim(is_dist_grp) = "" then
   MessageBox(ls_title,"배분그룹지정 코드를 입력하십시요!")
   dw_head.SetFocus()
...
il_rows = dw_temp.retrieve(is_style, is_chno, is_color, is_brand, is_dist_grp)
IF il_rows > 0 THEN 
	wf_temp_set() 
...
FOR k = 1 TO dw_ASSORT.RowCount()
	 ...
	 IF ll_deal_qty > dw_assort.Object.stock_qty[k] THEN 
		 MessageBox("알림", "[" + dw_assort.GetitemString(k, "color") + "/" + & 
      ...
      FOR k = 1 TO ll_assort_cnt 
            ...
            /* 예약 재고량 처리 */
	         IF gf_stresv_update (is_style,    is_chno,     ls_color, ls_size, &
				                     ls_shop_div, ll_resv_qty, "010000", ls_ErrMsg) = FALSE THEN 
		         il_rows = -1
        			EXIT
	         END IF
			END IF
