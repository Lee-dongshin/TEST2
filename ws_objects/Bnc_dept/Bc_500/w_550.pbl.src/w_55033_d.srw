...
//	end if	
	...
	if rb_item.checked = true then
		if is_gubun <> "0" then  // 조건 전체 수량 금액등
			dw_body.DataObject = "d_55033_d11" // 수량 금액 등
		else
			if is_saip_yn = "Y" then // 전체 and is_ps_chn = "K" 
   ...
   elseif rb_style.checked = true then
		if is_gubun <> "0" then
			...
			if is_saip_yn = "Y" then //  and is_ps_chn = "K" then
   ...
   elseif rb_color.checked = true then
		if is_gubun <> "0" then
			...
			if is_saip_yn = "Y" then // 전체and is_ps_chn = "K" then
   ...
   elseif rb_size.checked = true	 then
		if is_gubun <> "0" then
			...
			if is_saip_yn = "Y" then // 전체 and is_ps_chn = "K" then
...
//
			...
			if is_saip_yn = "Y" then //  and is_ps_chn = "K" then
				...
				il_rows = dw_body.retrieve(is_fr_ymd, is_yymmdd, is_brand, is_sojae, is_year, is_season, &
									is_item, is_dep_fg, is_gubun, is_chno_gubun, is_style_no, is_ps_chn, is_ps_except, is_plan_dc, is_ps_reord, is_plan_except, is_cost_season)
			end if	
