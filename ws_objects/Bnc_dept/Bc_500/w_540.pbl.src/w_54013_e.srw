...
type variables
DataWindowChild idw_brand, idw_year, idw_season, idw_item, idw_shop_div,idw_to_shop_brand
...
string is_ord_area, is_shop_div, is_sort, is_size1, is_size2, is_dep_gubn, is_sort_fr, is_sort_to, is_sort_style
string is_fr_shop_cd, is_to_shop_cd, IS_TO_SHOP_BRAND, IS_DW_SORT
...
long			ll_cur_row
string ls_item, ls_proc_yn,ls_move_yn
...
if dw_10.visible = true then
	...
	ls_move_yn = dw_10.GetItemString (ll_cur_row, "move_yn")	
	
	if IsNull(ls_move_yn) or Trim(ls_move_yn) = "" then
      ls_move_yn = "N"
	end if
	
	if ls_move_yn <> "Y" then
...
//messagebox("ls_move_yn", ls_move_yn)
	ls_move_yn = dw_5.GetItemString (ll_cur_row, "move_yn")	
...
//dw_8.settrans(SQLCA)
//dw_9.settrans(SQLCA)
//dw_10.settrans(SQLCA)
//dw_11.settrans(SQLCA)
//dw_12.settrans(SQLCA)
...
is_to_shop_brand = dw_head.GetItemString(1, "to_shop_brand")
if IsNull(is_to_shop_brand) or Trim(is_to_shop_brand) = "" then
   MessageBox(ls_title,"브랜드 코드를 입력하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("to_shop_brand")
   return false
end if
...
if dw_body.visible = true then
	...
	il_rows = dw_body.retrieve(is_brand, is_year, is_season, is_item, is_opt_between, is_opt_chno, is_shop_div,is_dep_gubn,is_style_rt)
	IF il_rows > 0 THEN
...
//messagebox("",	is_fr_shop_cd)
//messagebox("",	is_to_shop_cd)

	il_rows = dw_5.retrieve(is_yymmdd, is_brand, is_style_rt, is_proc_yn, is_shop_div,is_dep_gubn, is_fr_shop_cd, is_to_shop_cd)
	IF il_rows > 0 THEN
...
event type integer ue_popup(string as_column, long al_row, string as_data, integer ai_div);call super::ue_popup;String     ls_style, ls_chno, ls_shop_nm
Boolean    lb_check 
...
if dw_4.visible = true then
	...
	ls_move_yn = dw_4.GetItemString (ll_cur_row, "move_yn")	
...
This.GetChild("to_shop_brand", idw_to_shop_brand )
idw_to_shop_brand.SetTransObject(SQLCA)
idw_to_shop_brand.Retrieve('001')
idw_to_shop_brand.insertrow(1)
idw_to_shop_brand.setitem(1, "inter_cd", "%")
idw_to_shop_brand.setitem(1, "inter_nm", "보끄레전체")
...
ls_Type = ls_Type + "inter_cd = 'G' or inter_cd = 'K' or inter_cd = 'M'  or inter_cd = 'O' or inter_cd = 'I' "
...
//IF dw_head.Enabled = TRUE THEN Return 1
  	...
  	CHOOSE CASE index
		CASE 1
			...
			dw_6.visible = false			
			dw_7.visible = false			
			dw_head.object.t_fr_shop_cd.visible = false
			dw_head.object.fr_shop_cd.visible = false
			dw_head.object.cb_fr_shop_cd.visible = false			
			dw_head.object.fr_shop_nm.visible = false			
			dw_head.object.t_to_shop_cd.visible = false
			dw_head.object.to_shop_cd.visible = false
			dw_head.object.cb_to_shop_cd.visible = false		
			dw_head.object.to_shop_nm.visible = false						
      ...
      CASE 3
		...
		CASE 4			
			...
			dw_7.visible = false						
			is_proc_yn = "N"
			dw_head.object.t_fr_shop_cd.visible = true
			dw_head.object.fr_shop_cd.visible = true
			dw_head.object.cb_fr_shop_cd.visible = true			
			dw_head.object.fr_shop_nm.visible = true			
			dw_head.object.t_to_shop_cd.visible = true
			dw_head.object.to_shop_cd.visible = true
			dw_head.object.cb_to_shop_cd.visible = true		
			dw_head.object.to_shop_nm.visible = true						
...
//dw_11.visible = true
	...
	if ll_row2 > 0 then
	   is_size1 = dw_11.getitemstring(1 + ii, "size")
		...
		ll_row1  = dw_4.retrieve(is_yymmdd,  is_style, is_color, is_size1,is_shop_div,is_brand,is_to_shop_brand)
	if ll_row1 > 0 then
			li_rtrn =  1 //messagebox("경고!", "이미 처리된 스타일입니다! 작업을진행 하시겠습니까?",Question!,OKCancel! )	
			if li_rtrn = 1 then
//				dw_2.reset()
//				dw_3.reset()				
				dw_2.title = "사이즈-" + is_size1
				il_rows = dw_2.retrieve(is_brand, is_year, is_season, is_item, is_style, is_color, is_size1, is_opt_between, is_opt_chno, "B", is_ord_area, is_shop_div, il_avr_rate, is_yymmdd, is_shop_type,is_to_shop_brand)
				ll_rows = dw_3.retrieve(is_brand, is_year, is_season, is_item, is_style, is_color, is_size1, is_opt_between, is_opt_chno, "A", is_ord_area, is_shop_div, il_avr_rate, is_yymmdd, is_shop_type,is_to_shop_brand)
			else
				il_rows = 0
			end if
	else		
...
//		dw_10.reset()
			...
			ll_row1  = dw_10.retrieve(is_yymmdd,  is_style, is_color, is_size2,is_shop_div,is_brand,is_to_shop_brand)
		if ll_row1 > 0 then
				li_rtrn = 1 // messagebox("경고!", "이미 처리된 스타일입니다! 작업을진행 하시겠습니까?",Question!,OKCancel! )	
				if li_rtrn = 1 then
//					dw_8.reset()
//					dw_9.reset()		
					...
					ll_row3 = dw_8.retrieve(is_brand, is_year, is_season, is_item, is_style, is_color, is_size2, is_opt_between, is_opt_chno, "B", is_ord_area, is_shop_div, il_avr_rate, is_yymmdd, is_shop_type,is_to_shop_brand)
					ll_row4 = dw_9.retrieve(is_brand, is_year, is_season, is_item, is_style, is_color, is_size2, is_opt_between, is_opt_chno, "A", is_ord_area, is_shop_div, il_avr_rate, is_yymmdd, is_shop_type,is_to_shop_brand)
				else
					il_rows = 0
				end if
		else		
...
event constructor;
	...
	this.SetSort("shop_cd A, style A, chno A, color A, size A")
	this.Sort()
end event

event clicked;string ls_dwo
...
CHOOSE CASE dwo.name
	...
	CASE "fr_shop_cd_t"
		   ...
		   if is_sort_fr = "O" then
				dw_5.SetSort("fr_shop_cd A")
				dw_5.SetRedraw(false)
				dw_5.Sort()
				dw_5.SetRedraw(true)
				is_sort_fr = "A"
			elseif is_sort_fr = "A" then	
				dw_5.SetSort("fr_shop_cd d")
				dw_5.SetRedraw(false)
				dw_5.Sort()
				dw_5.SetRedraw(true)
				is_sort_fr = "d"
			...
			dw_5.object.l_1.visible = false
			dw_5.object.l_2.visible = false			
//			dw_5.GroupCalc()//
			
	CASE "to_shop_cd_t"
//			dw_5.SetSort("to_shop_cd A")
//			dw_5.Sort()
			
 		  if is_sort_to = "O" then
				dw_5.SetSort("to_shop_cd A")
				dw_5.SetRedraw(false)
				dw_5.Sort()
				dw_5.SetRedraw(true)
				is_sort_to = "A"
			elseif is_sort_to = "A" then	
				dw_5.SetSort("to_shop_cd d")
				dw_5.SetRedraw(false)
				dw_5.Sort()
				dw_5.SetRedraw(true)
				is_sort_to = "d"
...
//			dw_5.GroupCalc() 
	...
	CASE "style_t"
//			dw_5.SetSort("style A, color A, size A ")
//			dw_5.Sort()
			
 		  if is_sort_style = "O" then
				dw_5.SetSort("style A, color A, size A ")
				dw_5.SetRedraw(false)
				dw_5.Sort()
				dw_5.SetRedraw(true)
				is_sort_style = "A"
			elseif is_sort_style = "A" then	
				dw_5.SetSort("style d, color d, size d ")
				dw_5.SetRedraw(false)
				dw_5.Sort()
				dw_5.SetRedraw(true)
				is_sort_style = "d"
...
event doubleclicked;string ls_shop_cd, ls_fr_shop_cd, ls_to_shop_cd, ls_shop_nm, ls_qry, ls_force,ls_limit_yn, ls_week_start_date
long ll_row, ll_out_qty, ll_found, ll_qty, kk, ll_gijisi, ll_wanbul, ll_found_n, ll_move_qty
...
select  dbo.sf_week_start(:is_yymmdd)
into :ls_week_start_date
from dual;
		...
		where style       = :is_style
		  ...
		  and yymmdd    between  :ls_week_start_date  and  convert(char(08), dateadd(day, 7,:is_yymmdd) ,112) ;
		...
		ll_found = dw_4.Find("fr_shop_cd = '" + ls_shop_cd + "'", ll_found_n + 1 , kk)
			if ll_found > 0 then
				...
				ll_move_qty =  dw_4.GetitemNumber(ll_found, "move_qty")
				
				IF IsNull(ll_move_qty) THEN
					ll_move_qty = 0
				END IF
								
				ll_qty = ll_qty + ll_move_qty
				ll_found_n = ll_found
			end if	
		next
		...
		if ll_out_qty <= 0 then 
				if ls_force = "N"  and ll_gijisi > 0 then
 				   messagebox("경고!","금주 기지시된 " + string(ll_gijisi, "0000") + "장이 있어 가용재고를 초과 하였습니다!")
				elseif ls_force = "N"  and ll_gijisi <= 0 then
...
If is_DW_sort = "A" then
	...
	ls_Obj = MidA(LS_OBJ_1,1,li_po - 1) + " " + is_DW_sort
...
from dual;
		...
		ll_found = dw_10.Find("fr_shop_cd = '" + ls_shop_cd + "'", ll_found_n + 1 , kk)
			if ll_found > 0 then
				...
				ll_move_qty =  dw_10.GetitemNumber(ll_found, "move_qty")
