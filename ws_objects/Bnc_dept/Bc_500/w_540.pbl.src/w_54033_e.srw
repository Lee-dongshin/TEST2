$PBExportHeader$w_54033_e.srw
$PBExportComments$RT요청지시처리
forward
global type w_54033_e from w_com010_e
end type
type cb_out_proc from commandbutton within w_54033_e
end type
type st_1 from statictext within w_54033_e
end type
end forward
...
global w_54033_e w_54033_e

type variables
String is_brand, is_yymmdd, is_fr_ymd, is_to_ymd, is_proc_yn , is_st_brand, is_out_ymd, is_shop_cd, is_style
...
//messagebox("is_st_brand", is_st_brand)
//messagebox("is_fr_ymd", is_fr_ymd)
//messagebox("is_to_ymd", is_to_ymd)
//messagebox("is_proc_yn", is_proc_yn)
...
il_rows = dw_body.retrieve(is_brand, is_st_brand, is_fr_ymd, is_to_ymd, is_proc_yn, is_year, is_season, is_shop_cd, is_style)
...
event type integer ue_popup(string as_column, long al_row, string as_data, integer ai_div);call super::ue_popup;String     ls_shop_nm , ls_style, ls_brand, ls_st_brand
Boolean    lb_check 
...
CHOOSE CASE as_column
	CASE "shop_cd"				
			...
			ls_brand = dw_head.getitemstring(al_row, "brand")
		   ...
		   gst_cd.ai_div          = ai_div
			...
			gst_cd.default_where   = "WHERE BRAND = '" + ls_brand + "' AND SHOP_STAT = '00' AND SHOP_DIV <> 'A' "
...
//				   dw_head.SetItem(al_row, "shop_nm", "")
		   ...
		   gst_cd.ai_div          = ai_div
			...
			ls_st_brand = dw_head.getitemstring(al_row, "st_brand")			
			
			IF  gl_user_level = 0 then 
					gst_cd.default_where   = "WHERE   style like '" + ls_st_brand + "%'"	
				else 	
...
type cb_close from w_com010_e`cb_close within w_54033_e
end type

type cb_delete from w_com010_e`cb_delete within w_54033_e
boolean visible = false
end type

type cb_insert from w_com010_e`cb_insert within w_54033_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_54033_e
end type

type cb_update from w_com010_e`cb_update within w_54033_e
end type

type cb_print from w_com010_e`cb_print within w_54033_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_54033_e
boolean visible = false
integer width = 439
string text = "거래명세서(&V)"
end type

type gb_button from w_com010_e`gb_button within w_54033_e
end type

type cb_excel from w_com010_e`cb_excel within w_54033_e
integer x = 2510
boolean enabled = true
end type

type dw_head from w_com010_e`dw_head within w_54033_e
integer y = 156
integer height = 312
string dataobject = "d_54033_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_54033_e
integer beginy = 472
integer endy = 472
end type

type ln_2 from w_com010_e`ln_2 within w_54033_e
integer beginy = 476
integer endy = 476
end type

type dw_body from w_com010_e`dw_body within w_54033_e
integer y = 484
integer width = 3584
string dataobject = "d_54033_d01"
...
type dw_print from w_com010_e`dw_print within w_54033_e
integer x = 2455
integer y = 876
...
//ii_deal_seq = dw_head.GetItemNumber(1, "deal_seq")
//if IsNull(ii_deal_seq) or ii_deal_seq < 400 then
//   MessageBox("알림!","400번대로 배분차수를 입력하십시요!")
//   dw_head.SetFocus()
//   dw_head.SetColumn("deal_seq")
//end if
...
Net = MessageBox("경고", "수량 확정된 요청 내역이 매장으로 지시됩니다. 계속하시겠습니까?", Exclamation!, OKCancel!, 2)
...
IF Net = 1 THEN 
	...
	st_1.text = "<-- 처리 중입니다. 잠시 기다려 주세요!  "
		...
		DECLARE SP_54033_P01 PROCEDURE FOR SP_54033_P01  
			@brand		= :is_brand,
		 ...
		 EXECUTE SP_54033_P01 ;
		 
		IF SQLCA.SQLCODE = -1 THEN 
			rollback  USING SQLCA;				
			MessageBox("SQL오류", SQLCA.SqlErrText) 
			st_1.text = "<-- 확인 후 다시 작업하세요!"
//			st_1.visible = false			
			Return -1 			
		ELSE
