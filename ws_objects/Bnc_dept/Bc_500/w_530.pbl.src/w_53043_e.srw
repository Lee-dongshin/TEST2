...
Long	 i, ll_rowcnt,ll_row_count1, LL_STYLE_CHK, ll_shop_chk,ll_shop_chk1
...
FOR i=1 TO ll_row_count1
   ls_shop_cd = 	dw_BODY.GetItemString(i, "shop_cd")
	...
	if ll_shop_chk <= 0 then
		ll_shop_chk1 = ll_shop_chk1 + 1
	end if
	
	dw_BODY.SETItem(i, "CHK_STYLE", ls_dc_chk + "|" + "매장X" )	
...
if LL_STYLE_CHK <> 0 or ll_shop_chk1 <> 0  then
	messagebox("알림!", STRING(LL_STYLE_CHK , "0000") + "건의 품번, " + STRING(ll_shop_chk1 , "0000") +  "건의 매장코드 오류가 있습니다! 파일을 수정 후 재등록 해주세요!"		)
else 
	hpb_1.Position = 100
	dw_body.SetFocus()		
