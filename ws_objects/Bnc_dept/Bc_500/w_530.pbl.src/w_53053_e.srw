$PBExportHeader$w_53053_e.srw
$PBExportComments$점간 이송 등록(신)
forward
global type w_53053_e from w_com010_e
end type
type dw_out from u_dw within w_53053_e
end type
type dw_rtn from u_dw within w_53053_e
end type
type cbx_laser from checkbox within w_53053_e
end type
type dw_1 from u_dw within w_53053_e
end type
end forward
...
String is_fr_shop_cd, is_fr_shop_type, is_to_shop_cd, is_to_shop_type, is_brand_grp 
...
public function boolean wf_style_set_color (long al_row, string as_style, string as_color, string as_flag)
...
IF wf_style_set(al_row, ls_style, as_flag) THEN 
   ...
   dw_body.SetItem(al_row, "tag_price",  ll_tag_price)
	dw_body.SetItem(al_row, "d_year",     ls_year)
	dw_body.SetItem(al_row, "d_season",   ls_season)
	dw_body.SetItem(al_row, "d_sojae",    ls_sojae)
	dw_body.SetItem(al_row, "d_item",     ls_item)
...
public function boolean wf_style_set (long al_row, string as_style, string as_flag);String  ls_style,       ls_to_ymd,      ls_sale_type_chk = space(2), ls_chno
Long    ll_row,         ll_dc_rate_chk, ll_curr_price_chk,          ll_out_price_chk, ll_tag_price
Decimal ldc_margin_chk, ldc_dc_rate_chk

String  ls_null,        ls_sale_type = space(2)
Long    ll_curr_price,  ll_out_price, ll_qty ,ll_move_qty
decimal ldc_margin,     ldc_dc_rate

SetNull(ls_null) 

ls_chno = dw_body.getitemstring(al_row, "chno")
ll_qty = dw_body.getitemNumber(al_row, "move_qty")
...
//	dw_body.Setitem(al_row, "size",  ls_null)
		...
		dw_body.Setitem(al_row, "move_qty",        ll_move_qty            )
...
public function boolean wf_style_set_color (long al_row, string as_style, string as_color, string as_flag);String  ls_style,       ls_to_ymd,      ls_sale_type_chk = space(2), ls_chno
Long    ll_row,         ll_dc_rate_chk, ll_curr_price_chk,          ll_out_price_chk, ll_tag_price
Decimal ldc_margin_chk, ldc_dc_rate_chk
...
If as_flag = 'RTN' Then
	/* 발송(반품)시 마진율 체크 */
	IF gf_outmarjin_color (is_yymmdd,    is_fr_shop_cd, is_fr_shop_type, as_style, as_color, & 
							ls_sale_type, ldc_margin,    ldc_dc_rate,      ll_curr_price, ll_out_price) = FALSE THEN 
		RETURN False 
	END IF
...
/* 인수(출고)시 마진율 체크 */
IF gf_outmarjin_color (is_to_ymd,    is_to_shop_cd, is_to_shop_type, as_style, as_color, & 
						ls_sale_type, ldc_margin,    ldc_dc_rate,      ll_curr_price, ll_out_price) = FALSE THEN 
	RETURN False 
END IF
...
//		END IF
		...
		If ls_sale_type_chk <> ls_sale_type or ldc_margin_chk <> ldc_margin Then
			...
			MESSAGEBOX("ls_sale_type_chk", ls_sale_type_chk)
...
public function integer wf_data_set (long al_body_row, string as_bill_no, string as_no, string as_flag);Long ll_row, ll_row1
string ls_style
...
//	dw_rtn.SetItem(ll_row, "yymmdd",        is_yymmdd      )
	dw_rtn.SetItem(ll_row, "yymmdd",        is_to_ymd      )	
...
//	End If
	...
	ls_style =  dw_body.GetItemString (al_body_row, "style")
	
	dw_rtn.SetItem(ll_row, "brand",         LeftA(ls_style,1)       )
	dw_rtn.SetItem(ll_row, "year",          dw_body.GetItemString (al_body_row, "d_year")  )
	dw_rtn.SetItem(ll_row, "season",        dw_body.GetItemString (al_body_row, "d_season"))
	dw_rtn.SetItem(ll_row, "item",          dw_body.GetItemString (al_body_row, "d_item")  )
	dw_rtn.SetItem(ll_row, "sojae",         dw_body.GetItemString (al_body_row, "d_sojae") )
	dw_rtn.SetItem(ll_row, "reg_id",        gs_user_id     )

elseIf as_flag = 'RTN1' Then
	
	ll_row1 = dw_1.InsertRow(0)
	
//	dw_1.SetItem(ll_row1, "yymmdd",        is_yymmdd      )
	dw_1.SetItem(ll_row1, "yymmdd",        is_to_ymd      )	
	dw_1.SetItem(ll_row1, "shop_cd",       is_fr_shop_cd  )
	dw_1.SetItem(ll_row1, "shop_type",     is_fr_shop_type)
	dw_1.SetItem(ll_row1, "out_no",        as_bill_no     )
	dw_1.SetItem(ll_row1, "house_cd",      '990000'       )
	dw_1.SetItem(ll_row1, "jup_gubn",      'O2'           )
	dw_1.SetItem(ll_row1, "sale_type",     dw_body.GetItemString (al_body_row, "rtn_sale_type")  )
	dw_1.SetItem(ll_row1, "margin_rate",   dw_body.GetItemDecimal(al_body_row, "rtn_margin_rate"))
	dw_1.SetItem(ll_row1, "disc_rate",     dw_body.GetItemDecimal(al_body_row, "rtn_disc_rate")  )
	dw_1.SetItem(ll_row1, "no",            as_no          )
	dw_1.SetItem(ll_row1, "style",         dw_body.GetItemString (al_body_row, "style"))
	dw_1.SetItem(ll_row1, "chno",          dw_body.GetItemString (al_body_row, "chno") )
//	If is_fr_shop_type > '3' Then
//		dw_rtn.SetItem(ll_row, "color",      'XX')
//		dw_rtn.SetItem(ll_row, "size",       'XX')
//	Else
		dw_1.SetItem(ll_row1, "color",      dw_body.GetItemString (al_body_row, "color"))
		dw_1.SetItem(ll_row1, "size",       dw_body.GetItemString (al_body_row, "size") )
//	End If
	dw_1.SetItem(ll_row1, "class",         'A'            )
	dw_1.SetItem(ll_row1, "tag_price",     dw_body.GetItemDecimal(al_body_row, "tag_price")     )
	dw_1.SetItem(ll_row1, "curr_price",    dw_body.GetItemNumber (al_body_row, "rtn_curr_price"))
	dw_1.SetItem(ll_row1, "out_price",     dw_body.GetItemNumber (al_body_row, "rtn_out_price") )
	dw_1.SetItem(ll_row1, "qty",           dw_body.GetItemDecimal(al_body_row, "move_qty")      )
	dw_1.SetItem(ll_row1, "tag_amt",       dw_body.GetItemDecimal(al_body_row, "tag_amt")       )
	dw_1.SetItem(ll_row1, "curr_amt",      dw_body.GetItemDecimal(al_body_row, "rtn_curr_amt")  )
	dw_1.SetItem(ll_row1, "rtrn_collect",  dw_body.GetItemDecimal(al_body_row, "rtrn_collect")  )
	dw_1.SetItem(ll_row1, "vat",           dw_body.GetItemDecimal(al_body_row, "rtn_vat")       )
	dw_1.SetItem(ll_row1, "rot_shop",      is_to_shop_cd  )
	dw_1.SetItem(ll_row1, "rot_shop_type", is_to_shop_type)
	
	ls_style =  dw_body.GetItemString (al_body_row, "style")	
	
	dw_1.SetItem(ll_row1, "brand",         LeftA(ls_style,1)    )
	dw_1.SetItem(ll_row1, "year",          dw_body.GetItemString (al_body_row, "d_year")  )
	dw_1.SetItem(ll_row1, "season",        dw_body.GetItemString (al_body_row, "d_season"))
	dw_1.SetItem(ll_row1, "item",          dw_body.GetItemString (al_body_row, "d_item")  )
	dw_1.SetItem(ll_row1, "sojae",         dw_body.GetItemString (al_body_row, "d_sojae") )
	dw_1.SetItem(ll_row1, "reg_id",        gs_user_id     )	
Else
...
//	Else
	...
	dw_out.SetItem(ll_row, "brand",          LeftA(ls_style,1)          )
	dw_out.SetItem(ll_row, "year",          dw_body.GetItemString (al_body_row, "d_year")  )
	dw_out.SetItem(ll_row, "season",        dw_body.GetItemString (al_body_row, "d_season"))
	dw_out.SetItem(ll_row, "item",          dw_body.GetItemString (al_body_row, "d_item")  )
	dw_out.SetItem(ll_row, "sojae",         dw_body.GetItemString (al_body_row, "d_sojae") )
...
if MidA(is_fr_shop_cd,2,1) = "X" then 
	...
	MessageBox("확인", ls_style +ls_chno + ls_color + ls_size + " 재고수량: " + string(ll_stock_qty) + ", 재고가 없는 제품입니다! 처리 후 재고 확인이 필요합니다. ") 
	//RETURN FALSE 
	END IF
end if
...
/* 수정일      : 2002.03.14                                                  */
/*===========================================================================*/
String     ls_shop_nm, ls_style, ls_chno, ls_given_fg, ls_given_ymd, ls_shop_type,ls_work_gubn, ls_color, ls_brand_grp
Boolean    lb_check 
...
select DBO.SF_INTER_CD2('035',:is_brand)
into :ls_brand_grp
from dual;
...
//				END IF 							
				...
				END IF 			
 				...
 				IF wf_style_set(al_row, ls_style, 'RTN') THEN 
				   ...
				   dw_body.SetItem(al_row, "d_year",     lds_Source.GetItemString(1,"year")    )
				   dw_body.SetItem(al_row, "d_season",   lds_Source.GetItemString(1,"season")  )
				   dw_body.SetItem(al_row, "d_sojae",    lds_Source.GetItemString(1,"sojae")   )
				   dw_body.SetItem(al_row, "d_item",     lds_Source.GetItemString(1,"item")    )
					wf_style_set_color(al_row, ls_style, ls_color, "RTN")					
					ib_changed = true
					lb_check = TRUE 
...
/* 수정일      : 2002.03.14                                                  */
/*===========================================================================*/
Long i, ll_row_count, ll_rtn_row = 0, ll_out_row = 0,  ll_rtn_row2 = 0 , ll_rtn_row3 = 0
String ls_rtn_no, ls_out_no, ls_rtn_row, ls_out_row, ls_to_out_no, ls_rt_yn,ls_tran_cust,ls_tran_type,ls_rtrn_no, ls_rtn_row3
Decimal ldc_move_qty
Datetime ld_datetime
Long ll_rtn_rows, ll_out_rows, ll_qty, ll_rtn_rows2
...
//차후승인반품번호채번
If gf_style_outno(is_to_ymd, is_brand, ls_rtrn_no) = False Then
	MessageBox("저장오류", "반품번호 채번에 실패하였습니다!")
	Return -1
End IF
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified!  THEN				/* New Record */
								...
								Else
									...
									ll_rtn_row2++
									ls_rtn_row = String(ll_rtn_row2, '0000')
						//			wf_data_set(i, ls_rtn_no, ls_rtn_row, 'RTN')
									...
									dw_body.Setitem(i, "reg_id",       gs_user_id     )
											...
											If dw_body.GetItemString(i, "proc_yn") = 'Y'  and isnull(ls_to_out_no) Then
												...
												wf_data_set(i, ls_rtrn_no, ls_rtn_row, 'RTN')
												dw_body.Setitem(i, "rtrn_ymd",    is_to_ymd )
												dw_body.Setitem(i, "rtrn_outno",  ls_rtrn_no )
												dw_body.Setitem(i, "rtrn_no",     ls_rtn_row)				
												...
												//반품임시데이터 등록
												ll_rtn_row3++
												ls_rtn_row3 = String(ll_rtn_row3, '0000')
												wf_data_set(i, ls_rtn_no, ls_rtn_row3, 'RTN1')
												dw_body.Setitem(i, "rtrn_ymd",    is_to_ymd )
												dw_body.Setitem(i, "rtrn_outno",  ls_rtn_no )
												dw_body.Setitem(i, "rtrn_no",     ls_rtn_row3)	
...
ll_rtn_rows2 = dw_1. Update(TRUE, FALSE)
...
if il_rows = 1 and ll_rtn_rows = 1 and ll_out_rows = 1 then
   ...
   commit USING SQLCA;
	...
	DW_BODY.GetChild("tb_53020_h_tran_type", idw_tran_type)
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_53053_e","0")
end event
...
type cb_close from w_com010_e`cb_close within w_53053_e
end type

type cb_delete from w_com010_e`cb_delete within w_53053_e
end type

type cb_insert from w_com010_e`cb_insert within w_53053_e
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_53053_e
end type

type cb_update from w_com010_e`cb_update within w_53053_e
end type

type cb_print from w_com010_e`cb_print within w_53053_e
integer width = 439
string text = "거래명세서(&P)"
end type

type cb_preview from w_com010_e`cb_preview within w_53053_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_53053_e
end type

type cb_excel from w_com010_e`cb_excel within w_53053_e
boolean visible = false
end type

type dw_head from w_com010_e`dw_head within w_53053_e
integer width = 3502
integer height = 316
string dataobject = "d_53053_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_53053_e
integer beginy = 520
integer endy = 520
end type

type ln_2 from w_com010_e`ln_2 within w_53053_e
integer beginy = 524
integer endy = 524
end type

type dw_body from w_com010_e`dw_body within w_53053_e
integer y = 540
integer height = 1500
string dataobject = "d_53053_d01"
...
//			This.Post Event ue_set_col("sale_qty")
	...
	CASE "proc_yn"
		If data = 'Y' Then
			...
			IF wf_style_set_color(row, ls_style, ls_color,'RTN') = False THEN 
				Return 1
			END IF
			
			IF wf_style_set_color(row, ls_style, ls_color, 'OUT') = False THEN 
				Return 1
			END IF
		End If
	...
	CASE "color"	
		...
		ls_style = LeftA(This.GetItemString(row, "style_no"), 8)
		wf_style_set_color(row, ls_style, data, "RTN")
...
/*===========================================================================*/
Long i
String ls_style, ls_proc_yn, ls_proc_chk, ls_color
	...
	If dwo.Text = '선택' Then
		For i = 1 To This.RowCount()
			...
			if ls_proc_chk = "N" then
				...
				IF wf_style_set_color(i, ls_style, ls_color,'RTN') THEN 
					This.SetItem(i, "proc_yn", 'Y')
					
					IF wf_style_set_color(i, ls_style, ls_color, 'OUT') = False THEN 
						Return 1
					END IF					
...
type dw_print from w_com010_e`dw_print within w_53053_e
integer x = 288
integer y = 1132
integer width = 3346
integer height = 932
