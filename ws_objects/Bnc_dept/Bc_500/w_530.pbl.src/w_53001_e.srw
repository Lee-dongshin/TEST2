$PBExportHeader$w_53001_e.srw
$PBExportComments$판매등록
forward
global type w_53001_e from w_com010_e
end type
type dw_member from datawindow within w_53001_e
end type
type st_1 from statictext within w_53001_e
end type
type cb_input from commandbutton within w_53001_e
end type
type st_2 from statictext within w_53001_e
end type
type dw_member_sale from datawindow within w_53001_e
end type
type dw_list from datawindow within w_53001_e
end type
end forward
...
global w_53001_e w_53001_e

type variables
DataWindowChild idw_color, idw_size
String is_brand, is_yymmdd, is_shop_cd, is_shop_type, is_data_opt, is_member_return,is_style, is_chno, is_brand_grp
end variables
...
string  ls_jumin, ls_card_no, ls_sale_fg, ls_style_no, ls_item, ls_coupon_no, ls_coupon_yn, ls_date_chk, ls_sale_type
...
END IF
 ...
 il_remain_amt =  (ll_remain_point  / 10) 
 ll_remain_point = il_remain_amt  *  10
...
if  ld_coupon_amt > 0  and ll_goods_amt >= ld_coupon_amt  then	  	    		
	   FOR i=1 TO ll_row_count
		    ...
		    // 가장 최저가 상품에 구매할인권을 쓰게 처리 						 
				IF  ll_sale_price > ld_coupon_amt  and ll_sale_qty  > 0 and ld_dc_rate < 31 and ls_sale_type < '30' THEN  	
					 k = k + 1
					if k= 1 then
...
type cb_close from w_com010_e`cb_close within w_53001_e
integer taborder = 140
end type

type cb_delete from w_com010_e`cb_delete within w_53001_e
integer taborder = 130
end type

type cb_insert from w_com010_e`cb_insert within w_53001_e
integer taborder = 70
boolean enabled = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_53001_e
integer x = 2839
integer width = 384
integer taborder = 40
string text = "일보조회(&Q)"
...
type cb_update from w_com010_e`cb_update within w_53001_e
end type

type cb_print from w_com010_e`cb_print within w_53001_e
boolean visible = false
integer taborder = 100
end type

type cb_preview from w_com010_e`cb_preview within w_53001_e
boolean visible = false
integer taborder = 110
end type

type gb_button from w_com010_e`gb_button within w_53001_e
end type

type cb_excel from w_com010_e`cb_excel within w_53001_e
integer taborder = 120
boolean enabled = true
end type

type dw_head from w_com010_e`dw_head within w_53001_e
integer y = 156
integer width = 3447
integer height = 220
...
type ln_1 from w_com010_e`ln_1 within w_53001_e
integer beginy = 456
integer endy = 456
end type

type ln_2 from w_com010_e`ln_2 within w_53001_e
integer beginy = 460
integer endy = 460
end type

type dw_body from w_com010_e`dw_body within w_53001_e
...
type dw_print from w_com010_e`dw_print within w_53001_e
end type
