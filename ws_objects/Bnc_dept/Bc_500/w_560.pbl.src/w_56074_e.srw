$PBExportHeader$w_56074_e.srw
$PBExportComments$판매마감관리(년도시즌)
forward
global type w_56074_e from w_com010_d
end type
type dw_1 from datawindow within w_56074_e
end type
type cb_shop_input from commandbutton within w_56074_e
end type
type dw_2 from datawindow within w_56074_e
end type
type cb_style_input from commandbutton within w_56074_e
end type
end forward
...
dw_2 dw_2
cb_style_input cb_style_input
end type
global w_56074_e w_56074_e
...
this.cb_style_input=create cb_style_input
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.dw_1
this.Control[iCurrent+2]=this.cb_shop_input
this.Control[iCurrent+3]=this.dw_2
this.Control[iCurrent+4]=this.cb_style_input
end on

on w_56074_e.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.dw_1)
destroy(this.cb_shop_input)
destroy(this.dw_2)
destroy(this.cb_style_input)
end on
...
/* 수정일      : 2002.03.20                                                  */
/*===========================================================================*/
String     ls_card_no, ls_jumin, ls_custom_nm, ls_tel_1, ls_tel_no1, ls_tel_no2, ls_tel_no3, ls_null, ls_tel_no, ls_user_name,ls_part_nm, ls_style, ls_chno
...
CHOOSE CASE as_column
	CASE "shop_cd", "cb_shop_cd"			
		   ...
		   gst_cd.ai_div          = ai_div
			...
			if is_brand = "N" or is_brand = "J" or is_brand = "E" or is_brand = "M" then
				ls_brand = "[NJ]%"
			elseif is_brand = "O" or is_brand = "Y" or is_brand = "A" 	then 
				ls_brand = "[OYA]%"
			end if
			
			gst_cd.default_where   = "WHERE Shop_Stat = '00' " + & 
			                         "  AND SHOP_DIV  IN ('G', 'K', 'B','H') " + &
											 "  AND BRAND like '" + ls_brand + "'"
			...
			ELSE
		      ls_style = MidA(as_data, 1, 8)
				...
				END IF					
					...
					dw_2.SetItem(al_row, "year",  lds_Source.GetItemString(1,"year"))
					dw_2.SetItem(al_row, "season",lds_Source.GetItemString(1,"season"))
//					dw_body.SetItem(al_row, "brand", mid(lds_Source.GetItemString(1,"style"),1,1))					
...
inv_resize.of_Register(cb_style_input, "FixedToRight")
end event

type cb_close from w_com010_d`cb_close within w_56074_e
end type

type cb_delete from w_com010_d`cb_delete within w_56074_e
boolean visible = true
end type

type cb_insert from w_com010_d`cb_insert within w_56074_e
boolean visible = true
end type

type cb_retrieve from w_com010_d`cb_retrieve within w_56074_e
end type

type cb_update from w_com010_d`cb_update within w_56074_e
boolean visible = true
end type

type cb_print from w_com010_d`cb_print within w_56074_e
boolean visible = false
end type

type cb_preview from w_com010_d`cb_preview within w_56074_e
boolean visible = false
end type

type gb_button from w_com010_d`gb_button within w_56074_e
end type

type cb_excel from w_com010_d`cb_excel within w_56074_e
boolean visible = false
end type

type dw_head from w_com010_d`dw_head within w_56074_e
integer x = 9
integer y = 160
integer width = 3584
integer height = 132
string dataobject = "d_56074_h01"
end type
...
type ln_1 from w_com010_d`ln_1 within w_56074_e
integer beginy = 296
integer endy = 296
end type

type ln_2 from w_com010_d`ln_2 within w_56074_e
integer beginy = 300
integer endy = 300
end type

type dw_body from w_com010_d`dw_body within w_56074_e
integer y = 316
integer width = 3598
integer height = 1696
string dataobject = "d_56074_d01"
...
type dw_print from w_com010_d`dw_print within w_56074_e
integer x = 2450
integer y = 364
integer height = 432
end type
...
Choose Case ls_column_nm
	Case "add"
			...
			dw_1.Setitem(il_rows, "brand", is_brand)
			dw_1.Setitem(il_rows, "year", is_year)			
			dw_1.Setitem(il_rows, "season", is_season)				
	Case "delete"
		ll_row = This.GetRow()
...
Choose Case ls_column_nm
	Case "add"
		...
		/* 추가된 Row의 첫번째 Tab Order 항목으로 이동 */
			if il_rows > 0 then
				dw_2.ScrollToRow(il_rows)
				dw_2.SetColumn(ii_min_column_id)
				dw_2.SetFocus()
			end if
			
			//This.SetColumn(il_rows)			
			dw_2.Setitem(il_rows, "year", is_year)			
			dw_2.Setitem(il_rows, "season", is_season)						
...
dw_2.retrieve(ls_brand, ls_year, ls_season)
