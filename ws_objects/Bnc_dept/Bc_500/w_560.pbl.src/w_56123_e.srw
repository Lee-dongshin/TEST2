...
FOR i = 1 TO dw_body.RowCount() 
	...
	IF ls_print = "Y"  THEN 
		...
		il_rows = dw_print.Retrieve(is_brand, ls_yymmdd, ls_yymmdd, is_shop_div, ls_shop_cd)
...
//코인코즈 소연 A30301
		...
		elseif	data = "N" then
			...
			dw_1.setitem(row, "bank_no", "387-910006-81404")	
			dw_1.setitem(row, "mng_emp", "c30106")			
			dw_1.setitem(row, "tel_no", "2225-0122")	
			IF gf_user_nm("c30106", ls_person_nm) <> 0 THEN
				ls_person_nm = ''
			END IF
			dw_1.SetItem(row, "mng_nm", ls_person_nm)		
		elseif	data = "J" then
