...
long i_pos1, i_pos2
	...
	ll_buf = 25
		...
		ls_string = "http://with.ibeaucre.co.kr/instant/chkver.asp?gubn=dept&saup=bnc&name=pbd"
...
event open;//
MessageBox("BNC_CUST", "환영합니다.")			/*2025.12.29 dslee Git Test 를 위한 추가 */
end event
...
string   ls_work_gbn,ls_string ,ls_file_date, ls_file_path
Long     ll_err_no, ll_user_level 

string 		ls_ini_file,ls_compname, ls_Ipaddr
LONG ll_buf
String ls_publicip, ls_host,ls_file_name
int li_rc1
long i_pos1, i_pos2
...
//	messagebox("버전체크", "버전일치")
//else 
//	messagebox("버전오류 - 업데이트 필요! ", "실행하신 영업관리가 최신 버전과 달라 실행할 수 없습니다. 재설치 또는 MIS팀에 문의 바랍니다!" )
//	return
//end if	
//
...
//		Post Close(w_main01)
			...
			wstyle = 0
			ls_file_name = "c:\bnc_dept\beaucre_dept_install.exe"
