$PBExportHeader$w_81106_e.srw
$PBExportComments$인사평가(1차상사)
forward
global type w_81106_e from w_com020_e
end type
type dw_1 from datawindow within w_81106_e
end type
type dw_2 from datawindow within w_81106_e
end type
end forward
...
event open;call super::open;string ls_empno_nm
dw_head.setitem(1,"empno",gs_user_id)
dw_head.setitem(1,"kname",gs_user_nm)
dw_head.setitem(1,"yyyy","2019")
...
il_rows = dw_print.retrieve(is_yyyy, is_key_value, is_jikgun_type, is_first_staff, is_second_staff, 'B')
...
type cb_close from w_com020_e`cb_close within w_81106_e
end type

type cb_delete from w_com020_e`cb_delete within w_81106_e
boolean visible = false
end type

type cb_insert from w_com020_e`cb_insert within w_81106_e
boolean visible = false
end type

type cb_retrieve from w_com020_e`cb_retrieve within w_81106_e
end type

type cb_update from w_com020_e`cb_update within w_81106_e
end type

type cb_print from w_com020_e`cb_print within w_81106_e
boolean visible = false
end type

type cb_preview from w_com020_e`cb_preview within w_81106_e
end type

type gb_button from w_com020_e`gb_button within w_81106_e
end type

type cb_excel from w_com020_e`cb_excel within w_81106_e
end type

type dw_head from w_com020_e`dw_head within w_81106_e
integer y = 164
integer height = 164
string dataobject = "d_81106_H01"
end type

type ln_1 from w_com020_e`ln_1 within w_81106_e
integer beginy = 352
integer endy = 352
end type

type ln_2 from w_com020_e`ln_2 within w_81106_e
integer beginy = 356
integer endy = 356
end type

type dw_list from w_com020_e`dw_list within w_81106_e
integer y = 380
integer width = 1065
integer height = 1660
string dataobject = "d_81106_L01"
end type
...
type dw_body from w_com020_e`dw_body within w_81106_e
integer x = 1111
integer y = 716
integer width = 2482
integer height = 624
string dataobject = "d_81106_d01"
end type
...
type st_1 from w_com020_e`st_1 within w_81106_e
integer x = 1093
integer y = 380
integer height = 1660
end type
...
type dw_print from w_com020_e`dw_print within w_81106_e
integer x = 110
integer y = 896
	...
	CASE "cf_flag"	     //  Popup 검색창이 존재하는 항목 
		...
		if data = '1' then 
			for i = 1 to dw_body.rowcount()
				ls_grade = dw_body.getitemstring(i,"first_grade")
				ls_value_code = dw_body.getitemstring(i,"value_code")
				if ls_value_code <> 'C' and (isnull(ls_grade) or ls_grade = '') then
					messagebox('확인','자기평가를 입력해 주세요..')
					dw_body.setfocus()
					dw_body.setrow(i)
					dw_body.setcolumn('first_grade')					
					return 1
				elseif ls_value_code = 'C' then
					ls_grade = dw_body.getitemstring(i,"first_grade")
					ls_sub_title = dw_body.getitemstring(i,"sub_title")
					if MidA(ls_sub_title,4,10)='1차 상사' and (isnull(ls_grade) or ls_grade = '') then
						messagebox('확인','1차 상사 평가를 입력해 주세요..')
						dw_body.setfocus()
						dw_body.setrow(i)
						dw_body.setcolumn('first_grade')
						return 1						
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_81106_d03"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type

event ue_dw_1_set;string ls_first_decision

IF dw_2.AcceptText() <> 1 THEN RETURN 

ls_first_decision = dw_2.getitemstring(1,'first_decision')

dw_1.setitem(1,'first_decision',ls_first_decision)
end event
...
string ls_job_rt, ls_last_grade, ls_first_decision, ls_second_decision
...
//		dw_1.setitem(1,'job_month',ll_job_month)
	...
	case "first_decision"
		ls_first_decision = data
		dw_1.setitem(1,'first_decision',ls_first_decision)

////	case "second_decision"
////		ls_second_decision = data
////		dw_1.setitem(1,'second_decision',ls_second_decision)
////
////	CASE "cf_flag"	     //  Popup 검색창이 존재하는 항목 
////		ls_status = dw_1.getitemstring(1,"status")
////		if data = '1' then 
...
//				ls_status = 'A'
//			else
//				ls_status = char(asc(ls_status)+1)
//			end if
//			dw_1.setitem(1,"status",ls_status)
//
//		elseif data = '0' then 
