...
string is_Rqst_Gubn, is_Rqst_Date, is_season,   is_year,     is_item,   is_sojae, is_house_cd, is_brand_grp
//       Work_NO
...
//      and shop_cd   = :ls_shop_cd  ;
	...
	IF SQLCA.SQLCODE = 0 THEN
      IF gf_stresv_update (ls_style,    ls_chno,     ls_color,   ls_size, &
 	                        ls_shop_div, ll_out_qty * -1, is_HOUSE_CD, as_ErrMsg) = FALSE THEN 
	      RETURN FALSE
		END IF 
	END IF
NEXT 
	...
	CASE "style"				// 거래처 코드
			...
			END IF
				...
				IF  gl_user_level = 0 then 
					gst_cd.default_where   =  "WHERE  '" + is_brand_grp + "' like '%' + brand + '%' "	 //"WHERE   style like '" + gs_brand + "%'"	
				else 	
					if gs_brand <> "K" then
