$PBExportHeader$w_42022_d.srw
$PBExportComments$물류부자재 출고조회
forward
global type w_42022_d from w_com010_d
end type
end forward
...
global w_42022_d w_42022_d

type variables
String is_brand, is_yymmdd_st, is_yymmdd_ed, is_shop_cd, is_shop_div, is_print_gubn
...
if is_print_gubn = "N" then 
	dw_body.DataObject = "d_42022_d01"
	dw_body.SetTransObject(SQLCA)
elseif is_print_gubn = "S" then 
   dw_body.DataObject = "d_42022_d02"	
	dw_body.SetTransObject(SQLCA)
elseif is_print_gubn = "M" then 
   dw_body.DataObject = "d_42022_d03"	
	dw_body.SetTransObject(SQLCA)	
elseif is_print_gubn = "P" then 
   dw_body.DataObject = "d_42022_d07"	
	dw_body.SetTransObject(SQLCA)		
elseif is_print_gubn = "Q" then 
   dw_body.DataObject = "d_42022_d05"	
	dw_body.SetTransObject(SQLCA)
	
else
   dw_body.DataObject = "d_42022_d04"	
	dw_body.SetTransObject(SQLCA)		
end if	
...
IF is_yymmdd_st > is_yymmdd_ed THEN
   MessageBox(ls_title,"일자오류! 일자를 다시 입력하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("to_ymd")
   RETURN FALSE 
END IF

//IF DaysAfter(Date(String(is_yymmdd_st, '@@@@/@@/@@')), Date(String(is_yymmdd_ed, '@@@@/@@/@@'))) >= 60 then
...
is_print_gubn = dw_head.GetItemString(1, "print_gubn")
IF IsNull(is_print_gubn) OR Trim(is_print_gubn) = "" THEN
   MessageBox(ls_title,"출력형태를 선택하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("print_gubn")
   RETURN FALSE
END IF
...
//MESSAGEBOX("LS_MODUFY", LS_MODIFY)
...
event ue_print();
if is_print_gubn = "N" then 
	dw_print.DataObject = "d_42022_r01"
	dw_print.SetTransObject(SQLCA)
	dw_body.ShareData(dw_print)
	This.Trigger Event ue_title()
elseif is_print_gubn = "Q" then 
   dw_print.DataObject = "d_42022_r07"	
	dw_print.SetTransObject(SQLCA)
	dw_body.ShareData(dw_print)
	This.Trigger Event ue_title()	
	
elseif is_print_gubn = "S" then 
   dw_print.DataObject = "d_42022_r04"	
	dw_print.SetTransObject(SQLCA)
	dw_print.retrieve(is_brand, is_yymmdd_st, is_yymmdd_ed, is_shop_cd, is_shop_div)
	This.Trigger Event ue_title()	
	
elseif is_print_gubn = "M" then 
   dw_print.DataObject = "d_42022_r05"	
	dw_print.SetTransObject(SQLCA)
	dw_print.retrieve(is_brand, is_yymmdd_st, is_yymmdd_ed, is_shop_cd, is_shop_div)
	This.Trigger Event ue_title()		
elseif is_print_gubn = "P" then 
   ...
   dw_print.DataObject = "d_42022_r06"	
	dw_print.SetTransObject(SQLCA)
	dw_print.retrieve(is_brand, is_yymmdd_st, is_yymmdd_ed, is_shop_cd, is_shop_div)
	This.Trigger Event ue_title()	
end if	
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_42022_d","0")
end event

type cb_close from w_com010_d`cb_close within w_42022_d
end type

type cb_delete from w_com010_d`cb_delete within w_42022_d
end type

type cb_insert from w_com010_d`cb_insert within w_42022_d
end type

type cb_retrieve from w_com010_d`cb_retrieve within w_42022_d
end type

type cb_update from w_com010_d`cb_update within w_42022_d
end type

type cb_print from w_com010_d`cb_print within w_42022_d
end type

type cb_preview from w_com010_d`cb_preview within w_42022_d
end type

type gb_button from w_com010_d`gb_button within w_42022_d
end type

type cb_excel from w_com010_d`cb_excel within w_42022_d
end type

type dw_head from w_com010_d`dw_head within w_42022_d
integer y = 160
integer height = 276
string dataobject = "d_42022_h01"
end type
...
type ln_1 from w_com010_d`ln_1 within w_42022_d
end type

type ln_2 from w_com010_d`ln_2 within w_42022_d
end type

type dw_body from w_com010_d`dw_body within w_42022_d
string dataobject = "d_42022_d07"
boolean hscrollbar = true
end type

type dw_print from w_com010_d`dw_print within w_42022_d
string dataobject = "d_42022_r07"
end type
