$PBExportHeader$w_42002_e.srw
$PBExportComments$완불요청등록
forward
global type w_42002_e from w_com010_e
end type
end forward

global type w_42002_e from w_com010_e
end type
global w_42002_e w_42002_e

type variables
DataWindowChild idw_brand, idw_year, idw_season
String is_brand, is_yymmdd, is_fr_ymd, is_to_ymd, is_accept_yn , is_year, is_season
end variables
...
public function boolean wf_margin_set (long al_row);Long    ll_dc_rate,     ll_qty      
Long    ll_curr_price,  ll_out_price,  ll_out_collect
String  ls_sale_type = space(2)
String  ls_shop_cd,     ls_shop_type,  ls_style 
decimal ldc_marjin
...
///* 출고시 마진율 체크 */
//IF gf_out_marjin (is_yymmdd,    ls_shop_cd, ls_shop_type, ls_style, & 
//                  ls_sale_type, ldc_marjin, ll_dc_rate,   ll_curr_price, ll_out_price) = FALSE THEN 
//	RETURN False 
//END IF
//
...
//dw_body.Setitem(al_row, "disc_rate",   ll_dc_rate)
...
//	Return False 
    ...
    where out_ymd     = :ls_out_ymd 
      and deal_seq    = case when :ls_shop_type = "1" then 97
							 		  when :ls_shop_type = "3" then 98
								     else 99 end
...
//proc_yn	yymmdd	out_no	work_no	dps_yn	reg_id	reg_dt	mod_id	mod_dt	rshop_cd
	...
	insert into tb_52031_h
      ...
      values 
	  	    (:is_yymmdd,        case when :ls_shop_type = "1" then 97
							 		  when :ls_shop_type = "3" then 98
								     else 99 end,    
...
if is_brand = 'W' and is_accept_yn = "%"  and gs_user_id = "ASSIST" then
	 MessageBox(ls_title,"승인여부 전체로 조회 할 수 없습니다!")
   dw_head.SetFocus()
   dw_head.SetColumn("accept_yn")
   return false
end if	
...
il_rows = dw_body.retrieve(is_brand, is_fr_ymd, is_to_ymd, is_accept_yn, is_year, is_season, gs_user_id)
IF il_rows > 0 THEN 
...
event ue_print;
dw_print.Dataobject = "d_42002_r01"
dw_print.SetTransObject(SQLCA)
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_42002_e","0")
end event

type cb_close from w_com010_e`cb_close within w_42002_e
end type

type cb_delete from w_com010_e`cb_delete within w_42002_e
boolean visible = false
end type

type cb_insert from w_com010_e`cb_insert within w_42002_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_42002_e
end type

type cb_update from w_com010_e`cb_update within w_42002_e
end type

type cb_print from w_com010_e`cb_print within w_42002_e
end type

type cb_preview from w_com010_e`cb_preview within w_42002_e
integer width = 439
end type

type gb_button from w_com010_e`gb_button within w_42002_e
end type

type cb_excel from w_com010_e`cb_excel within w_42002_e
boolean visible = false
end type

type dw_head from w_com010_e`dw_head within w_42002_e
integer y = 156
integer width = 3378
integer height = 212
string dataobject = "d_42002_h01"
end type
...
if  gl_user_level   = 0  then 
	ls_filter_str = "inter_cd <> '%' " 
	idw_year.SetFilter(ls_filter_str)
	idw_year.Filter( )
end if

if  gl_user_level   = 0  then 
	ls_filter_str = "inter_cd <> '%' " 
	idw_season.SetFilter(ls_filter_str)
	idw_season.Filter( )
end if
...
type ln_1 from w_com010_e`ln_1 within w_42002_e
integer beginy = 380
integer endy = 380
end type

type ln_2 from w_com010_e`ln_2 within w_42002_e
integer beginy = 384
integer endy = 384
end type

type dw_body from w_com010_e`dw_body within w_42002_e
integer x = 18
integer y = 396
integer width = 3547
integer height = 1600
string dataobject = "d_42002_d01"
	...
	CASE "accept_yn" 
			...
			ELSE
   			IF isnull(ll_qty) OR ll_qty = 0 THEN 
	   			This.Setitem(row, "accept_qty", This.GetitemDecimal(row, "reserve_qty"))
		   	END IF 
			END IF
		ELSE
			SetNull(ll_qty)
...
type dw_print from w_com010_e`dw_print within w_42002_e
integer x = 302
integer y = 768
string dataobject = "d_42002_r01"
end type
