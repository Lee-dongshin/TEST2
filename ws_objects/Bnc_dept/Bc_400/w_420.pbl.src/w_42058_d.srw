$PBExportHeader$w_42058_d.srw
$PBExportComments$롯데EDI파일 발주(NEW)
forward
global type w_42058_d from w_com010_d
end type
type st_1 from statictext within w_42058_d
end type
end forward
...
if is_brand = "O" or is_brand = "D" then
	ls_web_id = "Olive_" + string(ld_datetime,"yyyymmdd") + "_" + MidA(is_edi_code,2,3) + ".xls"
else	
	ls_web_id = "beaucre_" + string(ld_datetime,"yyyymmdd") + "_" + MidA(is_edi_code,2,3) + ".xls"
end if
...
IF GetFileSaveName("Select File", ls_web_id, ls_nm, "xls", "Excel Files (*.xls),*.xls") <> 1 THEN
	RETURN
END IF	
//lb_exist = FileExists(ls_web_id + "_tmp")
...
//li_ret = dw_body.SaveasAscii(ls_web_id + "_tmp" , "	","")
...
////엑셀 오픈해서 다시 셀값 덮어 주기 테스트
/////////////////////////////////////////
//
//ll_colnum = long(dw_body.Object.Datawindow.Column.Count) //칼럼의 갯수를 카운트
//ll_rownum = dw_body.RowCount() //전체 Row갯수를 카운트
//
//// OLEObject 타입의 변수 xlApp생성
//xlApp = CREATE OLEObject
//
//// Excel을 실행하고 Sheet를 생성한다.
////li_rtn = xlApp.ConnectToNewObject( "Excel.Sheet" )
//li_rtn = xlApp.ConnectToNewObject( "excel.application" )
//IF li_rtn < 0 THEN
// MessageBox("ConnectToNewObject() Error !", string(li_rtn))
// Return
//else  
//// xlApp.Workbooks.add() // 엑셀파일 추가
//
//SetPointer(Old_pointer)
////xlApp.WorkBooks.Open(ls_web_id)
//xlApp.Application.Workbooks.Open(ls_web_id + "_tmp") 
//END IF
//
////
////// 디폴트로 sheet1이 생성되고  활성화됨
////xlApp.Application.workbooks(1).worksheets(1).Activate
////
////// Sheet명 설정
////xlApp.application.workbooks(1).Worksheets(1).name ="sheet1" 
////
//// Excel 화면 활성화(True=활성화,False:비활성화)
//xlApp.Application.Visible =  False
//
///* Excel WorkSheet의 Reference를 얻는다.
//   이와 같이 하면 스크립트양을 줄일수 있으며 성능을 향상시킬수 있다. */
//xlsub = xlapp.Application.ActiveWorkbook.Worksheets[1]
//
///* Loop를 돌면서 데이타윈도우의 Primary Buffer에 있는
//   데이타들을 Excel Sheet에 저장 */
//FOR c = 1 TO ll_colnum
// FOR r = 1 TO ll_rownum
//  xlsub.cells[r+1,c] = dw_body.object.data[r,c]
// NEXT
//NEXT
//
//// 다른이름으로 SaveAs
//xlApp.Application.Activeworkbook.SaveAs(ls_web_id, -4143)
...
type cb_close from w_com010_d`cb_close within w_42058_d
end type

type cb_delete from w_com010_d`cb_delete within w_42058_d
end type

type cb_insert from w_com010_d`cb_insert within w_42058_d
end type

type cb_retrieve from w_com010_d`cb_retrieve within w_42058_d
end type

type cb_update from w_com010_d`cb_update within w_42058_d
end type

type cb_print from w_com010_d`cb_print within w_42058_d
boolean visible = false
end type

type cb_preview from w_com010_d`cb_preview within w_42058_d
boolean visible = false
end type

type gb_button from w_com010_d`gb_button within w_42058_d
end type

type cb_excel from w_com010_d`cb_excel within w_42058_d
integer x = 23
integer width = 384
string text = "화일생성(&E)"
end type

type dw_head from w_com010_d`dw_head within w_42058_d
integer y = 152
integer width = 4219
integer height = 288
string dataobject = "d_42058_h01"
end type
...
type ln_1 from w_com010_d`ln_1 within w_42058_d
end type

type ln_2 from w_com010_d`ln_2 within w_42058_d
end type

type dw_body from w_com010_d`dw_body within w_42058_d
string dataobject = "d_42058_d02"
end type
...
type dw_print from w_com010_d`dw_print within w_42058_d
end type
