...
type variables
Datawindowchild idw_brand, idw_shop_type, idw_year, idw_season,idw_shop_div, idw_st_brand
String is_brand, is_shop_cd, is_yymmdd, is_shop_type, is_year, is_season, is_shop_div, is_st_brand, is_fr_ymd, is_view_opt
end variables
...
if IsNull(is_shop_div) or Trim(is_shop_div) = "" then
   MessageBox(ls_title,"유통망 구분 조건을 입력하십시요!")
   dw_head.SetFocus()
...
if is_view_opt = "N" and is_year = "%" then
   MessageBox(ls_title,"기간조회에서만 전체년도 조회 가능합니다. 년도(% 전체 불가)를 입력하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("year")
   return false
end if
...
if is_view_opt = "Y" then
	dw_body.DataObject = "d_43036_d12"
	dw_body.SetTransObject(SQLCA)
	dw_print.DataObject = "d_43036_r12"
	dw_print.SetTransObject(SQLCA)
	
	il_rows = dw_body.retrieve(is_fr_ymd, is_yymmdd, is_brand, is_shop_cd, is_shop_type, is_shop_div, is_year, is_season, '%','%', is_st_brand)	
else 	
	dw_body.DataObject = "d_43036_d02"
	dw_body.SetTransObject(SQLCA)
	dw_print.DataObject = "d_43036_r02"
	dw_print.SetTransObject(SQLCA)
	
	il_rows = dw_body.retrieve(is_yymmdd, is_brand, is_shop_cd, is_shop_type, is_shop_div, is_year, is_season, '%','%', is_st_brand)	
end if
...
string ls_modify, ls_datetime, ls_shop_nm, ls_yymmdd
...
integer width = 3584
integer height = 1612
string dataobject = "d_43036_d02"
end type

type dw_print from w_com010_d`dw_print within w_43036_d
string dataobject = "d_43036_r02"
end type
