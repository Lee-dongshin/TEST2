...
type cb_del from commandbutton within w_47001_e
end type
end forward
...
string ls_order_email, ls_receiver_name, ls_receiver_mobile	,ls_receiver_tel	,ls_style, ls_chno, ls_color, ls_size	
string ls_sale_ymd, ls_invoice_no, ls_prod_no, ls_settle_ymd, ls_tran_ymd,ls_style_chk	, ls_db, ls_order_no_sub,ls_invoice_no_rtn, ls_db_original, ls_order_no_sub1, ls_order_no_sub2
long ll_qty	
...
//				ls_color = mid(Trim(Mid(ls_data, 1,li_find - 1)),1,2)
			   ...
			   if ls_style + ls_chno = "GX3XX304G" and ls_ord_ymd >= "20130603"  then 
				...
				//부주문번호 37
				li_find = PosA(ls_data,",")
//				if ( ls_shop_cd = 'ND1970' or ls_shop_cd = 'JD1970' ) then 
//							messagebox("ls_order_no_sub", ls_data)
//						end if
				ls_order_no_sub = Trim(MidA(ls_data, 1,li_find - 1))
				ls_data =MidA(ls_data,li_find+1,600)		
				
				//브랜드명 38 올리브 부주문번호 OE2303, OE2207
				li_find = PosA(ls_data,",")
				ls_order_no_sub1 = Trim(MidA(ls_data, 1,li_find - 1))
				ls_data =MidA(ls_data,li_find+1,600)				
				
				//쇼핑몰id 39 올리브 부주문번호 OE2301, OE2305
				li_find = PosA(ls_data,",")
				ls_order_no_sub2 = Trim(MidA(ls_data, 1,li_find - 1))
				ls_data =MidA(ls_data,li_find+1,600)
...
//				dw_head.setitem(1, "yymmdd",  ls_yymmdd)			
				...
				// 유센틱 품번 OE3%은 유센틱 온라인에서만 판매
				if is_yymmdd >= "20240102" then
					select dbo.sf_eshop_cd(:ls_cust_nm, CASE WHEN substring(:ls_style,1,3) = 'OE3' THEN 'A' ELSE  substring(:ls_style,1,1) END)
					into :ls_shop_cd
					from dual;
				else	
...
//				ls_style = mid(Trim(Mid(ls_data, 1,li_find - 1)),1,8)
...
//						if ( ls_shop_cd = 'OE2305' or ls_shop_cd = 'OE2305' ) then 
//							messagebox("ls_order_no", (len(trim(ls_order_no_sub2))))
//							messagebox("ls_order_no", ls_order_no_sub2)
//							messagebox("ls_order_no", (len(trim(ls_order_no_sub2))))
//						end if
						
						if ( ls_shop_cd = 'ND1970' or ls_shop_cd = 'JD1970' or ls_shop_cd = 'OD1970' or ls_shop_cd = 'OD1973'  or ls_shop_cd = 'OD1975'  or ls_shop_cd = 'DD1974' or ls_shop_cd = 'AD1975'  or ls_shop_cd = 'GD190F' or ls_shop_cd = 'GD190C' ) and LenA(trim(ls_order_no_sub)) >= LenA(trim(ls_order_no))  then
							dw_body.setitem(il_rows, "order_no",	ls_order_no_sub	) //ls_order_no_sub
							
						elseif ( ls_shop_cd = 'OE2303' or ls_shop_cd = 'DE2340' or ls_shop_cd = 'AE2321' or ls_shop_cd = 'OE2207' or ls_shop_cd = 'DE2307' or ls_shop_cd = 'AE2330' or ls_shop_cd = 'DE1931'or ls_shop_cd = 'OE2304' or ls_shop_cd = 'DE0280'  or ls_shop_cd = 'DE2315'   or ls_shop_cd = 'DE2313' or ls_shop_cd = 'OE2312' or ls_shop_cd = 'DE2314' or ls_shop_cd = 'AD2321'  ) and LenA(trim(ls_order_no_sub1)) >= LenA(trim(ls_order_no))  then
							dw_body.setitem(il_rows, "order_no",	ls_order_no_sub1	) //ls_order_no_sub1 
							
					elseif ( ls_shop_cd = 'OE2301' or  ls_shop_cd = 'DE0288' or  ls_shop_cd = 'OE0315' or ls_shop_cd = 'DE2309' or ls_shop_cd = 'OE2305' or ls_shop_cd = 'AE2330' or ls_shop_cd = 'OE2322'  or ls_shop_cd = 'OE0286'   or ls_shop_cd = 'AD2320' or ls_shop_cd = 'OE2323' or ls_shop_cd = 'OE0342' ) and LenA(trim(ls_order_no_sub2)) >= LenA(trim(ls_order_no))  then //or ls_shop_cd = 'OE2320'  or ls_shop_cd = 'AE2320'
							dw_body.setitem(il_rows, "order_no",	ls_order_no_sub2	) //ls_order_no_sub2							
						else 
							dw_body.setitem(il_rows, "order_no",	ls_order_no	) //ls_order_no_sub
						end if	
		...
		//				dw_body.setitem(il_rows, "sale_ymd",	sale_ymd	)
						...
						if MidA(ls_shop_cd ,1,1) = "D" AND MidA(ls_style,1,1) <> "D" THEN
							 ls_style_chk = "품번오류"
						END IF
						
						dw_body.Setitem(il_rows, "style_chk", ls_style_chk)
						
						if MidA(ls_style,1,1) = "V" or MidA(ls_style,1,1) = "B" or MidA(ls_style,1,1) = "F" or MidA(ls_style,1,1) = "L" OR MidA(ls_style,1,1) = "G" then
									dw_body.Setitem(il_rows, "style_chk", "이터널")
						End if		
...
string text = "* 파일을 등록하기전 파일의 각항목에 쉼표( , ) 가 제거되었는지 확인하세요!"
...
fontfamily fontfamily = modern!
string facename = "굴림체"
string text = "신규주문삭제(보끄레-배분용)"
end type

event clicked;long Net
...
Net = MessageBox("경고", is_yymmdd + "일자로 등록된 신규등록 데이터가 삭제됩니다. 계속하시겠습니까?", Exclamation!, OKCancel!, 2)
