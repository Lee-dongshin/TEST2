...
String is_order_ymd, is_shop_cd, is_order_no, is_order_name, is_order_mobile, is_proc_stat, is_fr_ymd, is_to_ymd, is_gubn, is_corp
end variables
...
is_corp = dw_head.GetItemString(1, "corp")
if IsNull(is_corp) or Trim(is_corp) = "" then
   MessageBox(ls_title,"사업장을 입력하십시요!")
   dw_head.SetFocus()
   dw_head.SetColumn("corp")
   return false
end if


if is_shop_cd = "%" and is_corp = "B" then 
	is_shop_cd = "[NJEMG]%" 
elseif is_shop_cd = "%" and is_corp = "O" then 
	is_shop_cd = "[ODA]%" 	
else 
...
string ls_sale_no, ls_s_no, ls_s_shop_type, ls_chk_corp, ls_db_no
				...
				if ls_chk_corp = "O" then 
					 ...
					 DECLARE sp_47003_p01 PROCEDURE FOR sp_47003_p01  
						...
						// 20221031 DB번호 추가ㅣ
						 ...
						 fetch   sp_47003_p03  into :ls_sale_ymd, :ldc_sale_price, :ls_sale_no, :ls_s_no, :ls_ok, :ls_s_shop_type;
						 CLOSE   sp_47003_p03 ;
