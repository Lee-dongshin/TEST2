$PBExportHeader$w_47032_e.srw
$PBExportComments$판매보충 수기배분_코인몰
forward
global type w_47032_e from w_com010_e
end type
type cb_refill from commandbutton within w_47032_e
end type
type st_1 from statictext within w_47032_e
end type
type st_2 from statictext within w_47032_e
end type
end forward
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = DataModified! THEN		/* Modify Record */
      ...
      ll_new_qty = dw_body.GetitemNumber(i, "deal_qty")
		...
		ls_shop_div = Mid(dw_body.GetitemString(i, "shop_cd"), 2, 1)
		//IF gf_stresv_update (ls_style, ls_chno, ls_color, ls_size, ls_shop_div, ll_new_qty - ll_old_qty, ls_ErrMsg) = FALSE THEN 
		IF gf_stresv_update (ls_style, ls_chno, ls_color, ls_size, ls_shop_div, ll_new_qty - ll_old_qty, 'X', ls_ErrMsg) = FALSE THEN 		//Mig 에러 : 매개변수 as_house 위치에 'X' 추가 
			il_rows = -1
			EXIT
		END IF
   END IF
...
type cb_close from w_com010_e`cb_close within w_47032_e
end type

type cb_delete from w_com010_e`cb_delete within w_47032_e
boolean visible = false
end type

type cb_insert from w_com010_e`cb_insert within w_47032_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_47032_e
end type

type cb_update from w_com010_e`cb_update within w_47032_e
end type

type cb_print from w_com010_e`cb_print within w_47032_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_47032_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_47032_e
end type

type cb_excel from w_com010_e`cb_excel within w_47032_e
boolean visible = false
end type

type dw_head from w_com010_e`dw_head within w_47032_e
integer y = 156
integer width = 3415
integer height = 200
string dataobject = "d_47032_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_47032_e
integer beginy = 364
integer endy = 364
end type

type ln_2 from w_com010_e`ln_2 within w_47032_e
integer beginy = 368
integer endy = 368
end type

type dw_body from w_com010_e`dw_body within w_47032_e
integer x = 0
integer y = 372
integer width = 3611
integer height = 1648
string dataobject = "d_47032_d01"
...
type dw_print from w_com010_e`dw_print within w_47032_e
integer x = 2345
integer y = 760
end type
...
DECLARE SP_REFILL_MID_COINMALL PROCEDURE FOR SP_REFILL_MID_COINMALL  
        @brand    = :is_brand, 
        @yymmdd   = :is_yymmdd,    
        @fr_ymd   = :is_fr_ymd, 
        @to_ymd   = :is_to_ymd, 
...
EXECUTE SP_REFILL_MID_COINMALL;
