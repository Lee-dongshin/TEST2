$PBExportHeader$w_22015_e.srw
$PBExportComments$원자재 검단승인
forward
global type w_22015_e from w_com010_e
end type
type dw_1 from datawindow within w_22015_e
end type
end forward
...
/* 수정일      : 2001..                                                  */
/*===========================================================================*/
long i, ll_row_count, ll_in_no = 0, j = 0, ll_in_no1, ll_in_no2
decimal ll_defect
string ls_in_no,  ls_mat_year, ls_mat_season, ls_mat_sojae, ls_mat_chno, ls_out_seq, ls_in_ymd
...
is_yymmdd = dw_body.getitemstring(1,"in_ymd")
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified! THEN				/* New Record */
		...
		if is_yymmdd <> ls_in_ymd then 
			select cast(max(in_no) as int) into :ll_in_no from tb_22011_d a(nolock)
			where in_ymd = :ls_in_ymd;
		else 
			ll_in_no = ll_in_no1
		end if	
...
event ue_insert();call super::ue_insert;string ls_spec, ls_in_ymd, ls_gaip_ymd
			...
			ls_in_ymd = dw_body.getitemstring(il_rows - 1,"in_ymd")
			dw_body.setitem(il_rows,"in_ymd",ls_in_ymd)	
			
			ls_gaip_ymd = dw_body.getitemstring(il_rows - 1,"gaip_ymd")
			dw_body.setitem(il_rows,"gaip_ymd",ls_gaip_ymd)				
...
il_rows = dw_print.retrieve(is_mat_cd, is_color)
dw_print.inv_printpreview.of_SetZoom()

end event

event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_22015_e","0")
end event

type cb_close from w_com010_e`cb_close within w_22015_e
end type

type cb_delete from w_com010_e`cb_delete within w_22015_e
boolean visible = false
end type

type cb_insert from w_com010_e`cb_insert within w_22015_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_22015_e
end type

type cb_update from w_com010_e`cb_update within w_22015_e
end type

type cb_print from w_com010_e`cb_print within w_22015_e
end type

type cb_preview from w_com010_e`cb_preview within w_22015_e
end type

type gb_button from w_com010_e`gb_button within w_22015_e
end type

type cb_excel from w_com010_e`cb_excel within w_22015_e
end type

type dw_head from w_com010_e`dw_head within w_22015_e
string dataobject = "d_22006_h01"
end type
...
type ln_1 from w_com010_e`ln_1 within w_22015_e
end type

type ln_2 from w_com010_e`ln_2 within w_22015_e
end type

type dw_body from w_com010_e`dw_body within w_22015_e
integer y = 844
integer height = 1196
string dataobject = "d_22015_d01"
...
//		IF ib_itemchanged THEN RETURN 1
//		ll_tmp_qty = this.getitemnumber(row,"tmp_qty")
//		if isnull(ll_tmp_qty) or ll_tmp_qty = 0 then
//			return Parent.Trigger Event ue_Popup(dwo.name, row, data, 1)
//		end if 
END CHOOSE
...
type dw_print from w_com010_e`dw_print within w_22015_e
integer x = 699
integer y = 324
string dataobject = "d_22015_r01"
end type
