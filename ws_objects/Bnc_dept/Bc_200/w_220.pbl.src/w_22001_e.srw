...
type variables
String is_mat_cd 
String is_brand, is_year, is_season, is_sojae 
...
CHOOSE CASE as_column
	...
	CASE "cust_cd"				
		   ...
		   gst_cd.ai_div          = ai_div
			...
			gst_cd.default_where   = "Where brand     = case when '" + is_brand + "' in ('J','T','W','C','S','I','E','M') then 'N' "      + &
...
il_rows = dw_body.retrieve(is_brand, is_mat_cd, is_in_ymd, is_in_gubn, is_cust_cd, 'Dat')
IF il_rows > 0 THEN
	for i = 1 to il_rows
...
il_rows = dw_body.retrieve(is_brand, is_mat_cd, is_in_ymd, is_in_gubn, is_cust_cd, 'New')
IF il_rows > 0 THEN
	for i = 1 to il_rows
		dw_body.SetItemStatus(i, 0, Primary!,New!)
	next
