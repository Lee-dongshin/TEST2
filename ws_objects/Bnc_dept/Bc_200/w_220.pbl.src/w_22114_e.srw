$PBExportHeader$w_22114_e.srw
$PBExportComments$자재 이체 등록
forward
global type w_22114_e from w_com020_e
end type
end forward
...
if as_cb_div >= '8' then	//d_izene_L01
	...
	is_flag    = dw_list.GetItemString(il_list_row, "flag")	
else
...
il_rows = dw_list.retrieve(is_brand, is_house, is_out_ymd, is_out_gubn, is_mat_cd, is_style, is_chno, is_cust_cd, "Dat", is_iche_mat_cd)
dw_body.Reset()
IF il_rows > 0 THEN
...
FOR i=1 TO dw_body.rowcount()
   ...
   END IF
	...
	IF idw_status = NewModified!  THEN
			...
			ll_qty   = dec(dw_body.getitemnumber(i,"sil_qty"))
			ll_price = dec(dw_body.getitemnumber(i,"price"))
//messagebox("ll_qty",string(ll_qty))
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_22114_e","0")
end event

type cb_close from w_com020_e`cb_close within w_22114_e
end type

type cb_delete from w_com020_e`cb_delete within w_22114_e
boolean enabled = true
end type

type cb_insert from w_com020_e`cb_insert within w_22114_e
boolean enabled = true
end type

type cb_retrieve from w_com020_e`cb_retrieve within w_22114_e
end type

type cb_update from w_com020_e`cb_update within w_22114_e
end type

type cb_print from w_com020_e`cb_print within w_22114_e
end type

type cb_preview from w_com020_e`cb_preview within w_22114_e
end type

type gb_button from w_com020_e`gb_button within w_22114_e
end type

type cb_excel from w_com020_e`cb_excel within w_22114_e
end type

type dw_head from w_com020_e`dw_head within w_22114_e
string dataobject = "d_22114_h01"
end type
...
type ln_1 from w_com020_e`ln_1 within w_22114_e
end type

type ln_2 from w_com020_e`ln_2 within w_22114_e
end type

type dw_list from w_com020_e`dw_list within w_22114_e
event body_reset ( )
integer y = 452
integer height = 1584
string dataobject = "d_22114_l01"
end type
...
il_rows = dw_body.retrieve(is_mat_cd, is_iche_mat_cd, is_out_ymd, is_flag)			
for i = 1 to il_rows			
...
type dw_body from w_com020_e`dw_body within w_22114_e
event ue_set_amt ( long as_row )
event type integer ue_popup ( string as_column,  long al_row,  string as_data,  integer ai_div )
integer y = 452
string dataobject = "d_22114_d01"
...
type st_1 from w_com020_e`st_1 within w_22114_e
integer y = 456
integer height = 1584
end type

type dw_print from w_com020_e`dw_print within w_22114_e
integer x = 32
integer y = 524
