$PBExportHeader$w_22115_e.srw
$PBExportComments$자재 재가공 등록
forward
global type w_22115_e from w_com020_e
end type
end forward

global type w_22115_e from w_com020_e
end type
global w_22115_e w_22115_e

type variables
string is_brand, is_out_ymd, is_house, is_out_gubn, is_style, is_chno, is_mat_cd, is_cust_cd
string is_flag, is_iche_mat_cd, is_io_gubn
...
if is_flag = 'New' and (IsNull(is_cust_cd) or Trim(is_cust_cd) = "")  then
...
//messagebox("is_house",is_house)
//messagebox("is_out_ymd",is_out_ymd)
//messagebox("is_out_gubn",is_io_gubn)
//messagebox("is_mat_cd",is_mat_cd)
//messagebox("is_style",is_style)
//messagebox("is_chno",is_chno)
//messagebox("is_cust_cd",is_cust_cd)


il_rows = dw_list.retrieve(is_brand, is_house, is_out_ymd, is_io_gubn, is_mat_cd, is_style, is_chno, is_cust_cd, "Dat", is_iche_mat_cd)
dw_body.Reset()
IF il_rows > 0 THEN
...
il_rows = dw_list.retrieve(is_brand, is_house, is_out_ymd, is_io_gubn, is_mat_cd, is_style, is_chno, is_cust_cd, "New", is_iche_mat_cd)
dw_body.Reset()
IF il_rows > 0 THEN
   dw_list.setfocus()
...
string ls_brand, ls_mat_cd ,ls_color, ls_iche_mat_cd, t_io_gubn, ls_io_gubn
decimal ll_qty ,ll_price, ll_amt, ll_out_amt_cal
...
FOR i=1 TO dw_body.rowcount()
   ...
   IF idw_status = NewModified! THEN				/* New Record */
			ls_io_gubn = dw_body.getitemstring(i,"io_gubn")
			ll_qty = dw_body.getitemnumber(i,"sil_qty")
			
			ll_out_amt_cal = dw_body.getitemnumber(i,"out_amt_cal")
//			messagebox("out_amt_cal", string(ll_out_amt_cal))
			dw_body.Setitem(i,"out_amt", ll_out_amt_cal)
			
			if ls_io_gubn = "-" and ll_qty > 0 then ll_qty = -1*ll_qty			

			ll_price = dw_body.getitemnumber(i,"price")
			dw_body.Setitem(i, "sil_qty", ll_qty)
...
if il_rows = 1 then
   dw_body.ResetUpdate()
   commit  USING SQLCA;
   dw_list.retrieve(is_brand, is_house, is_out_ymd, is_io_gubn, is_mat_cd, is_style, is_chno, is_cust_cd, "Dat", is_iche_mat_cd)
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_22115_e","0")
end event

type cb_close from w_com020_e`cb_close within w_22115_e
end type

type cb_delete from w_com020_e`cb_delete within w_22115_e
boolean enabled = true
end type

type cb_insert from w_com020_e`cb_insert within w_22115_e
boolean enabled = true
end type

event cb_insert::clicked;call super::clicked;is_flag = 'New'
end event

type cb_retrieve from w_com020_e`cb_retrieve within w_22115_e
end type

event cb_retrieve::clicked;call super::clicked;is_flag = 'Dat'
end event

type cb_update from w_com020_e`cb_update within w_22115_e
end type

type cb_print from w_com020_e`cb_print within w_22115_e
end type

type cb_preview from w_com020_e`cb_preview within w_22115_e
end type

type gb_button from w_com020_e`gb_button within w_22115_e
end type

type cb_excel from w_com020_e`cb_excel within w_22115_e
end type

type dw_head from w_com020_e`dw_head within w_22115_e
string dataobject = "d_22115_h01"
end type
...
CHOOSE CASE dwo.name
	...
	CASE "style","mat_cd","cust_cd"	     //  Popup 검색창이 존재하는 항목 
...
type ln_1 from w_com020_e`ln_1 within w_22115_e
end type

type ln_2 from w_com020_e`ln_2 within w_22115_e
end type

type dw_list from w_com020_e`dw_list within w_22115_e
event body_reset ( )
integer y = 452
integer height = 1552
string dataobject = "d_22115_l01"
end type
...
//	case "02"   
//			dw_body.dataobject = "d_22111_d07"
//	case "03"	
...
//messagebox("is_out_ymd",is_out_ymd)
//messagebox("is_io_gubn",is_io_gubn)
//messagebox("is_flag",is_flag)
//
il_rows = dw_body.retrieve(ls_mat_cd, is_out_ymd, is_flag, is_io_gubn, is_cust_cd)			
for i = 1 to il_rows			
	is_flag = dw_body.getitemstring(i,"flag")
	if is_flag = 'New' then	dw_body.SetItemStatus(i, 0, Primary!, NewModified!)							
next 
...
type dw_body from w_com020_e`dw_body within w_22115_e
event ue_set_amt ( long as_row )
event type integer ue_popup ( string as_column,  long al_row,  string as_data,  integer ai_div )
integer y = 452
integer width = 2770
integer height = 1552
string dataobject = "d_22115_d01"
boolean hscrollbar = true
end type

event dw_body::ue_set_amt(long as_row);decimal ll_qty, ll_price
string ls_io_gubn
ls_io_gubn = this.getitemstring(as_row,"out_gubn")	

IF dw_body.AcceptText() <> 1 THEN RETURN
if ls_io_gubn = '-' then return

if dw_body.dataobject = "d_22101_d07" then
	ll_qty = this.getitemnumber(as_row,"sil_qty")
else
...
type st_1 from w_com020_e`st_1 within w_22115_e
integer y = 456
integer height = 1584
end type

type dw_print from w_com020_e`dw_print within w_22115_e
integer x = 32
integer y = 524
