$PBExportHeader$w_32018_e.srw
$PBExportComments$사전원가계산서(TasseTasse)
forward
global type w_32018_e from w_com010_e
end type
type dw_mast2 from u_dw within w_32018_e
end type
type dw_1 from datawindow within w_32018_e
end type
end forward
...
public function integer wf_12020_set ();Decimal ldc_busi_price, ldc_md_price, ldc_tag_price, ldc_expense, ldc_margin

ldc_md_price   = dw_body.GetItemDecimal(1, "imsi_price")
ldc_tag_price  = dw_body.GetItemDecimal(1, "tag_price")
ldc_expense  = dw_body.GetItemDecimal(1, "expense")
ldc_margin   = dw_body.GetItemDecimal(1, "margin")

  UPDATE TB_12020_M
     SET BUSI_PRICE = :ldc_tag_price,
	      MD_PRICE   = :ldc_tag_price,
...
Decimal ldc_mmat_price1, ldc_smat_price1, ldc_mmat_price2, ldc_smat_price2, ldc_curr_price
...
dw_body.SetItemStatus(1, "curr_price", Primary!, DataModified!)
...
/*===========================================================================*/
string ls_sojae, ls_user_level
ls_sojae = dw_head.getitemstring(1,"style_no")
ls_sojae = MidA(ls_sojae,2,1)

select user_level into :gsv_cd.gs_cd9 from tb_93013_d (nolock) where work_gbn = '1' and person_id = :gs_user_id;
...
//			dw_body.object.tag_price.protect  = 1
			...
			dw_1.modify("md_agree.width  = 114")				
			dw_1.modify("design_agree.width  = 114")				
			dw_1.modify("tag_agree.width  = 114")						
			

			if PosA(gsv_cd.gs_cd9, "1") > 0 then
				dw_1.object.smat_agree.protect = 0
				dw_1.modify("smat_agree.background.color = " + string(rgb(255,0,0)) )
				dw_1.modify("smat_agree.width = 200")
			end if
			
			if PosA(gsv_cd.gs_cd9, "2") > 0 then					
				dw_1.object.make_agree.protect = 0
				dw_1.modify("make_agree.background.color = " + string(rgb(255,0,0)) )
				dw_1.modify("make_agree.width = 200")
			end if

			if PosA(gsv_cd.gs_cd9, "3") > 0 then	
				dw_1.object.busi_agree.protect = 0
				dw_1.modify("busi_agree.background.color = " + string(rgb(255,0,0)) )
				dw_1.modify("busi_agree.width = 200")
			end if

			if PosA(gsv_cd.gs_cd9, "4") > 0 then	
				dw_1.object.md_agree.protect = 0
				dw_1.modify("md_agree.background.color = " + string(rgb(255,0,0)) )
				dw_1.modify("md_agree.width = 200")
			end if

			if PosA(gsv_cd.gs_cd9, "5") > 0 then					
				dw_1.object.design_agree.protect = 0
				dw_1.modify("design_agree.background.color = " + string(rgb(255,0,0)) )
...
//
//			if len(gsv_cd.gs_cd9) > 1 then
//				choose case mid(gsv_cd.gs_cd9,2,1)
//					case "2"
//						dw_1.object.make_agree.protect = 0
//						dw_1.modify("make_agree.background.color = " + string(rgb(255,0,0)) )
//						dw_1.modify("make_agree.width = 200")
...
//			end if
//
//			if len(gsv_cd.gs_cd9) > 2 then
//				choose case mid(gsv_cd.gs_cd9,3,1)
//					case "3"
//						dw_1.object.design_agree.protect = 0	
//						dw_1.modify("design_agree.background.color = " + string(rgb(255,0,0)) )
//						dw_1.modify("design_agree.width = 200")
...
dw_print.object.t_napum.Text = String(dw_body.getitemNumber(1, 'napum_price'), "#,##0")
...
type cb_close from w_com010_e`cb_close within w_32018_e
integer taborder = 120
end type

type cb_delete from w_com010_e`cb_delete within w_32018_e
boolean visible = false
integer taborder = 70
end type

type cb_insert from w_com010_e`cb_insert within w_32018_e
boolean visible = false
integer taborder = 60
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_32018_e
end type

type cb_update from w_com010_e`cb_update within w_32018_e
integer taborder = 110
end type

type cb_print from w_com010_e`cb_print within w_32018_e
integer taborder = 80
end type

type cb_preview from w_com010_e`cb_preview within w_32018_e
integer taborder = 90
end type

type gb_button from w_com010_e`gb_button within w_32018_e
end type

type cb_excel from w_com010_e`cb_excel within w_32018_e
boolean visible = false
integer taborder = 100
end type

type dw_head from w_com010_e`dw_head within w_32018_e
integer y = 164
integer height = 184
...
type ln_1 from w_com010_e`ln_1 within w_32018_e
integer beginy = 356
integer endy = 356
end type

type ln_2 from w_com010_e`ln_2 within w_32018_e
integer beginy = 360
integer endy = 360
end type

type dw_body from w_com010_e`dw_body within w_32018_e
integer y = 368
integer height = 560
integer taborder = 50
...
CHOOSE CASE dwo.name
	CASE "tag_price"	     //  Popup 검색창이 존재하는 항목 
		this.setitem(row, "curr_price" , dec(data))
END CHOOSE

end event

type dw_print from w_com010_e`dw_print within w_32018_e
integer x = 201
integer y = 588
