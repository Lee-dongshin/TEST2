$PBExportHeader$w_32001_e.srw
$PBExportComments$사전 원가계산서 등록
forward
global type w_32001_e from w_com010_e
end type
type dw_mast2 from u_dw within w_32001_e
end type
type dw_1 from datawindow within w_32001_e
end type
type dw_price from datawindow within w_32001_e
end type
type cbx_1 from checkbox within w_32001_e
end type
end forward
...
idc_in_qty = dw_head.GetItemDecimal(1, "in_qty")
...
if gs_brand = 'N' and (MidA(is_style_no,1,1) = 'O' or MidA(is_style_no,1,1) = 'D') then
   ...
   dw_head.SetColumn("Style_no")
   return false
elseif (gs_brand = 'O' or  gs_brand = 'D' or  gs_brand = 'Y' or  gs_brand = 'U')  and (MidA(is_style_no,1,1) = 'N' or MidA(is_style_no,1,1) = 'B' or MidA(is_style_no,1,1) = 'L' or MidA(is_style_no,1,1) = 'F' or MidA(is_style_no,1,1) = 'G' or MidA(is_style_no,1,1) = 'J') then
...
//					RETURN 0
			...
			END IF
			   gst_cd.ai_div          = ai_div
				...
				gst_cd.default_where   = " where brand not in ('T','Y') "		//WHERE TB_11010_M.PART_FG IN ('1', '2', '3') 
...
event open;call super::open;dw_head.setitem(1,"style_no",gsv_cd.gs_cd10)
end event

type cb_close from w_com010_e`cb_close within w_32001_e
integer taborder = 130
end type

type cb_delete from w_com010_e`cb_delete within w_32001_e
boolean visible = false
integer taborder = 80
end type

type cb_insert from w_com010_e`cb_insert within w_32001_e
boolean visible = false
integer taborder = 70
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_32001_e
end type

type cb_update from w_com010_e`cb_update within w_32001_e
integer taborder = 120
end type

type cb_print from w_com010_e`cb_print within w_32001_e
integer taborder = 90
end type

type cb_preview from w_com010_e`cb_preview within w_32001_e
integer taborder = 100
end type

type gb_button from w_com010_e`gb_button within w_32001_e
end type

type cb_excel from w_com010_e`cb_excel within w_32001_e
boolean visible = false
integer taborder = 110
end type

type dw_head from w_com010_e`dw_head within w_32001_e
integer height = 220
...
type ln_1 from w_com010_e`ln_1 within w_32001_e
integer beginy = 424
integer endy = 424
end type

type ln_2 from w_com010_e`ln_2 within w_32001_e
integer beginy = 428
integer endy = 428
end type

type dw_body from w_com010_e`dw_body within w_32001_e
integer x = 14
integer y = 436
integer height = 528
integer taborder = 40
...
String ls_year, ls_brand ,ls_apply_rate
...
type dw_print from w_com010_e`dw_print within w_32001_e
integer x = 0
integer y = 532
