$PBExportHeader$w_32010_e.srw
$PBExportComments$사전 원가계산서 등록(상설용)
forward
global type w_32010_e from w_com010_e
end type
type dw_mast2 from u_dw within w_32010_e
end type
end forward
...
ll_rows2 = dw_mast2.retrieve(is_style_no)
	...
	CASE "style_no"				
			...
			END IF
			   gst_cd.ai_div          = ai_div
				...
				gst_cd.default_where   = " where brand in ('J','Y','V') "		//WHERE TB_11010_M.PART_FG IN ('1', '2', '3') 
...
ls_modify =  "t_style.Text = '" + is_style_no  + "'" + &
             "t_yymmdd.Text = '" + String(is_yymmdd,"@@@@/@@/@@")  + "'" + &
   	   	 ...
   	   	 "t_mat_price.Text = '" + String(dw_body.getitemdecimal(1, 'mat_price'), "#,##0") + "'" + & 
				 ...
				 "t_etc_price.Text = '" + String(dw_body.getitemdecimal(1, 'etc_price'), "#,###") + "'" + &
				 "t_won_price.Text = '" + String(dw_body.getitemdecimal(1, 'won_price'), "#,##0") + "'" + &
				 "t_margin.Text = '" + String(dw_body.getitemdecimal(1, 'margin'), "#,##0") + "'" + &
				 "t_busi_price.Text = '" + String(dw_body.getitemdecimal(1, 'busi_price'), "#,##0") + "'" + &
...
type cb_close from w_com010_e`cb_close within w_32010_e
integer taborder = 120
end type

type cb_delete from w_com010_e`cb_delete within w_32010_e
boolean visible = false
integer taborder = 70
end type

type cb_insert from w_com010_e`cb_insert within w_32010_e
boolean visible = false
integer taborder = 60
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_32010_e
end type

type cb_update from w_com010_e`cb_update within w_32010_e
integer taborder = 110
end type

type cb_print from w_com010_e`cb_print within w_32010_e
integer taborder = 80
end type

type cb_preview from w_com010_e`cb_preview within w_32010_e
integer taborder = 90
end type

type gb_button from w_com010_e`gb_button within w_32010_e
end type

type cb_excel from w_com010_e`cb_excel within w_32010_e
boolean visible = false
integer taborder = 100
end type

type dw_head from w_com010_e`dw_head within w_32010_e
integer height = 220
string dataobject = "d_32010_h01"
...
type ln_1 from w_com010_e`ln_1 within w_32010_e
integer beginy = 424
integer endy = 424
end type

type ln_2 from w_com010_e`ln_2 within w_32010_e
integer beginy = 428
integer endy = 428
end type

type dw_body from w_com010_e`dw_body within w_32010_e
integer y = 1584
integer height = 456
integer taborder = 50
string dataobject = "d_32010_d02"
...
type dw_print from w_com010_e`dw_print within w_32010_e
integer x = 187
integer y = 300
string dataobject = "d_32010_r01"
end type
