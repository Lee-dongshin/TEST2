$PBExportHeader$w_33002_e.srw
$PBExportComments$지급 임가공가 수정
forward
global type w_33002_e from w_com000
end type
type dw_head from u_dw within w_33002_e
end type
type ln_1 from line within w_33002_e
end type
type ln_2 from line within w_33002_e
end type
type dw_body from u_dw within w_33002_e
end type
type dw_print from u_dw within w_33002_e
end type
end forward
...
global w_33002_e w_33002_e

type variables
string is_brand,  is_style, is_chno, is_yymmdd, is_end_ymd, is_cust_cd, is_opt_add
...
IF gf_cdate(ld_datetime,0)  THEN  
	dw_head.setitem(1,"end_ymd",string(ld_datetime,"yyyymmdd"))
end if
...
if isnull(is_style) then 
	setnull(is_style)
	setnull(is_chno)
end if

il_rows = dw_body.retrieve(is_brand, is_style, is_chno, is_yymmdd, is_end_ymd, is_cust_cd, is_opt_add)
IF il_rows > 0 THEN
   dw_body.SetFocus()
END IF
...
is_opt_add = dw_head.GetItemString(1, "opt_add")
is_end_ymd   = dw_head.GetItemString(1, "end_ymd")
...
CHOOSE CASE as_column
	...
	case "style_no"
			...
			IF Isvalid(Message.PowerObjectParm) THEN
				...
				dw_head.SetRow(al_row)
				dw_head.SetColumn(as_column)
				dw_head.SetItem(al_row, "style_no", lds_Source.GetItemString(1,"style")+lds_Source.GetItemString(1,"chno"))
...
il_rows = dw_body.getrow()+1
ls_add_chk = dw_body.getitemstring(il_rows - 1, "add_chk")

if ls_add_chk <> "Y" then dw_body.setitem(il_rows - 1, "add_chk","Y")
dw_body.rowscopy(il_rows - 1, il_rows - 1,Primary!, dw_body, il_rows,Primary!)
dw_body.setitem(il_rows - 1, "add_chk",ls_add_chk)
...
if il_rows > 0 then
	...
	dw_body.selectrow(il_rows, true)
end if
...
FOR i=1 TO ll_row_count
   ...
   IF idw_status = NewModified! THEN				/* New Record */
		...
		dw_body.setitem(i, "no", string(ll_max_no,"0000"))
//		dw_body.setitem(i, "real_make_amt", dw_body.getitemdecimal(i,"t_real_make_amt"))
		dw_body.setitem(i, "amt", dw_body.getitemdecimal(i,"t_amt"))
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_33002_e","0")
end event

type cb_close from w_com000`cb_close within w_33002_e
integer taborder = 100
end type

type cb_delete from w_com000`cb_delete within w_33002_e
integer x = 1083
integer taborder = 50
end type

type cb_insert from w_com000`cb_insert within w_33002_e
integer x = 741
integer taborder = 40
end type

type cb_retrieve from w_com000`cb_retrieve within w_33002_e
integer taborder = 20
end type
...
type cb_update from w_com000`cb_update within w_33002_e
integer taborder = 90
end type

type cb_print from w_com000`cb_print within w_33002_e
integer x = 1426
integer taborder = 60
end type

type cb_preview from w_com000`cb_preview within w_33002_e
integer x = 1769
integer taborder = 70
end type

type gb_button from w_com000`gb_button within w_33002_e
integer taborder = 0
end type

type cb_excel from w_com000`cb_excel within w_33002_e
integer x = 2112
end type
...
type dw_body from u_dw within w_33002_e
event ue_keydown pbm_dwnkey
event real_amt_set ( string ps_name,  long ps_row )
integer x = 5
integer y = 432
integer width = 3589
integer height = 1608
integer taborder = 30
string dataobject = "d_33002_d01"
...
event real_amt_set(string ps_name, long ps_row);decimal	ldc_qty, ldc_dc_rate, ldc_make_price, ldc_real_make_price, ldc_real_make_amt, ldc_t_real_make_amt

ldc_qty 					= this.getitemdecimal(ps_row,"qty")
ldc_dc_rate 			= this.getitemdecimal(ps_row,"dc_rate")
ldc_make_price  		= this.getitemdecimal(ps_row,"make_price")
ldc_real_make_price  = this.getitemdecimal(ps_row,"t_real_make_price")
ldc_t_real_make_amt 	= this.getitemdecimal(ps_row,"t_real_make_amt")
ldc_real_make_amt 	= this.getitemdecimal(ps_row,"real_make_amt")

choose case ps_name 
	case "qty", "dc_rate"
		this.setitem(ps_row,"real_make_amt", ldc_t_real_make_amt)
		
	case "real_make_amt"
		ldc_dc_rate = 100 - (100 * ldc_real_make_amt) / (ldc_qty * ldc_make_price)
		this.setitem(ps_row,"dc_rate", ldc_dc_rate  )
...
/*===========================================================================*/
string ls_column_nm, ls_column_value, ls_report
decimal ldc_cal_real_make_amt

if dwo.name = "b_refresh" then
	ldc_cal_real_make_amt = this.getitemdecimal(row,"cal_real_make_amt")
	this.setitem(row,"real_make_amt",ldc_cal_real_make_amt)
	cb_update.enabled = true
end if
...
post event real_amt_set(dwo.name,row)
end event
