...
if ls_syear >= "20133"  then
	...
	ls_brand1 = ls_brand
				...
				ldw_child.Retrieve('%',ls_brand1)
				ldw_child.SetRedraw(false)
				if ls_brand1 = "P" or ls_brand1 = "B" or ls_brand1 = "K" or  ls_brand1 = 'L' or  ls_brand1 = 'H' or  ls_brand1 = 'U' or  ls_brand1 = 'G'  then
				else
...
IF ls_syear <= '20134' and ls_brand = 'B' or ls_brand = 'P'  then
	IF gf_sojae_nm(ls_sojae, ls_nm) <> 0 THEN Return False 
elseIF ls_syear >= '20173' and ls_brand = 'B'  then
...
//		messagebox("ll_fill",string(ll_fill,"0000"))
//		dw_body.Setitem(ll_row, "fill_weight",   100)		
...
this.Control[iCurrent+8]=this.cb_sketch
this.Control[iCurrent+9]=this.cb_copy_style
this.Control[iCurrent+10]=this.dw_7
this.Control[iCurrent+11]=this.dw_copy_style
this.Control[iCurrent+12]=this.dw_9
this.Control[iCurrent+13]=this.dw_10
this.Control[iCurrent+14]=this.cbx_pop
end on
...
if gs_brand = 'N' and (MidA(is_style,1,1) = 'O' or MidA(is_style,1,1) = 'D' or MidA(is_style,1,1) = 'Y') then
   ...
   dw_head.SetColumn("Style_no")
   return false
elseif  ( gs_brand = 'O' or  gs_brand = 'D' or  gs_brand = 'Y' or  gs_brand = 'U')  and (MidA(is_style,1,1) = 'N' or MidA(is_style,1,1) = 'M' or MidA(is_style,1,1) = 'E' or MidA(is_style,1,1) = 'J' or MidA(is_style,1,1) = 'G'  or MidA(is_style,1,1) = 'J') then
   MessageBox(ls_title,"올리브 직원은 보끄레 데이터 조회에 제한이 있습니다.!")
   dw_head.SetFocus()
   dw_head.SetColumn("Style_no")
   return false	
elseif gs_brand = 'Y' and (MidA(is_style,1,1) = 'N' or MidA(is_style,1,1) = 'M' or MidA(is_style,1,1) = 'E' or MidA(is_style,1,1) = 'J' or MidA(is_style,1,1) = 'G'  or MidA(is_style,1,1) = 'J') then
   MessageBox(ls_title,"올리브 직원은 보끄레 데이터 조회에 제한이 있습니다.!")
   dw_head.SetFocus()
   dw_head.SetColumn("Style_no")
   return false		
...
ELSEIF il_rows = 0 THEN 
 ...
 //  dw_master.Setitem(1, "cost_year",    is_year)	
	IF MidA(is_style, 5, 1) = 'Z' or MidA(is_style, 2, 1) = 'Y' THEN
     dw_master.Setitem(1, "plan_yn", "Y") 
	elseif is_brand = "N" and is_sojae = "H" and is_year >= "2019" then
     dw_master.Setitem(1, "plan_yn", "Y") 		
	elseif is_brand = "N" and is_sojae = "E" and is_year >= "2018" then
     dw_master.Setitem(1, "plan_yn", "Y") 			  
   ELSE
...
datetime ld_datetime
String   ls_mat_cd, ls_mat_nm, ls_mat_nm_chk, ls_ErrMsg, ls_flag, ls_mat_fg, ls_brand,ls_dsgn_emp, ls_cost_year,ls_cost_season
...
ls_cost_season = dw_master.GetItemString(1, "cost_season")
	if ls_cost_season = '' or isnull(ls_cost_season) then 
		ls_cost_season = is_SEASON
	end if	
...
if ls_brand = "V" or ls_brand = "B" or ls_brand = "F" or ls_brand = "L" then
			messagebox("주의", "이터널 브랜드의 경우 이터널영업관리를 이용하세요!")
			return -1
End if		

if is_sojae = 'C' or is_sojae = 'D' or is_sojae = 'E' then 
	if ls_brand = "Y" or ls_brand = "B" or ls_brand = "D" or ls_brand = "L" or ls_brand = "H" or ls_brand = "U" or ls_brand = "K" or ls_brand = "S" or ls_brand = "V" then
	else	
...
//				messagebox("ll_size_spec4",ll_size_spec4)				
//		
		
		
			if ls_brand = "P" or ls_brand = "B" or ls_brand = "D" or ls_brand = "L" or ls_brand = 'G' then
			else	
...
//				messagebox("ll_size_spec2",ll_size_spec2)
				...
				tab_1.SelectedTab = 2
				tab_1.tabpage_2.dw_2.setrow(22)
	...
	//			messagebox("ll_size_spec3",ll_size_spec3)
			...
			elseif is_item = 'A' and ll_size_spec4 <= 1  then 			
				...
				tab_1.SelectedTab = 2
				tab_1.tabpage_2.dw_2.setrow(25)
				tab_1.tabpage_2.dw_2.SetFocus()
				tab_1.tabpage_2.dw_2.SetColumn("size_spec_1")	
				return -1				
			end if	
...
IF i > 0 THEN  
   ...
   dw_master.Setitem(1, "mat_cd", ls_mat_cd) 
	ls_mat_nm_chk = dw_master.GetitemString(1, "mat_nm") 
...
//	dw_detail.Setitem(1, "cost_season",   ls_cost_season) 				
...
This.GetChild("cost_season", ldw_child)
ldw_child.SetTransObject(SQLCA)
ldw_child.Retrieve('003', gs_brand, '%')
...
//			Return 1   //20031101 부로 가능량에 상관없이 요척가능(이명진)
		...
		IF ldc_bf_tneed + ldc_stock_qty - ldc_need_qty < Dec(Data) AND &
		   ldc_bf_tneed + ldc_stock_qty - ldc_need_qty > 0  AND is_brand <> "M" THEN
		   ldc_loss = ldc_bf_tneed + ldc_stock_qty - ldc_need_qty
		ELSE 
			ldc_loss = 0 
		END IF
...
CHOOSE CASE as_column
	CASE "mat_cd"				
			IF ai_div = 1 THEN 	
				IF gf_mat_nm(as_data, ls_mat_nm) = 0 and is_brand = MidA(as_data,1,1) THEN
					This.Setitem(al_row, "mat_nm", ls_mat_nm)
					This.Setitem(al_row, "mat_color", ls_null)
					RETURN 0
				END IF 
