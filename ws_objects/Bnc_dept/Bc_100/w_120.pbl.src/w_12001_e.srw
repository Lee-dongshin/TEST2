$PBExportHeader$w_12001_e.srw
$PBExportComments$원자재 구매의뢰등록
forward
global type w_12001_e from w_com010_e
end type
type dw_master from datawindow within w_12001_e
end type
type dw_rate from datawindow within w_12001_e
end type
type dw_model from datawindow within w_12001_e
end type
type dw_1 from datawindow within w_12001_e
end type
type dw_copy_mat from datawindow within w_12001_e
end type
type cb_copy_mat from commandbutton within w_12001_e
end type
end forward
...
dw_model dw_model
dw_1 dw_1
dw_copy_mat dw_copy_mat
cb_copy_mat cb_copy_mat
end type
global w_12001_e w_12001_e
...
if gs_brand = "K" then
  if is_brand <> "K" then Return False 
end if	
...
this.dw_copy_mat=create dw_copy_mat
this.cb_copy_mat=create cb_copy_mat
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.dw_master
this.Control[iCurrent+2]=this.dw_rate
this.Control[iCurrent+3]=this.dw_model
this.Control[iCurrent+4]=this.dw_1
this.Control[iCurrent+5]=this.dw_copy_mat
this.Control[iCurrent+6]=this.cb_copy_mat
end on

on w_12001_e.destroy
call super::destroy
...
destroy(this.dw_1)
destroy(this.dw_copy_mat)
destroy(this.cb_copy_mat)
end on
...
//			elseif  left(is_mat_cd ,1) = 'O' or left(is_mat_cd ,1) = 'Y' then
...
/* 수정일      : 2002.01.04                                                  */
/*===========================================================================*/
string ls_user_level, ls_brand

/* dw_head 필수입력 column check */
IF Trigger Event ue_keycheck('1') = FALSE THEN RETURN

ls_brand = LeftA(is_mat_cd,1)

dw_body.retrieve(is_mat_cd)

if ls_brand = 'O' or ls_brand = 'D' then
	
	select '999' into :ls_user_level
	from tb_93013_d(nolock) 	
	where person_id = :gs_user_id
	and   work_gbn = '1'
	and 	user_level like '%8%';
//	and   ctrl_brand like '%'+:ls_brand+'%';
	
//messagebox("",	ls_user_level)
	
	if ls_user_level = '999' then 
		dw_body.object.nego_price.protect = 0
		dw_body.object.ord_qty.protect = 0		
	else
		dw_body.object.nego_price.protect = 1
		dw_body.object.ord_qty.protect = 1				
	end if
end if
...
il_rows = dw_master.retrieve(is_mat_cd)
IF il_rows = 0 THEN 
...
if gs_brand = 'N' and (MidA(is_mat_cd,1,1) = 'O' or MidA(is_mat_cd,1,1) = 'D' or MidA(is_mat_cd,1,1) = 'Y') then
   MessageBox(ls_title,"보끄레 직원은 올리브 데이터 조회에 제한이 있습니다.!")
   dw_head.SetFocus()
   dw_head.SetColumn("mat_cd")
   return false
elseif  ( gs_brand = 'O' or  gs_brand = 'D' or  gs_brand = 'Y' or  gs_brand = 'U')  and (MidA(is_mat_cd,1,1) = 'N' or MidA(is_mat_cd,1,1) = 'M' or MidA(is_mat_cd,1,1) = 'E' or MidA(is_mat_cd,1,1) = 'F' or MidA(is_mat_cd,1,1) = 'G'  or MidA(is_mat_cd,1,1) = 'J' ) then
   MessageBox(ls_title,"올리브 직원은 보끄레 데이터 조회에 제한이 있습니다.!")
   dw_head.SetFocus()
   dw_head.SetColumn("mat_cd")
   return false	
elseif gs_brand = 'Y' and (MidA(is_mat_cd,1,1) = 'N' or MidA(is_mat_cd,1,1) = 'M' or MidA(is_mat_cd,1,1) = 'E' or MidA(is_mat_cd,1,1) = 'F' or MidA(is_mat_cd,1,1) = 'G'  or MidA(is_mat_cd,1,1) = 'J' ) then
   MessageBox(ls_title,"올리브 직원은 보끄레 데이터 조회에 제한이 있습니다.!")
   dw_head.SetFocus()
   dw_head.SetColumn("mat_cd")
   return false		
elseif gs_brand = 'B' and (MidA(is_mat_cd,1,1) = 'O' or MidA(is_mat_cd,1,1) = 'D') then
   ...
   dw_head.SetColumn("mat_cd")
   return false		
//elseif (mid(is_mat_cd,1,1) = 'O' or mid(is_mat_cd,1,1) = 'D') then
//   MessageBox(ls_title,"이터널 직원은 올리브 데이터 조회에 제한이 있습니다.!")
//   dw_head.SetFocus()
//   dw_head.SetColumn("mat_cd")
//   return false			
...
For i = 1 TO ll_row 
   ...
   ldc_in_qty  = dw_body.GetitemDecimal(i, "in_qty") 
	IF ldc_ord_qty <> 0 OR ldc_in_qty <> 0 THEN 
...
if MidA(is_mat_cd,1,1) = "N" or MidA(is_mat_cd,1,1) = "E"  or MidA(is_mat_cd,1,1) = "M"  then
	dw_print.DataObject = 'd_12001_r01_on'
elseif MidA(is_mat_cd,1,1) = "J"  then	
	dw_print.DataObject = 'd_12001_r01_jon'	
elseif MidA(is_mat_cd,1,1) = "O"  then
	dw_print.DataObject = 'd_12001_r01_olive'	
else	
	dw_print.DataObject = 'd_12001_r01'
end if		
...
//if mid(is_mat_cd,1,1) = "N" or mid(is_mat_cd,1,1) = "J" then
//	dw_print.DataObject = 'd_12001_r01_on'
//else	
//	dw_print.DataObject = 'd_12001_r01'
//end if	
...
IF dw_print.RowCount() = 0 Then
   ...
   il_rows = 0
ELSE
	dw_print.Modify("DataWindow.Print.Copies = 1")
...
event pfc_close();call super::pfc_close;gf_user_connect_pgm(gs_user_id,"w_12001_e","0")
end event

type cb_close from w_com010_e`cb_close within w_12001_e
integer taborder = 140
end type

type cb_delete from w_com010_e`cb_delete within w_12001_e
integer x = 1047
integer width = 384
integer taborder = 90
string text = "전체삭제(&D)"
...
type cb_insert from w_com010_e`cb_insert within w_12001_e
boolean visible = false
integer taborder = 70
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_12001_e
end type

type cb_update from w_com010_e`cb_update within w_12001_e
integer taborder = 130
end type

type cb_print from w_com010_e`cb_print within w_12001_e
integer taborder = 100
end type

type cb_preview from w_com010_e`cb_preview within w_12001_e
integer taborder = 110
end type

type gb_button from w_com010_e`gb_button within w_12001_e
end type

type cb_excel from w_com010_e`cb_excel within w_12001_e
boolean visible = false
integer taborder = 120
end type

type dw_head from w_com010_e`dw_head within w_12001_e
integer y = 164
integer height = 152
...
type ln_1 from w_com010_e`ln_1 within w_12001_e
integer beginy = 316
integer endy = 316
end type

type ln_2 from w_com010_e`ln_2 within w_12001_e
integer beginy = 320
integer endy = 320
end type

type dw_body from w_com010_e`dw_body within w_12001_e
integer x = 1056
integer y = 1540
integer width = 2551
integer height = 508
...
IF dwo.name = "b_delete" THEN 
	ldc_ord_qty = This.GetitemDecimal(this.getrow(), "ord_qty")
	ldc_in_qty  = This.GetitemDecimal(this.getrow(), "in_qty") 
	IF ldc_ord_qty <> 0 or ldc_in_qty <> 0 THEN 
		MessageBox("경고", "삭제할수 없는 자료입니다")
	ELSE
      idw_status = This.GetItemStatus (this.getrow(), 0, primary!)	
		il_rows = This.DeleteRow(this.getrow()) 
...
type dw_print from w_com010_e`dw_print within w_12001_e
integer y = 232
integer height = 360
string dataobject = "d_12001_r01_on"
end type
...
string title = "원자재 발주서 복사"
string dataobject = "d_12001_d05"
boolean controlmenu = true
boolean resizable = true
boolean livescroll = true
end type

event buttonclicked;string ls_fr_mat_cd, ls_to_mat_cd 


choose case dwo.name
	case "cb_copy_mat"
			IF dw_copy_mat.AcceptText() <> 1 THEN RETURN -1
			
			ls_fr_mat_cd = dw_copy_mat.getitemstring(1,"fr_mat_cd")
			ls_to_mat_cd = dw_copy_mat.getitemstring(1,"to_mat_cd")
		
			if not wf_mat_chk(ls_fr_mat_cd) or not wf_mat_chk(ls_to_mat_cd) then 
			...
			DECLARE sp_copy_mat PROCEDURE FOR sp_copy_mat  
					@fr_mat_cd	= :ls_fr_mat_cd,
					@to_mat_cd	= :ls_to_mat_cd,
					@mod_id		= :gs_user_id;
...
string facename = "굴림체"
boolean enabled = false
string text = "원자재 발주서 복사"
end type

event clicked;
		dw_copy_mat.reset()
		dw_copy_mat.insertrow(0)
		dw_copy_mat.setitem(1,"fr_mat_cd", dw_head.getitemstring(1,"mat_cd"))
		dw_copy_mat.visible = true
		dw_copy_mat.setcolumn("to_mat_cd")
		dw_copy_mat.setfocus()
end event
