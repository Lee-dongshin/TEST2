...
event type boolean ue_st_info_set(string as_sample_cd, long al_row);	string ls_year 
	...
	if	isnumber(MidA(as_sample_cd, 3, 1)) = false  then
		messagebox("알림!", "세번째 자리는 년도로 숫자로 입력하세요!" )
		return false
	end if	

	if	isnumber(MidA(as_sample_cd, 1, 1)) = true  then
		messagebox("알림!", "첫번째 자리는 브랜드 구분이므로 알파벳으로 입력하세요!" )
		return false
	end if	

//	ls_year = '202' + mid(as_sample_cd,3,1)
	
	select dbo.sf_inter_cd1('002', substring(:as_sample_cd, 3, 1))
	into :ls_year
	from dual;
...
//				gst_cd.default_where   = "where goout_gubn = '1' and dept_code in ('" + ls_dept + "','B200','O200','O100','B100','T000','K100','O000')" /* 니트 , 악세라시 추가 */  
				...
				gst_cd.default_where   = "where dept_cd in (CASE WHEN '" + gs_brand + "' = 'N' THEN 'N600' ELSE 'O000' END ,CASE WHEN '" + gs_brand + "' = 'N' THEN 'N610' ELSE 'O000' END , CASE WHEN '" + gs_brand + "' = 'N' THEN 'N610' ELSE 'O000' END , CASE WHEN'" + gs_brand + "' = 'N' THEN 'B000' ELSE 'O000' END ,'B200','O200','O100','T000','K100','K400','F220')"
//				gst_cd.default_where   = "where dept_cd in (CASE WHEN '" + gs_brand + "' = 'N' THEN 'A060' ELSE 'O000' END ,CASE WHEN '" + gs_brand + "' = 'N' THEN 'A061' ELSE 'O000' END , CASE WHEN '" + gs_brand + "' = 'N' THEN 'N610' ELSE 'O000' END , CASE WHEN'" + gs_brand + "' = 'N' THEN 'B000' ELSE 'O000' END ,'B200','O200','O100','T000','K100','K400','F220')"
//			end if
...
///////////////
	...
	if LenA(ls_style) = 8 then 	
		...
		ls_year = '200' + MidA(ls_sample_cd,3,1)
