$PBExportHeader$w_sh204_d.srw
$PBExportComments$회원현황
forward
global type w_sh204_d from w_com000
end type
type dw_head from u_dw within w_sh204_d
end type
type ln_1 from line within w_sh204_d
end type
type ln_2 from line within w_sh204_d
end type
type dw_print from u_dw within w_sh204_d
end type
type dw_body from u_dw within w_sh204_d
end type
type dw_1 from datawindow within w_sh204_d
end type
end forward
...
type variables
string is_shop_cd, is_card_day, is_vip_grd, is_rfm_grd
datawindowchild	idw_rfm
...
il_rows = dw_body.retrieve(is_shop_cd, is_card_day, is_vip_grd, is_rfm_grd)
IF il_rows > 0 THEN
   dw_body.SetFocus()
...
if MidA(is_shop_cd,2,1) <> "K" then
   MessageBox(ls_title,"대리점만 사용 가능합니다!")
   return false
end if

is_card_day = dw_head.GetItemString(1, "card_day")
is_vip_grd = dw_head.GetItemString(1, "vip_grd")
...
if MidA(gs_shop_cd,2,1) <> "K" then
   MessageBox("알림!","대리점만 사용 가능합니다!")
   return 0
end if
end event

type cb_close from w_com000`cb_close within w_sh204_d
integer x = 2496
integer taborder = 100
end type

type cb_delete from w_com000`cb_delete within w_sh204_d
boolean visible = false
integer x = 1083
integer taborder = 50
end type

type cb_insert from w_com000`cb_insert within w_sh204_d
boolean visible = false
integer x = 741
integer taborder = 40
end type

type cb_retrieve from w_com000`cb_retrieve within w_sh204_d
integer x = 2153
integer taborder = 20
end type
...
type cb_update from w_com000`cb_update within w_sh204_d
boolean visible = false
integer taborder = 90
end type

type cb_print from w_com000`cb_print within w_sh204_d
boolean visible = false
integer x = 1426
integer taborder = 60
end type

type cb_preview from w_com000`cb_preview within w_sh204_d
boolean visible = false
integer x = 1769
integer taborder = 70
end type

type gb_button from w_com000`gb_button within w_sh204_d
integer width = 2894
integer taborder = 0
long backcolor = 16777215
end type
...
event constructor;call super::constructor;This.GetChild("rfm_grd", idw_rfm )
idw_rfm.SetTransObject(SQLCA)
idw_rfm.Retrieve('704')
idw_rfm.insertRow(1)
idw_rfm.Setitem(1, "inter_cd", "%")
idw_rfm.Setitem(1, "inter_nm", "전체")
end event
...
event clicked;call super::clicked;string ls_jumin, ls_user_name

if row > 0 then
	choose case dwo.name 
		case "card_no","user_name"
			ls_jumin 	 = this.getitemstring(row,"jumin")
			ls_user_name = this.getitemstring(row,"user_name")
			il_rows = dw_1.retrieve(ls_jumin)
			if il_rows > 0 then 
				dw_1.title= "고객구매정보(" + ls_user_name + ")" 
				dw_1.visible = true
			end if
	end choose
