		...
		Select brand,     year,     season,     
		  ...
		  into :ls_brand, :ls_year, :ls_season, 
			 ...
			 where brand = :gs_brand 
				...
				and color = :ls_color 
				and size  = :ls_size
  			   and plan_yn <>  case when substring(:gs_shop_cd,2,1)  = "I" then 'X' else 'Y' end					 
				and size    <>  case when  substring(:gs_shop_cd,3,6) = "2000" then 'CC' else 'XX' end
				and year + season in (	select year + season
												from tb_51036_d a (nolock), tb_51035_h b (nolock)
												where :is_yymmdd between a.frm_ymd and a.to_ymd
...
if is_chk_back = 'Y' and  MidA(gs_shop_cd,2,1) = "I" then 
...
//		else	
			if gs_brand = "B" or gs_brand = "P" or gs_brand = "K" or gs_brand = 'U' then 					
				...
				gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and plan_yn <> case when substring('"  + gs_shop_cd + "', 1,2) = 'NI' then 'X' else 'Y' end  and  year + season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where  '" + is_yymmdd + "' "  + &
											 " between a.frm_ymd and a.to_ymd and a.shop_cd =  '" + gs_shop_cd + "' and a.shop_cd = b.shop_cd and a.frm_ymd = b.frm_ymd and b.cancel <> 'Y') and sojae <> 'C' "  
			end if		
//		end if	
	end if
	...
	if MidA(gs_shop_CD,3,6) = "2000" then							
		...
		ELSe 
			...
			if is_chk_back = "Y" and is_back_ymd <> is_yymmdd and MidA(gs_shop_cd, 2,1) = "I" then 
						...
						Where style      = :ls_style 
						  ...
						  and start_ymd <= :is_back_ymd
						  and end_ymd   >= :is_back_ymd
						  and shop_type >= case when substring(shop_cd, 2,1) = 'I' then '3' else '4' end
						  and sale_type >= case when substring(shop_cd, 2,1) = 'I' then '33' else '40' end
