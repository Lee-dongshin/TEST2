$PBExportHeader$w_sh150_e.srw
$PBExportComments$RT 마켓 점간이동관리
forward
global type w_sh150_e from w_com010_e
end type
type st_1 from statictext within w_sh150_e
end type
type st_3 from statictext within w_sh150_e
end type
end forward
...
/* 수정일      : 2002.03.28                                                  */
/*===========================================================================*/
long i, ll_row_count, ll_move_qty 
datetime ld_datetime
String   ls_ymd,   ls_rtn_no, ls_no,    ls_to_shop_cd, ls_shop_type  , ls_rt_yn2
...
FOR i=1 TO ll_row_count
   ll_move_qty   = dw_body.GetitemNumber(i, "move_qty")
	ls_rt_yn2 = dw_body.getitemstring(i,"rt_yn2")
   ...
   ls_to_shop_cd = dw_body.GetitemString(i, "to_shop_cd")
	...
	if ls_rt_yn2 = 'Y' then
		...
		IF ll_move_qty <> 0 THEN 
			DECLARE SP_SH103_INSERT PROCEDURE FOR SP_SH103_INSERT 
					  @yymmdd     = :ls_ymd       , 
					  @shop_cd    = :gs_shop_cd   , 
...
type cb_close from w_com010_e`cb_close within w_sh150_e
end type

type cb_delete from w_com010_e`cb_delete within w_sh150_e
boolean visible = false
end type

type cb_insert from w_com010_e`cb_insert within w_sh150_e
boolean visible = false
end type

type cb_retrieve from w_com010_e`cb_retrieve within w_sh150_e
end type

type cb_update from w_com010_e`cb_update within w_sh150_e
end type

type cb_print from w_com010_e`cb_print within w_sh150_e
boolean visible = false
end type

type cb_preview from w_com010_e`cb_preview within w_sh150_e
boolean visible = false
end type

type gb_button from w_com010_e`gb_button within w_sh150_e
integer height = 160
long backcolor = 16777215
end type

type dw_head from w_com010_e`dw_head within w_sh150_e
integer width = 2811
integer height = 136
string dataobject = "d_sh150_h01"
end type

type ln_1 from w_com010_e`ln_1 within w_sh150_e
integer beginy = 320
integer endy = 320
end type

type ln_2 from w_com010_e`ln_2 within w_sh150_e
integer beginy = 324
integer endy = 324
end type

type dw_body from w_com010_e`dw_body within w_sh150_e
event ue_set_column ( long al_row )
integer y = 344
integer height = 1488
string dataobject = "d_sh150_d01"
...
/*===========================================================================*/
Long ll_ret 
string ls_rt_yn, ls_rt_yn2, ls_fr_shop_cd, ls_mk
...
CHOOSE CASE dwo.name
	...
	CASE "rt_yn2"
		if data = 'Y' then
			ls_rt_yn = this.GetitemString(row,"rt_yn")
			ls_mk    = this.GetitemString(row,"mk")
			
			if ls_mk = 'S' then
				ls_rt_yn = ls_mk + MidA(ls_rt_yn,2,1)
			else
				ls_rt_yn = MidA(ls_rt_yn,1,1) + ls_mk
			end if
			
			this.Setitem(row,"rt_yn",ls_rt_yn)
			
		elseif string(data) = 'N' then
			ls_rt_yn = this.GetitemString(row,"rt_yn")
			ls_mk    = this.GetitemString(row,"mk")
			
			if ls_mk = 'S' then
				ls_rt_yn = 'X' + MidA(ls_rt_yn,2,1)
			else
				ls_rt_yn = MidA(ls_rt_yn,1,1) + 'X'
			end if
...
type dw_print from w_com010_e`dw_print within w_sh150_e
end type
...
long textcolor = 255
long backcolor = 16777215
string text = "※ 거래승인된 제품을 점간이송 처리합니다."
boolean focusrectangle = false
end type
