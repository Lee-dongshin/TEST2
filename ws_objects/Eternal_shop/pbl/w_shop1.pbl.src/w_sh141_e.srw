...
into :ld_coupon_amt
from beaucre.dbo.tb_71011_p (nolock)
...
this.Control[iCurrent+17]=this.dw_member_sale
this.Control[iCurrent+18]=this.st_6
this.Control[iCurrent+19]=this.dw_14
this.Control[iCurrent+20]=this.dw_15
this.Control[iCurrent+21]=this.st_ok_coupon1
this.Control[iCurrent+22]=this.st_ok_coupon2
this.Control[iCurrent+23]=this.dw_16
this.Control[iCurrent+24]=this.shl_member
this.Control[iCurrent+25]=this.dw_17
this.Control[iCurrent+26]=this.dw_18
this.Control[iCurrent+27]=this.dw_19
this.Control[iCurrent+28]=this.dw_cosmetic
this.Control[iCurrent+29]=this.em_1
this.Control[iCurrent+30]=this.st_7
this.Control[iCurrent+31]=this.dw_20
this.Control[iCurrent+32]=this.dw_list
this.Control[iCurrent+33]=this.tab_1
this.Control[iCurrent+34]=this.dw_7
this.Control[iCurrent+35]=this.sle_id
this.Control[iCurrent+36]=this.st_online_id
this.Control[iCurrent+37]=this.st_online_gubn
this.Control[iCurrent+38]=this.rb_1
this.Control[iCurrent+39]=this.rb_2
this.Control[iCurrent+40]=this.cb_online_coupon
this.Control[iCurrent+41]=this.cb_back_sale
this.Control[iCurrent+42]=this.dw_head_2
this.Control[iCurrent+43]=this.dw_6
this.Control[iCurrent+44]=this.dw_3
this.Control[iCurrent+45]=this.dw_2
				...
				elseif gs_brand = "B" or gs_brand = "P" or gs_brand = "K" or gs_brand = 'L' or gs_brand = 'U' then 					
					...
					elseif gs_brand = 'B' or gs_brand = 'P' or gs_brand = 'K' or gs_brand = 'U' then
						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and (( ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20101' ) )" + &
										...
										" and a.shop_cd = '" + gs_shop_cd + "' and '" + is_yymmdd + "' between a.start_ymd and a.end_ymd " + &
										" and  a.year + a.season in (select year + season from tb_51036_d a (nolock), tb_51035_h b (nolock) where '" + is_yymmdd + "' between a.frm_ymd and a.to_ymd " + &
										" and a.shop_cd = '" + gs_shop_cd + "' and a.shop_cd = b.shop_cd and a.frm_ymd = b.frm_ymd and b.cancel <> 'Y') )) "
					else 
						gst_cd.default_where   = "WHERE brand = '" + gs_brand + "' and (( ( year + convert(char(01),dbo.sf_inter_sort_seq('003', SEASON)) > '20101' ) )" + &
										" or style in (select a.style from tb_56012_d a (nolock)	where a.shop_type = '3'	and a.sale_type = '33' " + &
										" and a.shop_cd = '" + gs_shop_cd + "' and '" + is_yymmdd + "' between a.start_ymd and a.end_ymd " + &
...
IF GS_SHOP_CD = "LG2988" OR GS_SHOP_CD = "KG2988" THEN
	st_online_gubn.TEXT = ""
	st_online_id.TEXT = "직원사번 :"
   rb_1.visible = FALSE
   rb_2.visible = FALSE	
END IF	
...
FOR i=1 TO ll_row_count
		...
		if LeftA(dw_body.GetItemString(i, "style_no"),1) = '8' then
			...
			ELSEif  MidA(gs_shop_cd,3,4) = "1191" then
				gs_shop_cd = gs_brand + 'G' + MidA(gs_shop_cd,3,4)
			ELSEif gs_shop_cd = "LG2988"  OR gs_shop_cd = "KG2988" then
				gs_shop_cd = GS_SHOP_CD			
...
elseif gs_shop_cd = 'LG2988' OR gs_shop_cd = 'KG2988' then
	dw_body.dataobject = 'd_sh141_d01'	
	dw_body.OBJECT.t_8.TEXT = "구매자 사번"
else
...
//messagebox("gs_shop_cd", gs_shop_cd)
//messagebox("ls_sale_no", ls_sale_no)
...
long ll_row_count, i
	...
	if is_coupon = 'Y' then
		messagebox('확인!','이미 신규가입 쿠폰을 사용 했습니다.')
		cb_online_coupon.enabled = false
		return
	end if
