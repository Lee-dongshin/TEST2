...
long ll_rowcnt
string ls_deposit_chk, ls_date, ld_datetime
...
string	ls_sale_type_tmp, ls_style, ls_season, ls_coupon_no_chk , ls_sale_no
		...
		if dw_body.GetItemString(i,"yymmdd") <= dw_body.GetItemString(i,"empty_4") then
			...
			if ts_sale_no = '0' then
				select right(isnull(max(SALE_NO), 0) + 10001, 4)
				  into :ts_sale_no
				 from tb_53010_h
				where yymmdd = :ps_yymmdd
				and shop_cd = :ps_shop_cd
				and shop_type = :ps_shop_type;
				
			messagebox('확인','예약판매일 ' + LeftA(ps_yymmdd,4) + '/' + MidA(ps_yymmdd,5,2) + '/' + MidA(ps_yymmdd,7,2) + ' 로 판매등록 되었습니다.')
			end if
