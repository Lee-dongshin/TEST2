...
//			Post Close(w_main01)
			...
			wstyle = 0
			ls_file_name = "C:\ETERNAL_SHOP\shop_install.exe"
   ...
   gs_shop_nm     = st_nm.text  
	...
	if MidA(ls_shop_cd, 2, 1) = 'X' then
		...
		if MidA(gs_shop_cd,3,4) = '1187' then 
			gs_brand_nm = 'Lounge B'
			gs_shop_nm = '롯데대구 복합매장'
		end if	
		
		if MidA(gs_shop_cd,3,4) = '1191' then 
			gs_brand_nm = 'Lounge B'
			gs_shop_nm = '롯데일산 복합매장'
		end if			

		if MidA(gs_shop_cd,3,4) = '1190' then 
			gs_brand_nm = 'Lounge B'
			gs_shop_nm = '롯데부산 복합매장'
		end if	

		
		if MidA(gs_shop_cd,3,4) = '1890' then 
			gs_brand_nm = '본사매장'
			gs_shop_nm = '본사매장'
		end if	
	else
...
string picturename = "C:\ETERNAL_SHOP\bmp\eternal.gif"
end type
...
event sle_userid::losefocus;call super::losefocus;IF LenA(This.Text) <> 0 THEN
	IF gf_shop_nm(This.text, 'S', st_nm.text) <> 0 THEN
		...
		ELSEif This.text = 'XX1187' THEN 
			st_nm.text = 'LOUNGE B 매장'			
		ELSEif This.text = 'XX1190' THEN 
			st_nm.text = 'LOUNGE B 매장'			
		ELSEif This.text = 'XX1191' THEN 
			st_nm.text = 'LOUNGE B 매장'				
