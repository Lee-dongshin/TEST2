...
event ue_pgm_open();
OpenSheet(w_kola_101_e, this, gi_menu_pos, Original! ) 

end event

event ue_report_open();
OpenSheet(w_kola_102_e, this, gi_menu_pos, Original! ) 
...
if gb_db_status Then
   // MS SQL session-id를 읽어온다.
   gf_get_session(gs_sessionid)
   This.Title = '보끄레머천다이징 시스템' + " [ " + gs_home_dir + " ] "
